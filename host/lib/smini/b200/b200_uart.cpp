...
struct b200_uart_impl : b200_uart
{
    ...
    void send_char(const char ch)
    {
        ...
        packet_buff[packet_info.num_header_words32+0] = shd::htowx(uint32_t(_baud_div));
        packet_buff[packet_info.num_header_words32+1] = shd::htowx(uint32_t(ch));
    ...
    void handle_uart_packet(managed_recv_buffer::sptr buff)
    {
        ...
        const char ch = char(shd::wtohx(packet_buff[packet_info.num_header_words32+1]));
        _line += ch;
        if (ch == '\n')
        {
