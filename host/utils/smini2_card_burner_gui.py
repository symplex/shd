...
import smini2_card_burner #import implementation
try:
...
class DeviceEntryWidget(tkinter.Frame):
    ...
    def _reload_cb(self):
        ...
        for hint in smini2_card_burner.get_raw_device_hints():
...
class SMINI2CardBurnerApp(tkinter.Frame):
    """
    The top level gui application for the smini2 sd card burner.
    ...
    def _burn(self):
        ...
        #burn the sd card
        try:
            verbose = smini2_card_burner.burn_sd_card(dev=dev, fw=fw, fpga=fpga)
...
if __name__=='__main__':
    options = smini2_card_burner.get_options()
    root = tkinter.Tk()
    root.title('SMINI2 SD Card Burner')
    SMINI2CardBurnerApp(root, dev=options.dev, fw=options.fw, fpga=options.fpga).pack()
    root.mainloop()
    exit()
