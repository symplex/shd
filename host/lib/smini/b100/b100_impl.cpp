...
static device_addrs_t b100_find(const device_addr_t &hint)
{
    ...
    BOOST_FOREACH(usb_device_handle::sptr handle, usb_device_handle::get_device_list(vid, pid)) {
        ...
        try{
            ...
            SHD_MSG(warning) << boost::format("Could not locate B100 firmware. %s\n") % print_utility_error("shd_images_downloader.py");
            return b100_addrs;
        }
        SHD_LOG << "the firmware image: " << b100_fw_image << std::endl;

        usb_control::sptr control;
        try{control = usb_control::make(handle, 0);}
        catch(const shd::exception &){continue;} //ignore claimed

        fx2_ctrl::make(control)->smini_load_firmware(b100_fw_image);
        found++;
    }
...
SHD_STATIC_BLOCK(register_b100_device){
    device::register_device(&b100_find, &b100_make, device::SMINI);
}
...
b100_impl::b100_impl(const device_addr_t &device_addr){
    ...
    _fx2_ctrl->smini_load_fpga(b100_fpga_image);
    _fx2_ctrl->smini_fpga_reset(false); //active low reset
    _fx2_ctrl->smini_fpga_reset(true);
...
void b100_impl::check_fw_compat(void){
    ...
    const uint16_t fw_compat_num = _fx2_ctrl->smini_control_read(
    ...
    if (fw_compat_num != B100_FW_COMPAT_NUM){
        throw shd::runtime_error(str(boost::format(
            ...
            "%s"
        ) % int(B100_FW_COMPAT_NUM) % fw_compat_num % print_utility_error("shd_images_downloader.py")));
    }
...
void b100_impl::check_fpga_compat(void){
    ...
    if (fpga_major != B100_FPGA_COMPAT_NUM){
        throw shd::runtime_error(str(boost::format(
            ...
            "%s"
        ) % int(B100_FPGA_COMPAT_NUM) % fpga_major % print_utility_error("shd_images_downloader.py")));
    }
...
void b100_impl::set_mb_eeprom(const shd::smini::mboard_eeprom_t &mb_eeprom){
    mb_eeprom.commit(*_fx2_ctrl, B100_EEPROM_MAP_KEY);
}

void b100_impl::set_db_eeprom(const std::string &type, const shd::smini::dboard_eeprom_t &db_eeprom){
...
void b100_impl::update_clock_source(const std::string &source){
    ...
    else throw shd::runtime_error("unhandled clock configuration reference source: " + source);
}

////////////////// some GPIF preparation related stuff /////////////////
void b100_impl::enable_gpif(const bool en) {
    _fx2_ctrl->smini_control_write(VRQ_ENABLE_GPIF, en ? 1 : 0, 0, 0, 0);
}

void b100_impl::clear_fpga_fifo(void) {
    _fx2_ctrl->smini_control_write(VRQ_CLEAR_FPGA_FIFO, 0, 0, 0, 0);
}
