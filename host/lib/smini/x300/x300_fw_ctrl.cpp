    ...
    void poke32(const wb_addr_type addr, const uint32_t data)
    {
        for (size_t i = 1; i <= num_retries; i++)
        {
            ...
            catch(const shd::io_error &ex)
            {
                ...
                if (errors) SHD_MSG(error) << error_msg << std::endl;
...
{
public:
    x300_ctrl_iface_enet(shd::transport::udp_simple::sptr udp, bool enable_errors = true):
...
protected:
    virtual void __poke32(const wb_addr_type addr, const uint32_t data)
    {
        ...
        request.flags = shd::htonx<uint32_t>(X300_FW_COMMS_FLAGS_ACK | X300_FW_COMMS_FLAGS_POKE32);
        request.sequence = shd::htonx<uint32_t>(seq++);
        request.addr = shd::htonx(addr);
        request.data = shd::htonx(data);
        ...
        if (nbytes == 0) throw shd::io_error("x300 fw poke32 - reply timed out");

        //sanity checks
        const size_t flags = shd::ntohx<uint32_t>(reply.flags);
        SHD_ASSERT_THROW(nbytes == sizeof(reply));
        SHD_ASSERT_THROW(not (flags & X300_FW_COMMS_FLAGS_ERROR));
        SHD_ASSERT_THROW(flags & X300_FW_COMMS_FLAGS_POKE32);
        SHD_ASSERT_THROW(flags & X300_FW_COMMS_FLAGS_ACK);
        SHD_ASSERT_THROW(reply.sequence == request.sequence);
        SHD_ASSERT_THROW(reply.addr == request.addr);
        SHD_ASSERT_THROW(reply.data == request.data);
    }
    ...
    virtual uint32_t __peek32(const wb_addr_type addr)
    {
        //load request struct
        x300_fw_comms_t request = x300_fw_comms_t();
        request.flags = shd::htonx<uint32_t>(X300_FW_COMMS_FLAGS_ACK | X300_FW_COMMS_FLAGS_PEEK32);
        request.sequence = shd::htonx<uint32_t>(seq++);
        request.addr = shd::htonx(addr);
        request.data = 0;
        ...
        x300_fw_comms_t reply = x300_fw_comms_t();
        const size_t nbytes = udp->recv(boost::asio::buffer(&reply, sizeof(reply)), 1.0);
        if (nbytes == 0) throw shd::io_error("x300 fw peek32 - reply timed out");

        //sanity checks
        const size_t flags = shd::ntohx<uint32_t>(reply.flags);
        SHD_ASSERT_THROW(nbytes == sizeof(reply));
        SHD_ASSERT_THROW(not (flags & X300_FW_COMMS_FLAGS_ERROR));
        SHD_ASSERT_THROW(flags & X300_FW_COMMS_FLAGS_PEEK32);
        SHD_ASSERT_THROW(flags & X300_FW_COMMS_FLAGS_ACK);
        SHD_ASSERT_THROW(reply.sequence == request.sequence);
        SHD_ASSERT_THROW(reply.addr == request.addr);

        //return result!
        return shd::ntohx<uint32_t>(reply.data);
    }
...
private:
    shd::transport::udp_simple::sptr udp;
    size_t seq;
};
...
public:
        ...
        x300_ctrl_iface(enable_errors), _drv_proxy(drv_proxy)
    {
        ...
        //user is switching to SHD after using LabVIEW FPGA.
        ...
        if (pcie_fpga_signature != FPGA_X3xx_SIG_VALUE)
            throw shd::io_error("cannot create x300_ctrl_iface_pcie. incorrect/no fpga image");
...
protected:
    virtual void __poke32(const wb_addr_type addr, const uint32_t data)
    {
        ...
        if (nirio_status_fatal(status))
            throw shd::io_error("x300 fw poke32 - hardware IO error");
        if (elapsed.total_milliseconds() > READ_TIMEOUT_IN_MS)
            throw shd::io_error("x300 fw poke32 - operation timed out");
    }
    ...
    virtual uint32_t __peek32(const wb_addr_type addr)
    {
        ...
        if (nirio_status_fatal(status))
            throw shd::io_error("x300 fw peek32 - hardware IO error");
        if (elapsed.total_milliseconds() > READ_TIMEOUT_IN_MS)
            throw shd::io_error("x300 fw peek32 - operation timed out");
...
wb_iface::sptr x300_make_ctrl_iface_enet(shd::transport::udp_simple::sptr udp, bool enable_errors = true)
{
