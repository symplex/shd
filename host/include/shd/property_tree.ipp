...
#ifndef INCLUDED_SHD_PROPERTY_TREE_IPP
#define INCLUDED_SHD_PROPERTY_TREE_IPP
...
public:
    ...
    property<T> &set_coercer(const typename property<T>::coercer_type &coercer){
        if (not _coercer.empty()) shd::assertion_error("cannot register more than one coercer for a property");
        if (_coerce_mode == property_tree::MANUAL_COERCE) shd::assertion_error("cannot register coercer for a manually coerced property");
    ...
    property<T> &set_publisher(const typename property<T>::publisher_type &publisher){
        if (not _publisher.empty()) shd::assertion_error("cannot register more than one publisher for a property");
    ...
    property<T> &set(const T &value){
        ...
        if (not _coercer.empty()) {
            ...
            if (_coerce_mode == property_tree::AUTO_COERCE) shd::assertion_error("coercer missing for an auto coerced property");
        }
        return *this;
    }

    property<T> &set_coerced(const T &value){
        if (_coerce_mode == property_tree::AUTO_COERCE) shd::assertion_error("cannot set coerced value an auto coerced property");
        _set_coerced(value);
        return *this;
    }
    ...
    const T get(void) const{
        if (empty()) {
            throw shd::runtime_error("Cannot get() on an uninitialized (empty) property");
        }
        if (not _publisher.empty()) {
            return _publisher();
        } else {
            if (_coerced_value.get() == NULL and _coerce_mode == property_tree::MANUAL_COERCE)
                throw shd::runtime_error("uninitialized coerced value for manually coerced attribute");
            return get_value_ref(_coerced_value);
        }
    }

    const T get_desired(void) const{
        if (_value.get() == NULL) throw shd::runtime_error("Cannot get_desired() on an uninitialized (empty) property");
...
private:
    ...
    static const T& get_value_ref(const boost::scoped_ptr<T>& scoped_value) {
        if (scoped_value.get() == NULL) throw shd::assertion_error("Cannot use uninitialized property data");
...
#endif /* INCLUDED_SHD_PROPERTY_TREE_IPP */
