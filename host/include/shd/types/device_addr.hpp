...
#ifndef INCLUDED_SHD_TYPES_DEVICE_ADDR_HPP
#define INCLUDED_SHD_TYPES_DEVICE_ADDR_HPP
...
namespace shd{
     ...
     * - Ex, to find a smini2: my_dev_addr["addr"] = [resolvable_hostname_or_ip]
     *
     ...
     */
    class SHD_API device_addr_t : public dict<std::string, std::string>{
    public:
        /*!
    ...
    SHD_API device_addrs_t separate_device_addr(const device_addr_t &dev_addr);

    //! Combine a vector of device addresses into an indexed device address
    SHD_API device_addr_t combine_device_addrs(const device_addrs_t &dev_addrs);

} //namespace shd

#endif /* INCLUDED_SHD_TYPES_DEVICE_ADDR_HPP */
