...
void sig_int_handler(int){
    ...
    if(num_ctrl_c == 1){
        std::cout << std::endl << "Are you sure you want to abort the image burning? If you do, your "
                                  "SMINI-X series device will be bricked!" << std::endl
                               << "Press Ctrl+C again to abort the image burning procedure." << std::endl << std::endl;
    }
    else{
        std::cout << std::endl << "Aborting. Your SMINI X-Series device will be bricked." << std::endl
                  << "http://files.ettus.com/manual/page_smini_x3x0.html#x3x0_load_fpga_imgs_jtag" << std::endl
...
void list_sminis(){
    device_addrs_t found_devices = device::find(device_addr_t("type=x300"), device::SMINI);
...
device_addr_t find_smini_with_ethernet(std::string ip_addr, bool output){
    ...
    device_addrs_t found_devices = device::find(dev, device::SMINI);
...
device_addr_t find_smini_with_pcie(std::string resource, bool output){
...
std::string get_default_image_path(std::string model, std::string image_type){
    ...
    std::string image_name = str(boost::format("smini_%s_fpga_%s.bit")
...
void print_image_loader_warning(const std::string &fpga_path, const po::variables_map &vm){
    ...
    // Generate equivalent shd_image_loader command
    std::string shd_image_loader = "shd_image_loader --args=\"type=x300";

    if(vm.count("addr") > 0){
        shd_image_loader += str(boost::format(",addr=%s")
                                % vm["addr"].as<std::string>());

        if(vm.count("configure") > 0){
            shd_image_loader += ",configure";
        }

        if(vm.count("verify") > 0){
            shd_image_loader += ",verify";
        }
    }
    else{
        shd_image_loader += str(boost::format(",resource=%s")
        ...
        if(vm["rpc-port"].as<std::string>() != "5444"){
            shd_image_loader += str(boost::format(",rpc-port=%s")
                                    % vm["rpc-port"].as<std::string>());
        }
    }

    if(vm.count("type") > 0){
        shd_image_loader += str(boost::format(",fpga=%s")
...
void pcie_burn(std::string resource, std::string rpc_port, std::string fpga_path)
{
    ...
    shd::nisminirio::nisminirio_session fpga_session(resource, rpc_port);
...
int SHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    //Print help message
    if(vm.count("help")){
        std::cout << "SMINI X3x0 FPGA Burner" << std::endl << std::endl;

        std::cout << "Burns an FPGA image onto a SMINI X300/X310. To burn the image" << std::endl
     ...
     */
    device_addr_t dev = (vm.count("addr")) ? find_smini_with_ethernet(ip_addr, true)
                                           : find_smini_with_pcie(resource, true);
    ...
    /*
     * Attempt to find SMINI after burning
     */
    ...
    device_addr_t found_smini = (vm.count("addr")) ? find_smini_with_ethernet(ip_addr, false)
                                                  : find_smini_with_pcie(resource, false);
    ...
    if(vm.count("addr")) std::cout << str(boost::format("Power-cycle the SMINI %s to use the new image.") % found_smini["product"]) << std::endl;
    else std::cout << str(boost::format("Power-cycle the SMINI %s and reboot your machine to use the new image.") % found_smini["product"]) << std::endl;
