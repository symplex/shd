...
public:
                            ...
                            _delay(0)
    {
        ...
        if (_fd < 0)
            throw shd::runtime_error(str(boost::format("Could not open spidev device %s") % device));

        ret = ioctl(_fd, SPI_IOC_WR_MODE, &_mode);
        if (ret == -1)
            throw shd::runtime_error("Could not set spidev mode");

        ret = ioctl(_fd, SPI_IOC_RD_MODE, &_mode);
        if (ret == -1)
            throw shd::runtime_error("Could not get spidev mode");

        ret = ioctl(_fd, SPI_IOC_WR_BITS_PER_WORD, &_bits);
        if (ret == -1)
            throw shd::runtime_error("Could not set spidev bits per word");

        ret = ioctl(_fd, SPI_IOC_RD_BITS_PER_WORD, &_bits);
        if (ret == -1)
            throw shd::runtime_error("Could not get spidev bits per word");

        ret = ioctl(_fd, SPI_IOC_WR_MAX_SPEED_HZ, &_speed);
        if (ret == -1)
            throw shd::runtime_error("Could not set spidev max speed");

        ret = ioctl(_fd, SPI_IOC_RD_MAX_SPEED_HZ, &_speed);
        if (ret == -1)
            throw shd::runtime_error("Could not get spidev max speed");
    }
    ...
    uint32_t transact_spi(int, const shd::spi_config_t &,
                                 ...
                                 bool)
    {
        ...
        if (ret < 1)
            throw shd::runtime_error("Could not send spidev message");
...
{
    throw shd::assertion_error("spi::make() !E300_NATIVE");
}
}}};
#endif //E300_NATIVE
