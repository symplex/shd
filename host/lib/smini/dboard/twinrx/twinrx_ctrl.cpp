...
public:
    ...
    ) : _db_iface(db_iface), _gpio_iface(gpio_iface), _cpld_regs(cpld_regmap)
    {
        ...
        while (_gpio_iface->get_field(twinrx_gpio::FIELD_SWPS_PWR_GOOD) == 0) {
            ...
            if (--timeout_ms == 0) {
                throw shd::runtime_error("power supply failure");
            }
        }
        ...
        BOOST_FOREACH(double rate, _db_iface->get_clock_rates(dboard_iface::UNIT_TX)) {
            found_rate |= shd::math::frequencies_are_equal(rate, TWINRX_DESIRED_REFERENCE_FREQ);
        }
        BOOST_FOREACH(double rate, _db_iface->get_clock_rates(dboard_iface::UNIT_RX)) {
            found_rate |= shd::math::frequencies_are_equal(rate, TWINRX_DESIRED_REFERENCE_FREQ);
        }
        if (not found_rate) {
            throw shd::runtime_error("TwinRX not supported on this motherboard");
        }
...
//            _lo1_iface[i]->set_feedback_select(adf5355_iface::FB_SEL_DIVIDED);
    ...
    void set_crossover_cal_mode(cal_mode_t cal_mode, bool commit = true)
    {
        ...
        if (_lo1_export == LO_CH1_SYNTH && cal_mode == CAL_CH2) {
            throw shd::runtime_error("cannot enable cal crossover on CH2 when LO1 in CH1 is exported");
        }
        if (_lo1_export == LO_CH2_SYNTH && cal_mode == CAL_CH1) {
            throw shd::runtime_error("cannot enable cal crossover on CH1 when LO1 in CH2 is exported");
        }
...
private:    //Members
    ...
    struct tune_freq_t : public shd::math::fp_compare::fp_compare_delta<double> {
        tune_freq_t() : shd::math::fp_compare::fp_compare_delta<double>(
            0.0, shd::math::FREQ_COMPARISON_DELTA_HZ) {}

        tune_freq_t(double freq) : shd::math::fp_compare::fp_compare_delta<double>(
            freq, shd::math::FREQ_COMPARISON_DELTA_HZ) {}
    };
