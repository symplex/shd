...
void print_direction(const struct pbuf_info *packet_buffer, const struct in_addr *host_addr, const struct in_addr *smini_addr)
{
 ...
 if ((host_addr->s_addr == ip_header->ip_src.s_addr) && (smini_addr->s_addr == ip_header->ip_dst.s_addr))
   fprintf(stdout,"Host->SMINI");
 else if  ((host_addr->s_addr == ip_header->ip_dst.s_addr) && (smini_addr->s_addr == ip_header->ip_src.s_addr))
    fprintf(stdout,"SMINI->Host");
 else
...
void print_vita_header(const struct pbuf_info *packet_buffer, const struct in_addr *host_addr)
{
  ...
  // Determine SMINI Sink/Src Endpoint
  if (direction==H2U)
...
// Find IP addresses for Host and SMINI in this Session

void get_connection_endpoints( struct pbuf_info *packet_buffer,  struct in_addr *host_addr, struct in_addr *smini_addr)
{
  const struct ip_header *ip_header;
  const struct chdr_header *chdr_header;
  const struct chdr_sid *chdr_sid;

  // Determine which side of the stream is Host and which is SMINI by probing capture until a
  // CHDR message type is discovered. The SID reveals which direction the packet is traveling.
  // Then record apparent IP addresses of Host and SMINI for future packet clasification.
  packet_buffer->current = packet_buffer->start;

  host_addr->s_addr = 0x0;
  smini_addr->s_addr = 0x0;
  ...
  while (packet_buffer->current != NULL) {
    ...
    // By convention currently the host uses SID address 0.x so the first packets in a new SHD session
	...
	if ((chdr_sid->src_device == 0) && (chdr_sid->dst_device != 0))
	  {
	    ...
	    smini_addr->s_addr = ip_header->ip_dst.s_addr;
	    break;
	  }
	else if ((chdr_sid->src_device == 0) && (chdr_sid->dst_device != 0))
	  {
	    // SMINI->Host
	    smini_addr->s_addr = ip_header->ip_src.s_addr;
  ...
  if (host_addr->s_addr == 0) {
    fprintf(stderr, "Could not identify Host/SMINI direction in capture analysis, exiting.\n");
    exit(2);
  }
}
