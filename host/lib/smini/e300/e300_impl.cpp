    ...
    const std::string& addr_hint, const std::string& port)
{
    ...
    try {
        ...
        SHD_MSG(error) << "E300 Network discovery unknown error" << std::endl;
        return addrs;
    }
    ...
    udp_bcast_xport->send(boost::asio::buffer(&req, sizeof(req)));
        ...
        SHD_MSG(error) << "E300 Network discovery error " << ex.what() << std::endl;
        return addrs;
    } catch(...) {
        SHD_MSG(error) << "E300 Network discovery unknown error" << std::endl;
        return addrs;
 ...
 * Structors
 **********************************************************************/
e300_impl::e300_impl(const shd::device_addr_t &device_addr)
    : _device_addr(device_addr)
    , _xport_path(device_addr.has_key("addr") ? ETH : AXI)
    , _sid_framer(0)
{
    _type = shd::device::SMINI;
    ...
    if (_xport_path == ETH) {
        ...
        shd::transport::zero_copy_xport_params sensor_xport_params;
...
#ifdef E300_GPSD
    SHD_MSG(status) << "Detecting internal GPS " << std::flush;
    try {
        if (_xport_path == AXI)
        ...
        SHD_MSG(error) << "An error occured making GPSDd interface: " << e.what() << std::endl;
    }
    ...
    if (_gps) {
        ...
        SHD_MSG(status) << (_gps->gps_detected() ? "found" : "not found") << std::endl;
    }
#endif

    // Verify we can talk to the e300 core control registers ...
    SHD_MSG(status) << "Initializing core control..." << std::endl;
...
shd::sensor_value_t e300_impl::_get_fe_pll_lock(const bool is_tx)
{
    const uint32_t st =
        ...
        const std::string &direction)
{
    ...
    if (chan_count > max_chans) {
        ...
        const double min_tick_rate = shd::smini::e300::MIN_TICK_RATE / ((chan_count <= 1) ? 1 : 2);
...
{
    SHD_MSG(status) << "Asking for clock rate " << rate/1e6 << " MHz\n";
    _tick_rate = _codec_ctrl->set_clock_rate(rate);
    SHD_MSG(status) << "Actually got clock rate " << _tick_rate/1e6 << " MHz\n";
...
uint32_t e300_impl::_get_version(compat_t which)
{
    ...
    default:
        throw shd::value_error("Requested unknown version.");
    };
}
...
{
    SHD_LOG << boost::format("Setting up dest map for 0x%lx to be stream %d")
...
{
    SHD_MSG(status) << boost::format("Setting time source to %s") % source << std::endl;
...
void e300_impl::_set_time(const shd::time_spec_t& t)
{
    BOOST_FOREACH(radio_perifs_t &perif, _radio_perifs)
        ...
        uint8_t prefix)
{
    ...
    throw shd::value_error(str(boost::format("No UDP port defined for combination: %u %u") % destination % prefix));
}

e300_impl::both_xports_t e300_impl::_make_transport(
    const uint8_t &destination,
    const uint8_t &prefix,
    const shd::transport::zero_copy_xport_params &params,
    uint32_t &sid)
{
...
void e300_impl::_update_antenna_sel(const size_t &which, const std::string &ant)
{
    if (ant != "TX/RX" and ant != "RX2")
        throw shd::value_error("Unknown RX antenna option: " + ant);
...
SHD_STATIC_BLOCK(register_e300_device)
{
    device::register_device(&shd::smini::e300::e300_find, &shd::smini::e300::e300_make, shd::device::SMINI);
}
