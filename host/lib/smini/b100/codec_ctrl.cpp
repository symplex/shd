...
void b100_codec_ctrl_impl::send_reg(uint8_t addr){
    ...
    SHD_LOGV(rarely) << "codec control write reg: " << std::hex << reg << std::endl;
    _iface->transact_spi(
        B100_SPI_SS_AD9862,
...
void b100_codec_ctrl_impl::recv_reg(uint8_t addr){
    ...
    SHD_LOGV(rarely) << "codec control read reg: " << std::hex << reg << std::endl;
    uint32_t ret = _iface->transact_spi(
        B100_SPI_SS_AD9862,
        spi_config_t::EDGE_RISE,
        reg, 16, true /*rb*/
    );
    SHD_LOGV(rarely) << "codec control read ret: " << std::hex << uint16_t(ret & 0xFF) << std::endl;
