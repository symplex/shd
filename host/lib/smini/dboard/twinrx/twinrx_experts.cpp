...
using namespace shd::smini::dboard::twinrx;
...
void twinrx_nyquist_expert::resolve()
{
    ...
    _db_iface->set_fe_connection(dboard_iface::UNIT_RX, _channel,
        smini::fe_connection_t(_codec_conn, _if_freq_c));
}
...
void twinrx_chan_gain_expert::resolve()
{
    if (_gain_profile != "default") {
        //TODO: Implement me!
        throw shd::not_implemented_error("custom gain strategies not implemeted yet");
    }
...
{
    static const shd::dict<std::string, twinrx_ctrl::lo_source_t> src_lookup =
    ...
    if (src_lookup.has_key(_lo_source_ch0)) {
        ...
        throw shd::value_error("Invalid LO source for channel 0.Choose from {internal, external, companion, reimport}");
    }
    if (src_lookup.has_key(_lo_source_ch1)) {
        _lo1_src_ch1 = _lo2_src_ch1 = src_lookup[_lo_source_ch1];
    } else {
        throw shd::value_error("Invalid LO source for channel 1.Choose from {internal, external, companion, reimport}");
    }

    twinrx_ctrl::lo_export_source_t export_src = twinrx_ctrl::LO_EXPORT_DISABLED;
    if (_lo_export_ch0 and (_lo_source_ch0 == "external")) {
        throw shd::value_error("Cannot export an external LO for channel 0");
    }
    if (_lo_export_ch1 and (_lo_source_ch1 == "external")) {
        throw shd::value_error("Cannot export an external LO for channel 1");
    }

    // Determine which channel will provide the exported LO
    if (_lo_export_ch0 and _lo_export_ch1) {
        throw shd::value_error("Cannot export LOs for both channels");
...
void twinrx_antenna_expert::resolve()
{
    ...
    if (_antenna_ch0 == ANT0 and _antenna_ch1 == ANT1) {
        ...
        throw shd::value_error("Invalid antenna selection " + _antenna_ch0.get() + " for channel 0. Must be " + ANT0 + " or " + ANT1);
    } else if (_antenna_ch1 != ANT0 and _antenna_ch1 != ANT1) {
        throw shd::value_error("Invalid antenna selection " + _antenna_ch1.get() + " for channel 1. Must be " + ANT0 + " or " + ANT1);
    }
    ...
    if (_cal_mode == twinrx_ctrl::CAL_CH1 and _lo_export_ch1) {
        throw shd::value_error("Cannot calibrate channel 0 and export the LO for channel 1.");
    } else if (_cal_mode == twinrx_ctrl::CAL_CH2 and _lo_export_ch0) {
        throw shd::value_error("Cannot calibrate channel 1 and export the LO for channel 0.");
    }
}
...
void twinrx_ant_gain_expert::resolve()
{
    ...
    case twinrx_ctrl::ANT1_SHARED:
        ...
        {
            SHD_MSG(warning) << "incompatible gain settings for antenna sharing. temporarily using Ch0 settings for Ch1.";
        }
        _ant0_input_atten       = _ch0_input_atten;
        _ant0_preamp1           = _ch0_preamp1;
        _ant0_preamp2           = _ch0_preamp2;
...
void twinrx_settings_expert::_resolve_lox_freq(
    lo_stage_t                           lo_stage,
    shd::experts::data_reader_t<double>& ch0_freq_d,
    shd::experts::data_reader_t<double>& ch1_freq_d,
    shd::experts::data_writer_t<double>& ch0_freq_c,
    shd::experts::data_writer_t<double>& ch1_freq_c,
...
double twinrx_settings_expert::_set_lox_synth_freq(lo_stage_t stage, twinrx_ctrl::channel_t ch, double freq)
{
    ...
    if (stage == STAGE_LO1) {
        ...
        throw shd::assertion_error("Invalid LO stage");
    }
