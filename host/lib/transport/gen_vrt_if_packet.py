...
SHD_INLINE static uint32_t chdr_to_vrt(const uint32_t chdr, if_packet_info_t &info)
{
...
SHD_INLINE static uint32_t vrt_to_chdr(const uint32_t vrt, const if_packet_info_t &info)
{
 ...
 **********************************************************************/
SHD_INLINE void __if_hdr_pack_${suffix}(
 ...
 **********************************************************************/
SHD_INLINE void __if_hdr_unpack_${suffix}(
    ...
    if (if_packet_info.num_packet_words32 < packet_words32)
        throw shd::value_error("bad vrt header or packet fragment");
...
<% num_header_words += 1 %>
        ...
        ########## Variables ##########
            ...
            if (packet_words32 < ${num_header_words + num_trailer_words})
                throw shd::value_error("bad vrt header or invalid packet length");
            if_packet_info.num_header_words32 = ${num_header_words};
...
void vrt::if_hdr_unpack_${suffix}(
    ...
    case if_packet_info_t::LINK_TYPE_VRLP:
    {
        if (${XE_MACRO}(packet_buff[0]) != VRLP) throw shd::value_error("bad vrl header VRLP");
        const uint32_t vrl_hdr = ${XE_MACRO}(packet_buff[1]);
        vrt_hdr_word32 = ${XE_MACRO}(packet_buff[2]);
        if (if_packet_info.num_packet_words32 < (vrl_hdr & 0xfffff)) throw shd::value_error("bad vrl header or packet fragment");
        if (${XE_MACRO}(packet_buff[(vrl_hdr & 0xfffff)-1]) != VEND) throw shd::value_error("bad vrl trailer VEND");
