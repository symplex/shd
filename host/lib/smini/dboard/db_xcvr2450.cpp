...
static const shd::dict<std::string, gain_range_t> xcvr_tx_gain_ranges = map_list_of
    ("VGA", gain_range_t(0, 30, 0.5))
    ("BB", gain_range_t(0, 5, 1.5))
;
static const shd::dict<std::string, gain_range_t> xcvr_rx_gain_ranges = map_list_of
...
private:
    ...
    shd::dict<std::string, double> _tx_gains, _rx_gains;
...
SHD_STATIC_BLOCK(reg_xcvr2450_dboard){
...
void xcvr2450::update_atr(void){
    ...
    //due to the issue that SMINI1 will take the value of full duplex for its TXATR.
...
double xcvr2450::set_lo_freq_core(double target_freq){
    ...
    if(_lo_freq > (5.35e9 + 5.47e9)/2.0){
        SHD_LOGV(often) << "XCVR2450 tune: Using  high-high band" << std::endl;
        _max2829_regs.band_select_802_11a = max2829_regs_t::BAND_SELECT_802_11A_5_47GHZ_TO_5_875GHZ;
    }else{
        SHD_LOGV(often) << "XCVR2450 tune: Using  low-high band" << std::endl;
...
static int gain_to_tx_vga_reg(double &gain){
    ...
    int reg = shd::clip(boost::math::iround(gain*60/30.0) + 3, 0, 63);
...
static max2829_regs_t::tx_baseband_gain_t gain_to_tx_bb_reg(double &gain){
    int reg = shd::clip(boost::math::iround(gain*3/5.0), 0, 3);
    switch(reg){
    case 0:
        gain = 0;
...
static int gain_to_rx_vga_reg(double &gain){
    int reg = shd::clip(boost::math::iround(gain/2.0), 0, 31);
    gain = double(reg*2);
    return reg;
}
...
static int gain_to_rx_lna_reg(double &gain){
    int reg = shd::clip(boost::math::iround(gain*2/30.5) + 1, 0, 3);
    switch(reg){
    case 0:
    case 1: gain = 0;    break;
...
static max2829_regs_t::tx_lpf_coarse_adj_t bandwidth_to_tx_lpf_coarse_reg(double &bandwidth){
    int reg = shd::clip(boost::math::iround((bandwidth-6.0e6)/6.0e6), 1, 3);
...
static max2829_regs_t::rx_lpf_fine_adj_t bandwidth_to_rx_lpf_fine_reg(double &bandwidth, double requested_bandwidth){
    int reg = shd::clip(boost::math::iround((requested_bandwidth/bandwidth)/0.05), 18, 22);
...
static max2829_regs_t::rx_lpf_coarse_adj_t bandwidth_to_rx_lpf_coarse_reg(double &bandwidth){
    int reg = shd::clip(boost::math::iround((bandwidth-7.0e6)/1.0e6), 0, 11);
