...
#include "../../../host/lib/smini/n230/n230_fw_host_iface.h"
...
bool read_n230_eeprom()
{
    ...
    if (!status)  {
        SHD_FW_TRACE(WARN, "read_n230_eeprom: Initialized cache to the default map.");
...
bool write_n230_eeprom()
{
    ...
    if (sizeof(n230_eeprom_map_t) > flash_session.device->page_size) {
        SHD_FW_TRACE(ERROR, "write_n230_eeprom: sizeof(n230_eeprom_map_t) > flash_page_size");
        return false;
    }
    ...
    if (cache_dirty) {
        ...
        if (memcmp(&eeprom_cache, &device_eeprom, sizeof(n230_eeprom_map_t)) != 0) {
            ...
            SHD_FW_TRACE(DEBUG, "write_n230_eeprom: Writing data to flash...");
            ...
            if (!status) {
                SHD_FW_TRACE(ERROR, "write_n230_eeprom: Operation failed!");
            }
            cache_dirty = !status;
        } else {
            SHD_FW_TRACE(DEBUG, "write_n230_eeprom: No new data. Write skipped.");
...
void read_n230_fpga_image_page(uint32_t offset, void *buf, uint32_t num_bytes)
{
    if (offset >= (N230_FLASH_NUM_FPGA_IMAGES * N230_FLASH_FPGA_IMAGE_SIZE)) {
        SHD_FW_TRACE_FSTR(ERROR, "read_n230_fpga_image_page: Offset 0x%x out of bounds", offset);
    }
...
{
    if (offset >= (N230_FLASH_NUM_FPGA_IMAGES * N230_FLASH_FPGA_IMAGE_SIZE)) {
        SHD_FW_TRACE_FSTR(ERROR, "write_n230_fpga_image_page: Offset 0x%x out of bounds", offset);
        return false;
    }
...
{
    if (offset >= (N230_FLASH_NUM_FPGA_IMAGES * N230_FLASH_FPGA_IMAGE_SIZE)) {
        SHD_FW_TRACE_FSTR(ERROR, "erase_n230_fpga_image_sector: Offset 0x%x out of bounds", offset);
        return false;
    }
