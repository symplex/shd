...
public:
    ...
    {
        SHD_MSG(status) << "Configuring a tick rate of " << rate/1e6 << " MHz... ";
        _tick_rate = _codec_ctrl->set_clock_rate(rate);
        SHD_MSG(status) << "got " << _tick_rate/1e6 << " MHz\n";
    ...
    void set_clock_source(const std::string &source)
    {
        ...
        if (source == "internal") {
            ...
            throw shd::key_error("set_clock_source: unknown source: " + source);
        }
    ...
    shd::sensor_value_t get_ref_locked()
    {
        bool locked = false;
...
using namespace shd::smini::n230;
