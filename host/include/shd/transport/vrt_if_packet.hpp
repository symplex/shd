...
#ifndef INCLUDED_SHD_TRANSPORT_VRT_IF_PACKET_HPP
#define INCLUDED_SHD_TRANSPORT_VRT_IF_PACKET_HPP
...
namespace vrt{
     ...
     */
    struct SHD_API if_packet_info_t
    {
     ...
     *   will throw a shd::value_error.
     *
     * \param packet_buff memory to read the packed vrt header
     * \param if_packet_info the if packet info (read/write)
     */
    SHD_API void if_hdr_unpack_be(
     ...
     * \param if_packet_info the if packet info (read/write)
     */
    SHD_API void if_hdr_unpack_le(
        const uint32_t *packet_buff,
        if_packet_info_t &if_packet_info
    );

    SHD_INLINE if_packet_info_t::if_packet_info_t(void):
...
#endif /* INCLUDED_SHD_TRANSPORT_VRT_IF_PACKET_HPP */
