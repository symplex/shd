...
static hash_type generate_hash(const char *filename)
{
    ...
    if (count == 0){
        throw shd::io_error(std::string("empty input file ") + filename);
    }
...
public:
    ...
    void write_i2c(SHD_UNUSED(uint16_t addr), SHD_UNUSED(const byte_vector_t &bytes))
    {
        throw shd::not_implemented_error("b200 write i2c");
    }


    byte_vector_t read_i2c(SHD_UNUSED(uint16_t addr), SHD_UNUSED(size_t num_bytes))
    {
        throw shd::not_implemented_error("b200 read i2c");
    }
    ...
    void write_eeprom(uint16_t addr, uint16_t offset,
        ...
        if (ret < 0)
            throw shd::io_error((boost::format("Failed to write EEPROM (%d: %s)") % ret % libusb_error_name(ret)).str());
        else if ((size_t)ret != bytes.size())
            throw shd::io_error((boost::format("Short write on write EEPROM (expecting: %d, returned: %d)") % bytes.size() % ret).str());
    }
    ...
    byte_vector_t read_eeprom(
        ...
        if (bytes_read < 0)
            throw shd::io_error((boost::format("Failed to read EEPROM (%d: %s)") % bytes_read % libusb_error_name(bytes_read)).str());
        else if ((size_t)bytes_read != num_bytes)
            throw shd::io_error((boost::format("Short read on read EEPROM (expecting: %d, returned: %d)") % num_bytes % bytes_read).str());

        return recv_bytes;
    }

    void load_firmware(const std::string filestring, SHD_UNUSED(bool force) = false)
    {
        if (load_img_msg)
            SHD_MSG(status) << "Loading firmware image: "
        ...
        } catch (const shd::io_error &e) {
            throw shd::io_error(str(boost::format("Could not load firmware: \n%s") % e.what()));
        }

        SHD_MSG(status) << std::endl;
    ...
    void reset_fx3(void) {
        ...
        if (ret < 0)
            throw shd::io_error((boost::format("Failed to reset FX3 (%d: %s)") % ret % libusb_error_name(ret)).str());
        else if (ret != bytes_to_send)
            throw shd::io_error((boost::format("Short write on reset FX3 (expecting: %d, returned: %d)") % bytes_to_send % ret).str());
    }
    ...
    void reset_gpif(void) {
        ...
        if (ret < 0)
            throw shd::io_error((boost::format("Failed to reset GPIF (%d: %s)") % ret % libusb_error_name(ret)).str());
        else if (ret != bytes_to_send)
            throw shd::io_error((boost::format("Short write on reset GPIF (expecting: %d, returned: %d)") % bytes_to_send % ret).str());
    }
    ...
    void set_fpga_reset_pin(const bool reset) {
        ...
        // Below is dead code as long as SHD_THROW_INVALID_CODE_PATH(); is declared above.
        ...
        if (ret < 0)
            throw shd::io_error((boost::format("Failed to reset FPGA (%d: %s)") % ret % libusb_error_name(ret)).str());
        else if (ret != bytes_to_send)
            throw shd::io_error((boost::format("Short write on reset FPGA (expecting: %d, returned: %d)") % bytes_to_send % ret).str());
        */
    }
    ...
    uint8_t get_usb_speed(void) {
        ...
        if (ret < 0)
            throw shd::io_error((boost::format("Failed to get USB speed (%d: %s)") % ret % libusb_error_name(ret)).str());
        else if (ret != bytes_to_recv)
            throw shd::io_error((boost::format("Short read on get USB speed (expecting: %d, returned: %d)") % bytes_to_recv % ret).str());
    ...
    uint8_t get_fx3_status(void) {
        ...
        if (ret < 0)
            throw shd::io_error((boost::format("Failed to get FX3 status (%d: %s)") % ret % libusb_error_name(ret)).str());
        else if (ret != bytes_to_recv)
            throw shd::io_error((boost::format("Short read on get FX3 status (expecting: %d, returned: %d)") % bytes_to_recv % ret).str());
    ...
    uint16_t get_compat_num(void) {
        ...
        if (ret < 0)
            throw shd::io_error((boost::format("Failed to get compat num (%d: %s)") % ret % libusb_error_name(ret)).str());
        else if (ret != bytes_to_recv)
            throw shd::io_error((boost::format("Short read on get compat num (expecting: %d, returned: %d)") % bytes_to_recv % ret).str());
    ...
    void smini_get_firmware_hash(hash_type &hash) {
        ...
        if (sizeof(hash_type) != bytes_to_recv)
            throw shd::type_error((boost::format("hash_type is %d bytes but transfer length is %d bytes") % sizeof(hash_type) % bytes_to_recv).str());

        int ret = fx3_control_read(B200_VREQ_GET_FW_HASH, 0x00, 0x00, (unsigned char*) &hash, bytes_to_recv, 500);
        if (ret < 0)
            throw shd::io_error((boost::format("Failed to get firmware hash (%d: %s)") % ret % libusb_error_name(ret)).str());
        else if (ret != bytes_to_recv)
            throw shd::io_error((boost::format("Short read on get firmware hash (expecting: %d, returned: %d)") % bytes_to_recv % ret).str());
    }

    void smini_set_firmware_hash(hash_type hash) {
        const int bytes_to_send = 4;
        if (sizeof(hash_type) != bytes_to_send)
            throw shd::type_error((boost::format("hash_type is %d bytes but transfer length is %d bytes") % sizeof(hash_type) % bytes_to_send).str());

        int ret = fx3_control_write(B200_VREQ_SET_FW_HASH, 0x00, 0x00, (unsigned char*) &hash, bytes_to_send);
        if (ret < 0)
            throw shd::io_error((boost::format("Failed to set firmware hash (%d: %s)") % ret % libusb_error_name(ret)).str());
        else if (ret != bytes_to_send)
            throw shd::io_error((boost::format("Short write on set firmware hash (expecting: %d, returned: %d)") % bytes_to_send % ret).str());
    }
    ...
    void smini_get_fpga_hash(hash_type &hash) {
        ...
        if (ret < 0)
            throw shd::io_error((boost::format("Failed to get FPGA hash (%d: %s)") % ret % libusb_error_name(ret)).str());
        else if (ret != bytes_to_recv)
            throw shd::io_error((boost::format("Short read on get FPGA hash (expecting: %d, returned: %d)") % bytes_to_recv % ret).str());
    }
    ...
    void smini_set_fpga_hash(hash_type hash) {
        ...
        if (ret < 0)
            throw shd::io_error((boost::format("Failed to set FPGA hash (%d: %s)") % ret % libusb_error_name(ret)).str());
        else if (ret != bytes_to_send)
            throw shd::io_error((boost::format("Short write on set FPGA hash (expecting: %d, returned: %d)") % bytes_to_send % ret).str());
    }
    ...
    uint32_t load_fpga(const std::string filestring, bool force) {
        ...
        if (current_usb_speed == 3)
            ...
            throw shd::io_error("load_fpga: get_usb_speed returned invalid USB speed (not 2 or 3).");

        SHD_ASSERT_THROW(transfer_size <= VREQ_MAX_SIZE);
        ...
        if (nread < 0)
            throw shd::io_error((boost::format("load_fpga: unable to complete firmware loopback request (%d: %s)") % nread % libusb_error_name(nread)).str());
        else if (nread != ntoread)
            throw shd::io_error((boost::format("load_fpga: short read on firmware loopback request (expecting: %d, returned: %d)") % ntoread % nread).str());
        ...
        if (!file.good()) {
            throw shd::io_error("load_fpga: cannot open FPGA input file.");
        }

        // Zero the hash, in case we abort programming another image and revert to the previously programmed image
        smini_set_fpga_hash(0);

        memset(out_buff, 0x00, sizeof(out_buff));
        bytes_to_xfer = 1;
        ret = fx3_control_write(B200_VREQ_FPGA_CONFIG, 0, 0, out_buff, bytes_to_xfer, 1000);
        if (ret < 0)
            throw shd::io_error((boost::format("Failed to start FPGA config (%d: %s)") % ret % libusb_error_name(ret)).str());
        else if (ret != bytes_to_xfer)
            throw shd::io_error((boost::format("Short write on start FPGA config (expecting: %d, returned: %d)") % bytes_to_xfer % ret).str());
        ...
        if (load_img_msg) SHD_MSG(status) << "Loading FPGA image: " \
            << filestring << "..." << std::flush;

        bytes_to_xfer = 1;
        ret = fx3_control_write(B200_VREQ_FPGA_START, 0, 0, out_buff, bytes_to_xfer, 1000);
        if (ret < 0)
            throw shd::io_error((boost::format("Failed to start FPGA bitstream (%d: %s)") % ret % libusb_error_name(ret)).str());
        else if (ret != bytes_to_xfer)
            throw shd::io_error((boost::format("Short write on start FPGA bitstream (expecting: %d, returned: %d)") % bytes_to_xfer % ret).str());
        ...
        while (!file.eof()) {
            ...
            if (nwritten < 0)
                throw shd::io_error((boost::format("load_fpga: cannot write bitstream to FX3 (%d: %s)") % nwritten % libusb_error_name(nwritten)).str());
            else if (nwritten != transfer_count)
                throw shd::io_error((boost::format("load_fpga: short write while transferring bitstream to FX3  (expecting: %d, returned: %d)") % transfer_count % nwritten).str());

            if (load_img_msg)
            {
                if (bytes_sent == 0) SHD_MSG(status) << "  0%" << std::flush;
                ...
                {
                    SHD_MSG(status) << "\b\b\b\b" << std::setw(3) << percent_after << "%" << std::flush;
                }
            }
        }
        ...
        if (load_img_msg)
            SHD_MSG(status) << "\b\b\b\b done" << std::endl;
