...
struct rx_vita_core_3000_impl : rx_vita_core_3000
{
    ...
    void issue_stream_command(const shd::stream_cmd_t &stream_cmd)
    {
        if (not _is_setup)
        {
            //SHD_MSG(warning) << "rx vita core 3000 issue stream command - not setup yet!";
            return;
        }
    ...
    void setup(const shd::stream_args_t &)
    {
        _is_setup = true;
    }
