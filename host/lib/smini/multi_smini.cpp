...
const std::string multi_smini::ALL_GAINS = "";
const std::string multi_smini::ALL_LOS = "all";

SHD_INLINE std::string string_vector_to_string(std::vector<std::string> values, std::string delimiter = std::string(" "))
{
    std::string out = "";
...
#define THROW_GAIN_NAME_ERROR(name,chan,dir) throw shd::exception::runtime_error( \
...
/*static void do_tune_freq_results_message(
    ...
    bool requested_freq_success = shd::math::frequencies_are_equal(target_freq, clipped_target_freq);
    bool target_freq_success = shd::math::frequencies_are_equal(clipped_target_freq, actual_freq);
    bool rf_lo_tune_success = shd::math::frequencies_are_equal(target_rf_freq, actual_rf_freq);
    bool dsp_tune_success = shd::math::frequencies_are_equal(target_dsp_freq, actual_dsp_freq);

    if(requested_freq_success and target_freq_success and rf_lo_tune_success
            and dsp_tune_success) {
        SHD_MSG(status) << boost::format(
        ...
        SHD_MSG(warning) << results_string << std::endl;
    }
}*/
...
/***********************************************************************
 * Multi SMINI Implementation
 **********************************************************************/
class multi_smini_impl : public multi_smini{
public:
    multi_smini_impl(const device_addr_t &addr){
        _dev = device::make(addr, device::SMINI);
        _tree = _dev->get_tree();
        _is_device3 = bool(boost::dynamic_pointer_cast<shd::device3>(_dev));
    ...
    device3::sptr get_device3(void) {
        if (not is_device3()) {
            throw shd::type_error("Cannot call get_device3() on a non-generation 3 device.");
        }
        return boost::dynamic_pointer_cast<shd::device3>(_dev);
    }

    dict<std::string, std::string> get_smini_rx_info(size_t chan){
        mboard_chan_pair mcp = rx_chan_to_mcp(chan);
        dict<std::string, std::string> smini_info;
        ...
        smini_info["mboard_id"] = _tree->access<std::string>(mb_root(mcp.mboard) / "name").get();
        smini_info["mboard_name"] = mb_eeprom["name"];
        smini_info["mboard_serial"] = mb_eeprom["serial"];
        smini_info["rx_id"] = db_eeprom.id.to_pp_string();
        smini_info["rx_subdev_name"] = _tree->access<std::string>(rx_rf_fe_root(chan) / "name").get();
        smini_info["rx_subdev_spec"] = _tree->access<subdev_spec_t>(mb_root(mcp.mboard) / "rx_subdev_spec").get().to_string();
        smini_info["rx_serial"] = db_eeprom.serial;
        smini_info["rx_antenna"] =  _tree->access<std::string>(rx_rf_fe_root(chan) / "antenna" / "value").get();

        return smini_info;
    }

    dict<std::string, std::string> get_smini_tx_info(size_t chan){
        ...
        smini_info["mboard_name"] = mb_eeprom["name"];
        smini_info["mboard_serial"] = mb_eeprom["serial"];
        smini_info["tx_id"] = db_eeprom.id.to_pp_string();
        smini_info["tx_subdev_name"] = _tree->access<std::string>(tx_rf_fe_root(chan) / "name").get();
        smini_info["tx_subdev_spec"] = _tree->access<subdev_spec_t>(mb_root(mcp.mboard) / "tx_subdev_spec").get().to_string();
        smini_info["tx_serial"] = db_eeprom.serial;
        smini_info["tx_antenna"] = _tree->access<std::string>(tx_rf_fe_root(chan) / "antenna" / "value").get();
    ...
    void set_master_clock_rate(double rate, size_t mboard){
        if (mboard != ALL_MBOARDS){
            if (_tree->exists(mb_root(mboard) / "auto_tick_rate")
                ...
                SHD_MSG(status) << "Setting master clock rate selection to 'manual'." << std::endl;
            }
    ...
    void set_time_unknown_pps(const time_spec_t &time_spec){
        SHD_MSG(status) << "    1) catch time transition at pps edge" << std::endl;
        ...
        SHD_MSG(status) << "    2) set times next pps (synchronously)" << std::endl;
    ...
    void set_command_time(const time_spec_t &time_spec, size_t mboard){
        if (mboard != ALL_MBOARDS){
            if (not _tree->exists(mb_root(mboard) / "time/cmd")){
                throw shd::not_implemented_error("timed command feature not implemented on this hardware");
            }
    ...
    void set_clock_source_out(const bool enb, const size_t mboard)
    {
        if (mboard != ALL_MBOARDS)
        {
            ...
            {
                throw shd::runtime_error("multi_smini::set_clock_source_out - not supported on this device");
            }
            return;
        }
        for (size_t m = 0; m < get_num_mboards(); m++)
    ...
    void set_time_source_out(const bool enb, const size_t mboard)
    {
        if (mboard != ALL_MBOARDS)
        {
            ...
            {
                throw shd::runtime_error("multi_smini::set_time_source_out - not supported on this device");
            }
            return;
        }
        for (size_t m = 0; m < get_num_mboards(); m++)
    ...
    subdev_spec_t get_rx_subdev_spec(size_t mboard)
    {
        ...
        if (spec.empty())
        {
            ...
            {
                throw shd::index_error(str(boost::format("multi_smini::get_rx_subdev_spec(%u) failed to make default spec - %s") % mboard % e.what()));
            }
            SHD_MSG(status) << "Selecting default RX front end spec: " << spec.to_pp_string() << std::endl;
        }
        return spec;
    }
        ...
        if (_tree->exists(rx_rf_fe_root(chan) / "los")) {
            if (name == ALL_LOS) {
                ...
                if (_tree->exists(rx_rf_fe_root(chan) / "los")) {
                    ...
                    throw shd::runtime_error("Could not find LO stage " + name);
                }
            }
        } else {
            throw shd::runtime_error("This device does not support manual configuration of LOs");
        }
    }
    ...
    double set_rx_lo_freq(double freq, const std::string &name = ALL_LOS, size_t chan = 0){
        if (_tree->exists(rx_rf_fe_root(chan) / "los")) {
            if (name == ALL_LOS) {
                throw shd::runtime_error("LO frequency must be set for each stage individually");
            } else {
                if (_tree->exists(rx_rf_fe_root(chan) / "los")) {
    ...
    double get_rx_lo_freq(const std::string &name = ALL_LOS, size_t chan = 0){
        if (_tree->exists(rx_rf_fe_root(chan) / "los")) {
            if (name == ALL_LOS) {
                throw shd::runtime_error("LO frequency must be retrieved for each stage individually");
            } else {
                if (_tree->exists(rx_rf_fe_root(chan) / "los")) {
                    return _tree->access<double>(rx_rf_fe_root(chan) / "los" / name / "freq" / "value").get();
                } else {
    ...
    freq_range_t get_rx_lo_freq_range(const std::string &name = ALL_LOS, size_t chan = 0){
        if (_tree->exists(rx_rf_fe_root(chan) / "los")) {
            if (name == ALL_LOS) {
                throw shd::runtime_error("LO frequency range must be retrieved for each stage individually");
            } else {
                if (_tree->exists(rx_rf_fe_root(chan) / "los")) {
    ...
    void set_rx_gain(double gain, const std::string &name, size_t chan){
        ...
        if (chan != ALL_CHANS) {
            if (_tree->exists(rx_rf_fe_root(chan) / "gain" / "agc")) {
                ...
                if(agc) {
                    SHD_MSG(warning) << "AGC enabled for this channel. Setting will be ignored." << std::endl;
                }
            }
        } else {
            for (size_t c = 0; c < get_rx_num_channels(); c++){
    ...
    void set_normalized_rx_gain(double gain, size_t chan = 0)
    {
      if (gain > 1.0 || gain < 0.0) {
        throw shd::runtime_error("Normalized gain out of range, must be in [0, 1].");
      }
    ...
    void set_rx_agc(bool enable, size_t chan = 0)
    {
        if (chan != ALL_CHANS){
            if (_tree->exists(rx_rf_fe_root(chan) / "gain" / "agc" / "enable")) {
                ...
                SHD_MSG(warning) << "AGC is not available on this device." << std::endl;
            }
            return;
        }
        for (size_t c = 0; c < get_rx_num_channels(); c++){
    ...
    void set_rx_dc_offset(const bool enb, size_t chan){
        if (chan != ALL_CHANS){
            if (_tree->exists(rx_fe_root(chan) / "dc_offset" / "enable")) {
                ...
                SHD_MSG(warning) << "Setting DC offset compensation is not possible on this device." << std::endl;
            }
            return;
        }
        for (size_t c = 0; c < get_rx_num_channels(); c++){
    ...
    void set_rx_dc_offset(const std::complex<double> &offset, size_t chan){
        if (chan != ALL_CHANS){
            if (_tree->exists(rx_fe_root(chan) / "dc_offset" / "value")) {
                ...
                SHD_MSG(warning) << "Setting DC offset is not possible on this device." << std::endl;
            }
            return;
        }
        for (size_t c = 0; c < get_rx_num_channels(); c++){
    ...
    void set_rx_iq_balance(const bool enb, size_t chan){
        if (chan != ALL_CHANS){
            if (_tree->exists(rx_rf_fe_root(chan) / "iq_balance" / "enable")) {
                ...
                SHD_MSG(warning) << "Setting IQ imbalance compensation is not possible on this device." << std::endl;
            }
            return;
        }
        for (size_t c = 0; c < get_rx_num_channels(); c++){
    ...
    void set_rx_iq_balance(const std::complex<double> &offset, size_t chan){
        if (chan != ALL_CHANS){
            if (_tree->exists(rx_fe_root(chan) / "iq_balance" / "value")) {
                ...
                SHD_MSG(warning) << "Setting IQ balance is not possible on this device." << std::endl;
            }
            return;
        }
        for (size_t c = 0; c < get_rx_num_channels(); c++){
    ...
    filter_info_base::sptr get_filter(const std::string &path)
    {
        ...
        if (it == possible_names.end()) {
            throw shd::runtime_error("Attempting to get non-existing filter: "+path);
        }
    ...
    void set_filter(const std::string &path, filter_info_base::sptr filter)
    {
        ...
        it = find(possible_names.begin(), possible_names.end(), path);
        if (it == possible_names.end()) {
            throw shd::runtime_error("Attempting to set non-existing filter: "+path);
        }
    ...
    subdev_spec_t get_tx_subdev_spec(size_t mboard)
    {
        ...
        if (spec.empty())
        {
            ...
            {
                throw shd::index_error(str(boost::format("multi_smini::get_tx_subdev_spec(%u) failed to make default spec - %s") % mboard % e.what()));
            }
            SHD_MSG(status) << "Selecting default TX front end spec: " << spec.to_pp_string() << std::endl;
        }
        return spec;
    }
    ...
    void write_register(const std::string &path, const uint32_t field, const uint64_t value, const size_t mboard)
    {
        ...
        {
            shd::soft_regmap_accessor_t::sptr accessor =
                _tree->access<shd::soft_regmap_accessor_t::sptr>(mb_root(mboard) / "registers").get();
            shd::soft_register_base& reg = accessor->lookup(path);

            if (not reg.is_writable()) {
                throw shd::runtime_error("multi_smini::write_register - register not writable: " + path);
            }

            switch (reg.get_bitwidth()) {
            case 16:
                if (reg.is_readable())
                    shd::soft_register_base::cast<shd::soft_reg16_rw_t>(reg).write(field, static_cast<uint16_t>(value));
                else
                    shd::soft_register_base::cast<shd::soft_reg16_wo_t>(reg).write(field, static_cast<uint16_t>(value));
            break;

            case 32:
                if (reg.is_readable())
                    shd::soft_register_base::cast<shd::soft_reg32_rw_t>(reg).write(field, static_cast<uint32_t>(value));
                else
                    shd::soft_register_base::cast<shd::soft_reg32_wo_t>(reg).write(field, static_cast<uint32_t>(value));
            break;

            case 64:
                if (reg.is_readable())
                    shd::soft_register_base::cast<shd::soft_reg64_rw_t>(reg).write(field, value);
                else
                    shd::soft_register_base::cast<shd::soft_reg64_wo_t>(reg).write(field, value);
            break;

            default:
                throw shd::assertion_error("multi_smini::write_register - register has invalid bitwidth");
            }

        } else {
            throw shd::not_implemented_error("multi_smini::write_register - register IO not supported for this device");
        }
    }
    ...
    uint64_t read_register(const std::string &path, const uint32_t field, const size_t mboard)
    {
        if (_tree->exists(mb_root(mboard) / "registers"))
        {
            ...
            if (not reg.is_readable()) {
                throw shd::runtime_error("multi_smini::read_register - register not readable: " + path);
            }

            switch (reg.get_bitwidth()) {
            case 16:
                if (reg.is_writable())
                    return static_cast<uint64_t>(shd::soft_register_base::cast<shd::soft_reg16_rw_t>(reg).read(field));
                else
                    return static_cast<uint64_t>(shd::soft_register_base::cast<shd::soft_reg16_ro_t>(reg).read(field));
            break;

            case 32:
                if (reg.is_writable())
                    return static_cast<uint64_t>(shd::soft_register_base::cast<shd::soft_reg32_rw_t>(reg).read(field));
                else
                    return static_cast<uint64_t>(shd::soft_register_base::cast<shd::soft_reg32_ro_t>(reg).read(field));
            break;

            case 64:
                if (reg.is_writable())
                    return shd::soft_register_base::cast<shd::soft_reg64_rw_t>(reg).read(field);
                else
                    return shd::soft_register_base::cast<shd::soft_reg64_ro_t>(reg).read(field);
            break;

            default:
                throw shd::assertion_error("multi_smini::read_register - register has invalid bitwidth: " + path);
            }
        }
        throw shd::not_implemented_error("multi_smini::read_register - register IO not supported for this device");
    }
...
private:
    ...
    bool _is_device3;
    shd::rfnoc::legacy_compat::sptr _legacy_compat;
    ...
    mboard_chan_pair rx_chan_to_mcp(size_t chan){
        ...
        {
            throw shd::index_error(str(boost::format("multi_smini: RX channel %u out of range for configured RX frontends") % chan));
        }
        return mcp;
    }
    ...
    mboard_chan_pair tx_chan_to_mcp(size_t chan){
        ...
        if (mcp.mboard >= get_num_mboards())
        {
            throw shd::index_error(str(boost::format("multi_smini: TX channel %u out of range for configured TX frontends") % chan));
        }
        return mcp;
    }
    ...
    fs_path mb_root(const size_t mboard)
    {
        ...
        {
            throw shd::index_error(str(boost::format("multi_smini::mb_root(%u) - %s") % mboard % e.what()));
        }
    }
    ...
    fs_path rx_dsp_root(const size_t chan)
    {
        ...
        if (_tree->exists(mb_root(mcp.mboard) / "rx_chan_dsp_mapping")) {
            ...
            SHD_ASSERT_THROW(map.size() > mcp.chan);
        ...
        {
            throw shd::index_error(str(boost::format("multi_smini::rx_dsp_root(%u) - mcp(%u) - %s") % chan % mcp.chan % e.what()));
        }
    }
    ...
    fs_path tx_dsp_root(const size_t chan)
    {
        ...
        {
            throw shd::index_error(str(boost::format("multi_smini::tx_dsp_root(%u) - mcp(%u) - %s") % chan % mcp.chan % e.what()));
        }
    }
    ...
    fs_path rx_fe_root(const size_t chan)
    {
        ...
        {
            throw shd::index_error(str(boost::format("multi_smini::rx_fe_root(%u) - mcp(%u) - %s") % chan % mcp.chan % e.what()));
        }
    }
    ...
    fs_path tx_fe_root(const size_t chan)
    {
        ...
        {
            throw shd::index_error(str(boost::format("multi_smini::tx_fe_root(%u) - mcp(%u) - %s") % chan % mcp.chan % e.what()));
        }
    }
    ...
    fs_path rx_rf_fe_root(const size_t chan)
    {
        ...
        {
            throw shd::index_error(str(boost::format("multi_smini::rx_rf_fe_root(%u) - mcp(%u) - %s") % chan % mcp.chan % e.what()));
        }
    }
    ...
    fs_path tx_rf_fe_root(const size_t chan)
    {
        ...
        {
            throw shd::index_error(str(boost::format("multi_smini::tx_rf_fe_root(%u) - mcp(%u) - %s") % chan % mcp.chan % e.what()));
        }
    }
 ...
 * The Make Function
 **********************************************************************/
multi_smini::sptr multi_smini::make(const device_addr_t &dev_addr){
    SHD_LOG << "multi_smini::make with args " << dev_addr.to_pp_string() << std::endl;
    return sptr(new multi_smini_impl(dev_addr));
}
