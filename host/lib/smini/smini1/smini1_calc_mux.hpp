...
#ifndef INCLUDED_SMINI1_CALC_MUX_HPP
#define INCLUDED_SMINI1_CALC_MUX_HPP
...
static uint32_t calc_rx_mux(const std::vector<mapping_pair_t> &mapping){
    ...
    static const shd::dict<std::string, shd::dict<std::string, int> > name_to_conn_to_flag = boost::assign::map_list_of
    ...
    BOOST_FOREACH(const mapping_pair_t &pair, shd::reversed(mapping)){
    ...
    if (num_quads != 0 and num_reals != 0) SHD_MSG(warning) << boost::format(
...
static uint32_t calc_tx_mux(const std::vector<mapping_pair_t> &mapping){
    ...
    static const shd::dict<size_t, shd::dict<std::string, int> > chan_to_conn_to_flag = boost::assign::map_list_of
    ...
    shd::dict<std::string, int> slot_to_chan_count = boost::assign::map_list_of("A", 0)("B", 0);
    BOOST_FOREACH(const mapping_pair_t &pair, mapping){
        ...
        if (slot_to_chan_count[name] > 1) throw shd::value_error(
...
#endif /* INCLUDED_SMINI1_CALC_MUX_HPP */
