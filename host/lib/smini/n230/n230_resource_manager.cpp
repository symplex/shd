...
{
    if (_safe_mode) SHD_MSG(warning) << "Initializing device in safe mode\n";
    SHD_MSG(status) << "Setup basic communication...\n";
    ...
    BOOST_FOREACH(const std::string& addr, ip_addrs) {
        ...
        try {
            iface_id = smini3::smini3_fw_ctrl_iface::get_iface_id(
        ...
        switch (iface_id) {
            ...
            default: {
                if (dual_eth_expected) {
                    throw shd::runtime_error("N230 Initialization Error: Could not detect ethernet port number.");
                } else {
    ...
    if (_eth_conns.size() < 1) {
        throw shd::runtime_error("N230 Initialization Error: No eth interfaces specified.)");
    }

    //Create firmware communication interface
    _fw_ctrl = smini3::smini3_fw_ctrl_iface::make(
    ...
    if (_fw_ctrl.get() == NULL) {
        throw shd::runtime_error("N230 Initialization Error: Could not create n230_ctrl_iface.)");
    }
    _check_fw_compat();

    //Start the device claimer
    _claimer_task = shd::task::make(boost::bind(&n230_resource_manager::_claimer_loop, this));
    ...
    if (core_xport.get() == NULL) {
        throw shd::runtime_error("N230 Initialization Error: Could not create settings transport.)");
    }
    _core_ctrl = radio_ctrl_core_3000::make(
        fpga::CVITA_BIG_ENDIAN, core_xport, core_xport, core_sid.get());
    if (_core_ctrl.get() == NULL) {
        throw shd::runtime_error("N230 Initialization Error: Could not create settings ctrl.)");
    }
    _check_fpga_compat();

    SHD_MSG(status) << boost::format("Version signatures... Firmware:%s FPGA:%s...\n")
    ...
    if (_core_spi_ctrl.get() == NULL) {
        throw shd::runtime_error("N230 Initialization Error: Could not create SPI ctrl.)");
    }

    //Create AD9361 interface
    SHD_MSG(status) << "Initializing CODEC...\n";
    ...
    if (_codec_ctrl.get() == NULL) {
        throw shd::runtime_error("N230 Initialization Error: Could not create Catalina ctrl.)");
    }
    ...
    if (_ref_pll_ctrl.get() == NULL) {
        throw shd::runtime_error("N230 Initialization Error: Could not create ADF4001 ctrl.)");
    }
    ...
    if (_clk_pps_ctrl.get() == NULL) {
        throw shd::runtime_error("N230 Initialization Error: Could not create clock and PPS ctrl.)");
    }
    ...
    if (_frontend_ctrl.get() == NULL) {
        throw shd::runtime_error("N230 Initialization Error: Could not create front-end ctrl.)");
    }
    ...
    if (_core_status_reg.read(fpga::core_status_reg_t::GPSDO_STATUS) != fpga::GPSDO_ST_ABSENT) {
        SHD_MSG(status) << "Detecting GPSDO.... " << std::flush;
        try {
            ...
            _gps_uart = n230_uart::make(gps_uart_xport, shd::htonx(gps_uart_sid.get()));
    ...
    for (size_t i = 0; i < fpga::NUM_RADIOS; i++) {
        ...
        if (!radio_selftest_pass) {
            throw shd::runtime_error("N230 Initialization Error: Data loopback test failed.)");
        }
        ...
        if (!codec_selftest_pass) {
            throw shd::runtime_error("N230 Initialization Error: Codec loopback test failed.)");
        }
    }
    ...
    transport::udp_zero_copy::buff_params& buff_out_params)
{
    ...
    if (xport.get() == NULL) {
        throw shd::runtime_error("N230 Create Data Transport: Could not create data transport.)");
    } else {
...
bool n230_resource_manager::is_device_claimed(shd::smini::smini3::smini3_fw_ctrl_iface::sptr fw_ctrl)
{
    boost::mutex::scoped_lock(_claimer_mutex);
...
void n230_resource_manager::_initialize_radio(size_t instance)
{
    ...
    if (ctrl_xport.get() == NULL) {
        throw shd::runtime_error("N230 Initialization Error: Could not create radio transport.)");
    }
    radio.ctrl = radio_ctrl_core_3000::make(
        fpga::CVITA_BIG_ENDIAN, ctrl_xport, ctrl_xport, ctrl_sid.get());
    if (radio.ctrl.get() == NULL) {
        throw shd::runtime_error("N230 Initialization Error: Could not create radio ctrl.)");
    }
    ...
    if (!reg_selftest_pass) {
        throw shd::runtime_error("N230 Initialization Error: Register loopback test failed.)");
    }
    ...
    if (radio.time.get() == NULL) {
        throw shd::runtime_error("N230 Initialization Error: Could not create time core.)");
    }
    ...
    if (radio.framer.get() == NULL || radio.ddc.get() == NULL) {
        throw shd::runtime_error("N230 Initialization Error: Could not create RX DSP interface.)");
    }
    ...
    if (radio.user_settings.get() == NULL) {
        throw shd::runtime_error("N230 Initialization Error: Could not create user settings bus.)");
    }
}
    ...
    transport::zero_copy_if& xport, const n230_eth_port_t port, const sid_t& sid)
{
    ...
    buff->cast<uint32_t *>()[1] = shd::htonx(sid.get());
    buff->commit(8);
    buff.reset();
...
bool n230_resource_manager::_radio_data_loopback_self_test(wb_iface::sptr iface)
{
   bool test_fail = false;
    ...
    for (size_t i = 0; i < 100; i++) {
        ...
        if (test_fail){
            SHD_MSG(fastpath) << boost::format("mismatch (exp:%x, got:%x and %x)... ") % word32 % rb_tx % rb_rx;
...
std::string n230_resource_manager::_get_fpga_upgrade_msg() {
    std::string img_loader_path =
        (fs::path(shd::get_pkg_path()) / "bin" / "shd_image_loader").string();

    return str(boost::format(
            "\nDownload the appropriate FPGA images for this version of SHD.\n"
            "%s\n\n"
            "Then burn a new image to the on-board flash storage of your\n"
            "SMINI N230 device using the image loader utility. Use this command:\n"
...
void n230_resource_manager::_check_fpga_compat()
{
    ...
    if (signature != 0x0ACE0BA5E || product_id != fpga::RB_N230_PRODUCT_ID)
        throw shd::runtime_error("Signature check failed. Please contact support.");
    ...
    if (is_safe_image && !_safe_mode) {
        throw shd::runtime_error(
            ...
            "To continue using the failsafe image with SHD, create the SHD device with the \"safe_mode\" device arg.\n"
