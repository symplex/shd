...
#include <linux/smini_e.h> //ioctl structures and constants
#include <poll.h> //poll
...
shd::spi_iface::sptr e100_ctrl::make_aux_spi_iface(void){
    return shd::spi_iface::sptr(new aux_spi_iface_impl());
}
...
public:
    i2c_dev_iface(const std::string &node){
        if ((_node_fd = ::open(node.c_str(), O_RDWR)) < 0){
            throw shd::io_error("Failed to open " + node);
        }
    }
    ...
    void write_i2c(uint16_t addr, const byte_vector_t &bytes){
        ...
        //call the ioctl
        SHD_ASSERT_THROW(::ioctl(_node_fd, I2C_RDWR, &data) >= 0);
    }
...
shd::i2c_iface::sptr e100_ctrl::make_dev_i2c_iface(const std::string &node){
    return shd::i2c_iface::sptr(new i2c_dev_iface(node));
}
...
public:
    ...
    void write_uart(const std::string &buf){
        ...
        if (size_t(ret) != buf.size()) SHD_LOG << ret;
    }
...
shd::uart_iface::sptr e100_ctrl::make_gps_uart_iface(const std::string &node){
    return shd::uart_iface::sptr(new uart_dev_iface(node));
}
...
struct e100_simpl_mrb : managed_recv_buffer
{
    ...
    sptr get_new(void)
    {
        ...
        //call the ioctl
        ctrl->ioctl(SMINI_E_READ_CTL32, &data);
...
struct e100_simpl_msb : managed_send_buffer
{
    ...
    void release(void)
    {
        ...
        //call the ioctl
        ctrl->ioctl(SMINI_E_WRITE_CTL32, &data);
    }
...
/***********************************************************************
 * SMINI-E100 control implementation
 **********************************************************************/
...
public:
    ...
    e100_ctrl_impl(const std::string &node){
        SHD_MSG(status) << "Opening device node " << node << "..." << std::endl;
        ...
        int module_compat_num = ::ioctl(_node_fd, SMINI_E_GET_COMPAT_NUMBER, NULL);
        if (module_compat_num != SMINI_E_COMPAT_NUMBER){
            ...
            ) % SMINI_E_COMPAT_NUMBER % module_compat_num));
        }
        ...
        if (_irq_fd < 0) SHD_MSG(error) << "Unable to open GPIO for IRQ\n";
    }
     ...
     ******************************************************************/
    SHD_INLINE bool resp_read(void)
    {
    ...
    SHD_INLINE bool resp_wait(const double timeout)
    {
