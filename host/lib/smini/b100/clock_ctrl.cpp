...
static clock_settings_type get_clock_settings(double rate){
    ...
    for (size_t i = 1; i <= 100; i++){
        ...
        for ( //calculate an r divider that fits into the bounds of the vco
            ...
            SHD_MSG(status) << "SMINI-B100 clock control: " << i << std::endl << cs.to_pp_string() << std::endl;
            return cs;
        }
    }

    throw shd::value_error(str(boost::format(
        "SMINI-B100 clock control: could not calculate settings for clock rate %fMHz"
    ) % (rate/1e6)));
}
...
private:
    ...
    void calibrate_now(void){
        ...
        SHD_MSG(error) << "SMINI-B100 clock control: VCO calibration timeout" << std::endl;
        wait_for_ld:
        ...
        SHD_MSG(error) << "SMINI-B100 clock control: lock detection timeout" << std::endl;
    }
