...
public:
    ...
    SHD_INLINE sptr get_new(
        ...
        const size_t words32 = (shd::wtohx(mem32[0]) & 0xffff); //length in words32 (from VRT header)
        ...
        else if (shd::wtohx(mem32[words32]) == 0) mrb.reset();
...
public:
        ...
        _internal(internal), _fragmentation_size(fragmentation_size)
    {
        ...
        _task = shd::task::make(boost::bind(&usb_zero_copy_wrapper_msb::auto_flush, this));
    }
    ...
    void release(void){
        ...
        const bool eop = (shd::wtohx(vita_header) & (0x1 << 24)) != 0;
    ...
    SHD_INLINE sptr get_new(const double timeout){
        boost::mutex::scoped_lock lock(_mutex);
        _ok_to_auto_flush = false;
