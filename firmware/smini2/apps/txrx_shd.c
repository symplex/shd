...
static void handle_udp_data_packet(
    ...
    case SMINI2_UDP_RX_DSP0_PORT:
        //reset dsp to stop streaming
        sr_rx_ctrl0->clear = 1;
        fw_regs[U2_FW_REG_LOCK_TIME] = 0; //force an unlock
        return;

    case SMINI2_UDP_RX_DSP1_PORT:
        //reset dsp to stop streaming
        sr_rx_ctrl1->clear = 1;
        fw_regs[U2_FW_REG_LOCK_TIME] = 0; //force an unlock
        return;

    case SMINI2_UDP_TX_DSP0_PORT:
    ...
    case SMINI2_UDP_FIFO_CRTL_PORT:
        which = 3;
        break;
    ...
    if (payload_len >= sizeof(smini2_stream_ctrl_t)){

        //parse the destination ip addr and udp port from the payload
        const smini2_stream_ctrl_t *stream_ctrl = (const smini2_stream_ctrl_t *)payload;
...
#define OTW_GPIO_BANK_TO_NUM(bank) \
    (((bank) == SMINI2_DIR_RX)? (GPIO_RX_BANK) : (GPIO_TX_BANK))
...
static void handle_udp_ctrl_packet(
    ...
    const smini2_ctrl_data_t *ctrl_data_in = (smini2_ctrl_data_t *)payload;
    uint32_t ctrl_data_in_id = ctrl_data_in->id;

    //ensure that the protocol versions match
    if (payload_len >= sizeof(uint32_t) && ctrl_data_in->proto_ver != SMINI2_FW_COMPAT_NUM){
        if (ctrl_data_in->proto_ver) printf("!Error in control packet handler: Expected compatibility number %d, but got %d\n",
            SMINI2_FW_COMPAT_NUM, ctrl_data_in->proto_ver
        );
        ctrl_data_in_id = SMINI2_CTRL_ID_WAZZUP_BRO;
    }

    //ensure that this is not a short packet
    if (payload_len < sizeof(smini2_ctrl_data_t)){
        printf("!Error in control packet handler: Expected payload length %d, but got %d\n",
            (int)sizeof(smini2_ctrl_data_t), payload_len
        );
        ctrl_data_in_id = SMINI2_CTRL_ID_HUH_WHAT;
    }

    //setup the output data
    smini2_ctrl_data_t ctrl_data_out;
    ctrl_data_out.proto_ver = SMINI2_FW_COMPAT_NUM;
    ctrl_data_out.id=SMINI2_CTRL_ID_HUH_WHAT;
     ...
     * Addressing
     ******************************************************************/
    case SMINI2_CTRL_ID_WAZZUP_BRO:
        ctrl_data_out.id = SMINI2_CTRL_ID_WAZZUP_DUDE;
     ...
     * SPI
     ******************************************************************/
    case SMINI2_CTRL_ID_TRANSACT_ME_SOME_SPI_BRO:{
            //transact
            uint32_t result = spi_transact(
                ...
                ((ctrl_data_in->data.spi_args.mosi_edge == SMINI2_CLK_EDGE_RISE)? SPI_PUSH_FALL : SPI_PUSH_RISE) |
                ((ctrl_data_in->data.spi_args.miso_edge == SMINI2_CLK_EDGE_RISE)? SPI_LATCH_RISE : SPI_LATCH_FALL)
            );

            //load output
            ctrl_data_out.data.spi_args.data = result;
            ctrl_data_out.id = SMINI2_CTRL_ID_OMG_TRANSACTED_SPI_DUDE;
        }
        break;
     ...
     * I2C
     ******************************************************************/
    case SMINI2_CTRL_ID_DO_AN_I2C_READ_FOR_ME_BRO:{
            ...
            ctrl_data_out.id = SMINI2_CTRL_ID_HERES_THE_I2C_DATA_DUDE;
            ctrl_data_out.data.i2c_args.bytes = num_bytes;
        }
        break;

    case SMINI2_CTRL_ID_WRITE_THESE_I2C_VALUES_BRO:{
            uint8_t num_bytes = ctrl_data_in->data.i2c_args.bytes;
            i2c_write(
                ctrl_data_in->data.i2c_args.addr,
            ...
            ctrl_data_out.id = SMINI2_CTRL_ID_COOL_IM_DONE_I2C_WRITE_DUDE;
            ctrl_data_out.data.i2c_args.bytes = num_bytes;
        }
        break;
     ...
     ******************************************************************/
    case SMINI2_CTRL_ID_GET_THIS_REGISTER_FOR_ME_BRO:
        switch(ctrl_data_in->data.reg_args.action){
            case SMINI2_REG_ACTION_FPGA_PEEK32:
                ctrl_data_out.data.reg_args.data = *((uint32_t *) ctrl_data_in->data.reg_args.addr);
                break;

            case SMINI2_REG_ACTION_FPGA_PEEK16:
                ctrl_data_out.data.reg_args.data = *((uint16_t *) ctrl_data_in->data.reg_args.addr);
                break;

            case SMINI2_REG_ACTION_FPGA_POKE32:
                *((uint32_t *) ctrl_data_in->data.reg_args.addr) = (uint32_t)ctrl_data_in->data.reg_args.data;
                break;

            case SMINI2_REG_ACTION_FPGA_POKE16:
                *((uint16_t *) ctrl_data_in->data.reg_args.addr) = (uint16_t)ctrl_data_in->data.reg_args.data;
                break;

            case SMINI2_REG_ACTION_FW_PEEK32:
                ctrl_data_out.data.reg_args.data = fw_regs[(ctrl_data_in->data.reg_args.addr)];
                break;

            case SMINI2_REG_ACTION_FW_POKE32:
                fw_regs[(ctrl_data_in->data.reg_args.addr)] = ctrl_data_in->data.reg_args.data;
                break;

        }
        ctrl_data_out.id = SMINI2_CTRL_ID_OMG_GOT_REGISTER_SO_BAD_DUDE;
        break;

    /*******************************************************************
     * Echo test
     ******************************************************************/
    case SMINI2_CTRL_ID_HOLLER_AT_ME_BRO:
        ctrl_data_out.data.echo_args.len = payload_len;
        ctrl_data_out.id = SMINI2_CTRL_ID_HOLLER_BACK_DUDE;
        send_udp_pkt(SMINI2_UDP_CTRL_PORT, src, &ctrl_data_out, ctrl_data_in->data.echo_args.len);
        return;

    default:
        ctrl_data_out.id = SMINI2_CTRL_ID_HUH_WHAT;
    }
    send_udp_pkt(SMINI2_UDP_CTRL_PORT, src, &ctrl_data_out, sizeof(ctrl_data_out));
}
...
main(void)
{
  ...
  arp_cache_init();
#ifdef BOOTLOADER
  putstr("\nSMINI N210 UDP bootloader\n");
#else
  putstr("\nTxRx-SHD-ZPU\n");
#endif
  printf("FPGA compatibility number: %d\n", SMINI2_FPGA_COMPAT_NUM);
  printf("Firmware compatibility number: %d\n", SMINI2_FW_COMPAT_NUM);

  //init readback for firmware minor version number
  fw_regs[U2_FW_REG_VER_MINOR] = SMINI2_FW_VER_MINOR;
  ...
  pkt_ctrl_program_inspector(get_ip_addr(), SMINI2_UDP_TX_DSP0_PORT);

  //2) register callbacks for udp ports we service
  init_udp_listeners();
  register_udp_listener(SMINI2_UDP_CTRL_PORT, handle_udp_ctrl_packet);
  register_udp_listener(SMINI2_UDP_RX_DSP0_PORT, handle_udp_data_packet);
  register_udp_listener(SMINI2_UDP_RX_DSP1_PORT, handle_udp_data_packet);
  register_udp_listener(SMINI2_UDP_TX_DSP0_PORT, handle_udp_data_packet);
  register_udp_listener(SMINI2_UDP_FIFO_CRTL_PORT, handle_udp_data_packet);
  
#ifdef SMINI2P
  register_udp_listener(SMINI2_UDP_UPDATE_PORT, handle_udp_fw_update_packet);
#endif

  udp_uart_init(SMINI2_UDP_UART_BASE_PORT); //setup uart messaging
