...
class ad936x_manager_impl : public ad936x_manager
{
  public:
    ...
    void loopback_self_test(
        ...
        SHD_MSG(status) << "Performing CODEC loopback test... " << std::flush;
        ...
        for (size_t i = 0; i < 100; i++)
        {
            ...
            if(test_fail)
            {
                ...
                throw shd::runtime_error("CODEC loopback test failed.");
            }
        }
    ...
    double get_auto_tick_rate(
        ...
        SHD_ASSERT_THROW(num_chans >= 1 and num_chans <= _n_frontends);
        const shd::meta_range_t rate_range = _codec_ctrl->get_clock_rate_range();
        ...
        if (shd::math::fp_compare::fp_compare_delta<double>(lcm_rate, shd::math::FREQ_COMPARISON_DELTA_HZ) >
            shd::math::fp_compare::fp_compare_delta<double>(max_tick_rate, shd::math::FREQ_COMPARISON_DELTA_HZ)) {
        ...
        int32_t multiplier = (1 << int32_t(shd::math::log2(max_tick_rate / lcm_rate)));
        ...
        SHD_ASSERT_THROW(
            shd::math::fp_compare::fp_compare_delta<double>(new_rate, shd::math::FREQ_COMPARISON_DELTA_HZ) >=
            shd::math::fp_compare::fp_compare_delta<double>(min_tick_rate, shd::math::FREQ_COMPARISON_DELTA_HZ)
        );
        SHD_ASSERT_THROW(
            shd::math::fp_compare::fp_compare_delta<double>(new_rate, shd::math::FREQ_COMPARISON_DELTA_HZ) <=
            shd::math::fp_compare::fp_compare_delta<double>(max_tick_rate, shd::math::FREQ_COMPARISON_DELTA_HZ)
    ...
    void populate_frontend_subtree(shd::property_tree::sptr subtree, const std::string &key, shd::direction_t dir)
    {
        ...
        {
            subtree->create<meta_range_t>(shd::fs_path("gains") / name / "range")
                .set(ad9361_ctrl::get_gain_range(key));
            subtree->create<double>(shd::fs_path("gains") / name / "value")
        ...
        BOOST_FOREACH(const std::string &filter_name, _codec_ctrl->get_filter_names(key)) {
            subtree->create<filter_info_base::sptr>(shd::fs_path("filters") / filter_name / "value" )
