...
public:
    ...
    void set_underflow_policy(const std::string &policy){
        ...
        else throw shd::value_error("SMINI TX cannot handle requested underflow policy: " + policy);
    }
    ...
    void setup(const shd::stream_args_t &stream_args){
        ...
        else throw shd::value_error("SMINI TX cannot handle requested wire format: " + stream_args.otw_format);
