...
#ifndef INCLUDED_SHD_PROPERTY_TREE_HPP
#define INCLUDED_SHD_PROPERTY_TREE_HPP
 ...
 * associated with a property in a shd::property_tree
...
public:
     ...
     * \throws shd::assertion_error if called more than once
     */
 ...
 *
 * Notice: we do not declare SHD_API on the whole structure
 * because MSVC will do weird things with std::string and linking.
 */
struct fs_path : std::string{
    SHD_API fs_path(void);
    SHD_API fs_path(const char *);
    SHD_API fs_path(const std::string &);
    SHD_API std::string leaf(void) const;
    SHD_API fs_path branch_path(void) const;
};

SHD_API fs_path operator/(const fs_path &, const fs_path &);
SHD_API fs_path operator/(const fs_path &, size_t);

/*!
 * The property tree provides a file system structure for accessing properties.
 */
class SHD_API property_tree : boost::noncopyable{
public:
...
#include <shd/property_tree.ipp>

#endif /* INCLUDED_SHD_PROPERTY_TREE_HPP */
