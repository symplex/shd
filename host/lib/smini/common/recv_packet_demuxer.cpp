...
static SHD_INLINE uint32_t extract_sid(managed_recv_buffer::sptr &buff){
    //ASSUME that the data is in little endian format
    return shd::wtohx(buff->cast<const uint32_t *>()[1]);
}
...
class recv_packet_demuxer_impl : public shd::smini::recv_packet_demuxer{
public:
    ...
    managed_recv_buffer::sptr get_recv_buff(const size_t index, const double timeout){
        ...
        while (true){
            ...
            {
                SHD_MSG(error) << "Got a data packet with unknown SID " << extract_sid(buff) << std::endl;
