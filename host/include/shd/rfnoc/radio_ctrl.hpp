...
#ifndef INCLUDED_LIBSHD_RFNOC_RADIO_CTRL_HPP
#define INCLUDED_LIBSHD_RFNOC_RADIO_CTRL_HPP
 ...
 */
class SHD_RFNOC_API radio_ctrl :
...
{
public:
    SHD_RFNOC_BLOCK_OBJECT(radio_ctrl)
     ...
     *
     * \throws shd::value_error if \p ant is not a valid value.
     */
     ...
     */
    virtual size_t get_chan_from_dboard_fe(const std::string &fe, const shd::direction_t dir) = 0;

    /*! The inverse function to get_chan_from_dboard_fe()
     */
    virtual std::string get_dboard_fe_from_chan(const size_t chan, const shd::direction_t dir) = 0;

}; /* class radio_ctrl */

}} /* namespace shd::rfnoc */

#endif /* INCLUDED_LIBSHD_RFNOC_RADIO_CTRL_HPP */
