...
static bool b200_image_loader(const image_loader::image_loader_args_t &image_loader_args){
    ...
    if(image_loader_args.fpga_path == ""){
        ...
        if(not B2XX_PRODUCT_ID.has_key(boost::lexical_cast<uint16_t>(product))){
            if(user_specified){
                ...
                throw shd::runtime_error("Could not determine model. You must manually specify an FPGA image filename.");
            }
            else{
                return false;
            }
    ...
    std::cout << boost::format("Unit: SMINI %s (%s)")
...
SHD_STATIC_BLOCK(register_b200_image_loader){
