...
const mboard_eeprom_t& n230_eeprom_manager::read_mb_eeprom()
{
    ...
    uint16_t ver_major = shd::htonx<uint16_t>(map.data_version_major);
    uint16_t ver_minor = shd::htonx<uint16_t>(map.data_version_minor);

    _mb_eeprom["product"] = boost::lexical_cast<std::string>(
        shd::htonx<uint16_t>(map.hw_product));
    _mb_eeprom["revision"] = boost::lexical_cast<std::string>(
        shd::htonx<uint16_t>(map.hw_revision));
    ...
    if (ver_major == 1 and ver_minor == 0) {
        ...
        _mb_eeprom["revision_compat"] = boost::lexical_cast<std::string>(
            shd::htonx<uint16_t>(map.hw_revision_compat));
    }
...
void n230_eeprom_manager::write_mb_eeprom(const mboard_eeprom_t& eeprom)
{
    ...
    uint16_t old_ver_major = shd::htonx<uint16_t>(map.data_version_major);
    uint16_t old_ver_minor = shd::htonx<uint16_t>(map.data_version_minor);
    ...
    map.data_version_major = shd::htonx<uint16_t>(N230_EEPROM_VER_MAJOR);
    map.data_version_minor = shd::htonx<uint16_t>(N230_EEPROM_VER_MINOR);

    if (_mb_eeprom.has_key("product")) {
        map.hw_product = shd::htonx<uint16_t>(
            boost::lexical_cast<uint16_t>(_mb_eeprom["product"]));
    }
    if (_mb_eeprom.has_key("revision")) {
        map.hw_revision = shd::htonx<uint16_t>(
            boost::lexical_cast<uint16_t>(_mb_eeprom["revision"]));
    }
    if (_mb_eeprom.has_key("revision_compat")) {
        map.hw_revision_compat = shd::htonx<uint16_t>(
...
{
    //Load request struct
    _request.flags = shd::htonx<uint32_t>(N230_FLASH_COMM_FLAGS_ACK | command);
    _request.seq = shd::htonx<uint32_t>(_seq_num++);
    ...
    if (nbytes == 0) throw shd::io_error("n230_eeprom_manager::_transact failure");

    //Sanity checks
    const size_t flags = shd::ntohx<uint32_t>(_response.flags);
    SHD_ASSERT_THROW(nbytes == sizeof(_response));
    SHD_ASSERT_THROW(_response.seq == _request.seq);
    SHD_ASSERT_THROW(flags & command);
}
