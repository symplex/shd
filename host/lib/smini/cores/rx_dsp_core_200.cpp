...
public:
    ...
    void set_mux(const std::string &mode, const bool fe_swapped){
        static const shd::dict<std::string, uint32_t> mode_to_mux = boost::assign::map_list_of
            ("IQ", 0)
    ...
    shd::meta_range_t get_host_rates(void){
        meta_range_t range;
    ...
    shd::meta_range_t get_freq_range(void){
        return shd::meta_range_t(-_tick_rate/2, +_tick_rate/2, _tick_rate/std::pow(2.0, 32));
    }
    ...
    void setup(const shd::stream_args_t &stream_args){
        ...
        else throw shd::value_error("SMINI RX cannot handle requested wire format: " + stream_args.otw_format);
