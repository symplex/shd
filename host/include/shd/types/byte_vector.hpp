...
#ifndef INCLUDED_SHD_TYPES_BYTE_VECTOR_HPP
#define INCLUDED_SHD_TYPES_BYTE_VECTOR_HPP
...
namespace shd{
    ...
    template<typename RangeSrc, typename RangeDst> SHD_INLINE
    ...
    SHD_API std::string bytes_to_string(const byte_vector_t &bytes);

    //! Create a byte vector from a string, end at null terminator or max length
    SHD_API byte_vector_t string_to_bytes(const std::string &str, size_t max_length);

} //namespace shd

#endif /* INCLUDED_SHD_TYPES_BYTE_VECTOR_HPP */
