...
static const shd::dict<uint32_t, tvrx2_tda18272_cal_map_t> tvrx2_tda18272_cal_map = map_list_of
...
static const shd::dict<std::string, std::string> tvrx2_sd_name_to_antennas = map_list_of
    ("RX1", "J100")
    ("RX2", "J140")
;

static const shd::dict<std::string, std::string> tvrx2_sd_name_to_conn = map_list_of
    ("RX1",  "Q")
    ("RX2",  "I")
;

static const shd::dict<std::string, uint8_t> tvrx2_sd_name_to_i2c_addr = map_list_of
    ("RX1", 0x63)
    ("RX2", 0x60)
;

static const shd::dict<std::string, uint8_t> tvrx2_sd_name_to_irq_io = map_list_of
    ("RX1", (RX1_IRQ))
    ("RX2", (RX2_IRQ))
;

static const shd::dict<std::string, dboard_iface::aux_dac_t> tvrx2_sd_name_to_dac = map_list_of
    ("RX1", dboard_iface::AUX_DAC_A)
    ("RX2", dboard_iface::AUX_DAC_B)
;

static const shd::dict<std::string, gain_range_t> tvrx2_gain_ranges = map_list_of
...
private:
    ...
    double _if_freq;
    double _bandwidth;
    shd::dict<std::string, double> _gains;
    tda18272hnm_regs_t _tda18272hnm_regs;
    shd::dict<uint32_t, tvrx2_tda18272_rfcal_result_t> _rfcal_results;
    shd::dict<uint32_t, tvrx2_tda18272_rfcal_coeffs_t> _rfcal_coeffs;
...
SHD_STATIC_BLOCK(reg_tvrx2_dboard){
...
tvrx2::tvrx2(ctor_args_t args) : rx_dboard_base(args){
    //FIXME for SMINI1, we can only support one TVRX2 installed
    ...
    if (ref_clock == 64.0e6) {
        ...
        SHD_MSG(warning) << boost::format("ref_clock was 200e6, setting ref_clock divider for 100e6.") << std::endl;
        ...
        this->get_iface()->set_gpio_out(dboard_iface::UNIT_RX, REFCLOCK_DIV6);
        SHD_MSG(warning) << boost::format("Unsupported ref_clock %0.2f, valid options 64e6, 100e6, 200e6") % ref_clock << std::endl;
        _freq_scalar = 1.0;
    }
...
tvrx2::~tvrx2(void){
    ...
    SHD_SAFE_CALL(if (_enabled) set_enabled(false);)
}
...
void tvrx2::send_reg(uint8_t start_reg, uint8_t stop_reg){
    start_reg = uint8_t(shd::clip(int(start_reg), 0x0, 0x43));
    stop_reg = uint8_t(shd::clip(int(stop_reg), 0x0, 0x43));
...
freq_range_t tvrx2::get_tda18272_rfcal_result_freq_range(uint32_t result)
{
    ...
    shd::dict<uint32_t, freq_range_t> result_to_cal_freq_ranges_map = map_list_of
        ( 0, freq_range_t(
...
void tvrx2::tvrx2_tda18272_init_rfcal(void)
{
    ...
    shd::dict<uint32_t, uint8_t> result_to_cal_regs = map_list_of
...
void tvrx2::test_rf_filter_robustness(void){
    typedef shd::dict<std::string, std::string> tvrx2_filter_ratings_t;
    typedef shd::dict<std::string, double> tvrx2_filter_margins_t;
    ...
    shd::dict<std::string, uint8_t> filter_cal_regs = map_list_of
        ("VHFLow_0", 0x38)
        ("VHFLow_1", 0x3a)
        ("VHFHigh_0", 0x3b)
        ("VHFHigh_1", 0x3d)
...
void tvrx2::wait_irq(void){
    ...
    read_reg(0xA, 0xB);
    //SHD_ASSERT_THROW(timeout > 0);
    if(timeout <= 0) SHD_MSG(warning) << boost::format(
...
static tda18272hnm_regs_t::lp_fc_t bandwidth_to_lp_fc_reg(double &bandwidth){
    int reg = shd::clip(boost::math::iround((bandwidth-5.0e6)/1.0e6), 0, 4);
