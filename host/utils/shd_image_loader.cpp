...
void sigint_handler(int){
    ...
    if(num_ctrl_c == 1){
        ...
        std::cout << std::endl << shd::image_loader::get_recovery_instructions(device_type) << std::endl;
        exit(EXIT_FAILURE);
    }
}
...
int SHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    // Help message
    if (vm.count("help")){
        std::cout << "SHD Image Loader" << std::endl
                  << std::endl
    ...
    shd::image_loader::image_loader_args_t image_loader_args;
    ...
    if(not image_loader_args.args.has_key("type")){
        throw shd::runtime_error("You must specify a device type.");
    }
    ...
    if(not shd::image_loader::load(image_loader_args)){
        std::cerr << "No applicable SHD devices found" << std::endl;
        return EXIT_FAILURE;
    }
