 ...
 **********************************************************************/
device_addrs_t smini2_find(const device_addr_t &hint_){
    ...
    if (hints.size() > 1){
        ...
        BOOST_FOREACH(const device_addr_t &hint_i, hints){
            device_addrs_t found_devices_i = smini2_find(hint_i);
    ...
    device_addrs_t smini2_addrs;

    //return an empty list of addresses when type is set to non-smini2
    if (hint.has_key("type") and hint["type"] != "smini2") return smini2_addrs;
    ...
    if (hint.has_key("resource")) return smini2_addrs;
    ...
    if (not hint.has_key("addr")){
        BOOST_FOREACH(const if_addrs_t &if_addrs, get_if_addrs()){
            ...
            device_addrs_t new_smini2_addrs = smini2_find(new_hint);
            smini2_addrs.insert(smini2_addrs.begin(),
                new_smini2_addrs.begin(), new_smini2_addrs.end()
            );
        }
        return smini2_addrs;
    }
    ...
    try{
        udp_transport = udp_simple::make_broadcast(hint["addr"], BOOST_STRINGIZE(SMINI2_UDP_CTRL_PORT));
    }
    catch(const std::exception &e){
        SHD_MSG(error) << boost::format("Cannot open UDP transport on %s\n%s") % hint["addr"] % e.what() << std::endl;
        return smini2_addrs; //dont throw, but return empty address so caller can insert
    }

    //send a hello control packet
    smini2_ctrl_data_t ctrl_data_out = smini2_ctrl_data_t();
    ctrl_data_out.proto_ver = shd::htonx<uint32_t>(SMINI2_FW_COMPAT_NUM);
    ctrl_data_out.id = shd::htonx<uint32_t>(SMINI2_CTRL_ID_WAZZUP_BRO);
    try
    {
    ...
    {
        SHD_MSG(error) << "SMINI2 Network discovery error " << ex.what() << std::endl;
    }
    catch(...)
    {
        SHD_MSG(error) << "SMINI2 Network discovery unknown error " << std::endl;
    }
    ...
    while(true){
        size_t len = udp_transport->recv(asio::buffer(smini2_ctrl_data_in_mem));
        if (len > offsetof(smini2_ctrl_data_t, data) and ntohl(ctrl_data_in->id) == SMINI2_CTRL_ID_WAZZUP_DUDE){

            //make a boost asio ipv4 with the raw addr in host byte order
            device_addr_t new_addr;
            new_addr["type"] = "smini2";
            ...
            udp_simple::sptr ctrl_xport = udp_simple::make_connected(
                new_addr["addr"], BOOST_STRINGIZE(SMINI2_UDP_CTRL_PORT)
            );
            ctrl_xport->send(boost::asio::buffer(&ctrl_data_out, sizeof(ctrl_data_out)));
            size_t len = ctrl_xport->recv(asio::buffer(smini2_ctrl_data_in_mem));
            if (len > offsetof(smini2_ctrl_data_t, data) and ntohl(ctrl_data_in->id) == SMINI2_CTRL_ID_WAZZUP_DUDE){
            ...
            try{
                smini2_iface::sptr iface = smini2_iface::make(ctrl_xport);
            ...
            if (
                ...
                smini2_addrs.push_back(new_addr);
            }
 ...
 * Make
 **********************************************************************/
static device::sptr smini2_make(const device_addr_t &device_addr){
    return device::sptr(new smini2_impl(device_addr));
}

SHD_STATIC_BLOCK(register_smini2_device){
    device::register_device(&smini2_find, &smini2_make, device::SMINI);
}
...
static mtu_result_t determine_mtu(const std::string &addr, const mtu_result_t &user_mtu){
    udp_simple::sptr udp_sock = udp_simple::make_connected(
        addr, BOOST_STRINGIZE(SMINI2_UDP_CTRL_PORT)
    );
    ...
    smini2_ctrl_data_t *ctrl_data = reinterpret_cast<smini2_ctrl_data_t *>(&buffer.front());
    static const double echo_timeout = 0.020; //20 ms

    //test holler - check if its supported in this fw version
    ctrl_data->id = htonl(SMINI2_CTRL_ID_HOLLER_AT_ME_BRO);
    ctrl_data->proto_ver = htonl(SMINI2_FW_COMPAT_NUM);
    ctrl_data->data.echo_args.len = htonl(sizeof(smini2_ctrl_data_t));
    udp_sock->send(boost::asio::buffer(buffer, sizeof(smini2_ctrl_data_t)));
    udp_sock->recv(boost::asio::buffer(buffer), echo_timeout);
    if (ntohl(ctrl_data->id) != SMINI2_CTRL_ID_HOLLER_BACK_DUDE)
        throw shd::not_implemented_error("holler protocol not implemented");

    size_t min_recv_mtu = sizeof(smini2_ctrl_data_t), max_recv_mtu = user_mtu.recv_mtu;
    size_t min_send_mtu = sizeof(smini2_ctrl_data_t), max_send_mtu = user_mtu.send_mtu;
    ...
    while (min_send_mtu < max_send_mtu){
        ...
        size_t len = udp_sock->recv(boost::asio::buffer(buffer), echo_timeout);
        if (len >= sizeof(smini2_ctrl_data_t)) len = ntohl(ctrl_data->data.echo_args.len);
...
static zero_copy_if::sptr make_xport(
    ...
    //Send a small data packet so the smini2 knows the udp source port.
    //This setup must happen before further initialization occurs
    //or the async update packets will cause ICMP destination unreachable.
    static const uint32_t data[2] = {
        shd::htonx(uint32_t(0 /* don't care seq num */)),
        shd::htonx(uint32_t(SMINI2_INVALID_VRT_HEADER))
    };
 ...
 * Structors
 **********************************************************************/
smini2_impl::smini2_impl(const device_addr_t &_device_addr) :
    device_addr(_device_addr)
{
    SHD_MSG(status) << "Opening a SMINI2/N-Series device..." << std::endl;
    ...
    if (not device_addr.has_key("send_buff_size")){
        ...
        device_addr["send_buff_size"] = boost::lexical_cast<std::string>(SMINI2_SRAM_BYTES);
    }
    ...
    try{
        ...
        SHD_MSG(status) << boost::format("Current recv frame size: %d bytes") % mtu.recv_mtu << std::endl;
        SHD_MSG(status) << boost::format("Current send frame size: %d bytes") % mtu.send_mtu << std::endl;
    }
    catch(const shd::not_implemented_error &){
    ...
    _tree->create<std::string>("/name").set("SMINI2 / N-Series Device");
    ...
    for (size_t mbi = 0; mbi < device_args.size(); mbi++){
        ...
        ////////////////////////////////////////////////////////////////
        _mbc[mb].iface = smini2_iface::make(udp_simple::make_connected(
            addr, BOOST_STRINGIZE(SMINI2_UDP_CTRL_PORT)
        ));
        ...
        int expected_fpga_compat_num = std::min(SMINI2_FPGA_COMPAT_NUM, N200_FPGA_COMPAT_NUM);
        switch (_mbc[mb].iface->get_rev())
        {
        case smini2_iface::SMINI2_REV3:
        case smini2_iface::SMINI2_REV4:
            expected_fpga_compat_num = SMINI2_FPGA_COMPAT_NUM;
            break;
        ...
        default:
            ...
            if (fpga_major == SMINI2_FPGA_COMPAT_NUM or fpga_major == N200_FPGA_COMPAT_NUM)
            {
                SHD_MSG(warning)  << "Unable to identify device - assuming SMINI2/N-Series device" << std::endl;
        ...
        ////////////////////////////////////////////////////////////////
        SHD_LOG << "Making transport for RX DSP0..." << std::endl;
        _mbc[mb].rx_dsp_xports.push_back(make_xport(
            addr, BOOST_STRINGIZE(SMINI2_UDP_RX_DSP0_PORT), device_args_i, "recv"
        ));
        SHD_LOG << "Making transport for RX DSP1..." << std::endl;
        _mbc[mb].rx_dsp_xports.push_back(make_xport(
            addr, BOOST_STRINGIZE(SMINI2_UDP_RX_DSP1_PORT), device_args_i, "recv"
        ));
        SHD_LOG << "Making transport for TX DSP0..." << std::endl;
        _mbc[mb].tx_dsp_xport = make_xport(
            addr, BOOST_STRINGIZE(SMINI2_UDP_TX_DSP0_PORT), device_args_i, "send"
        );
        SHD_LOG << "Making transport for Control..." << std::endl;
        _mbc[mb].fifo_ctrl_xport = make_xport(
            addr, BOOST_STRINGIZE(SMINI2_UDP_FIFO_CRTL_PORT), device_addr_t(), ""
        );
        //set the filter on the router to take dsp data from this port
        _mbc[mb].iface->poke32(U2_REG_ROUTER_CTRL_PORTS, (SMINI2_UDP_FIFO_CRTL_PORT << 16) | SMINI2_UDP_TX_DSP0_PORT);

        //create the fifo control interface for high speed register access
        _mbc[mb].fifo_ctrl = smini2_fifo_ctrl::make(_mbc[mb].fifo_ctrl_xport);
        switch(_mbc[mb].iface->get_rev()){
        case smini2_iface::SMINI_N200:
        case smini2_iface::SMINI_N210:
        case smini2_iface::SMINI_N200_R4:
        ...
        _tree->create<double>(mb_path / "link_max_rate").set(SMINI2_LINK_RATE_BPS);
        ...
        _tree->create<mboard_eeprom_t>(mb_path / "eeprom")
            ...
            .add_coerced_subscriber(boost::bind(&smini2_impl::set_mb_eeprom, this, mb, _1));

        ////////////////////////////////////////////////////////////////
        // create clock control objects
        ////////////////////////////////////////////////////////////////
        _mbc[mb].clock = smini2_clock_ctrl::make(_mbc[mb].iface, _mbc[mb].spiface);
        _tree->create<double>(mb_path / "tick_rate")
            .set_publisher(boost::bind(&smini2_clock_ctrl::get_master_clock_rate, _mbc[mb].clock))
            .add_coerced_subscriber(boost::bind(&smini2_impl::update_tick_rate, this, _1));
        ...
        _mbc[mb].codec = smini2_codec_ctrl::make(_mbc[mb].iface, _mbc[mb].spiface);
        switch(_mbc[mb].iface->get_rev()){
        case smini2_iface::SMINI_N200:
        case smini2_iface::SMINI_N210:
        case smini2_iface::SMINI_N200_R4:
        case smini2_iface::SMINI_N210_R4:{
            ...
            _tree->create<double>(rx_codec_path / "gains/digital/value")
                .add_coerced_subscriber(boost::bind(&smini2_codec_ctrl::set_rx_digital_gain, _mbc[mb].codec, _1)).set(0);
            ...
            _tree->create<double>(rx_codec_path / "gains/fine/value")
                .add_coerced_subscriber(boost::bind(&smini2_codec_ctrl::set_rx_digital_fine_gain, _mbc[mb].codec, _1)).set(0);
        }break;
        ...
        if (_mbc[mb].iface->peekfw(U2_FW_REG_HAS_GPSDO) != dont_look_for_gpsdo)
        {
            ...
            try{
                _mbc[mb].gps = gps_ctrl::make(udp_simple::make_uart(udp_simple::make_connected(
                    addr, BOOST_STRINGIZE(SMINI2_UDP_UART_GPS_PORT)
                )));
            }
        ...
        _tree->create<sensor_value_t>(mb_path / "sensors/mimo_locked")
            .set_publisher(boost::bind(&smini2_impl::get_mimo_locked, this, mb));
        _tree->create<sensor_value_t>(mb_path / "sensors/ref_locked")
            .set_publisher(boost::bind(&smini2_impl::get_ref_locked, this, mb));
        ...
        _tree->create<subdev_spec_t>(mb_path / "rx_subdev_spec")
            .add_coerced_subscriber(boost::bind(&smini2_impl::update_rx_subdev_spec, this, mb, _1));
        _tree->create<subdev_spec_t>(mb_path / "tx_subdev_spec")
            .add_coerced_subscriber(boost::bind(&smini2_impl::update_tx_subdev_spec, this, mb, _1));
        ...
        _mbc[mb].rx_dsps.push_back(rx_dsp_core_200::make(
            _mbc[mb].wbiface, U2_REG_SR_ADDR(SR_RX_DSP0), U2_REG_SR_ADDR(SR_RX_CTRL0), SMINI2_RX_SID_BASE + 0, true
        ));
        _mbc[mb].rx_dsps.push_back(rx_dsp_core_200::make(
            _mbc[mb].wbiface, U2_REG_SR_ADDR(SR_RX_DSP1), U2_REG_SR_ADDR(SR_RX_CTRL1), SMINI2_RX_SID_BASE + 1, true
        ));
        for (size_t dspno = 0; dspno < _mbc[mb].rx_dsps.size(); dspno++){
            _mbc[mb].rx_dsps[dspno]->set_link_rate(SMINI2_LINK_RATE_BPS);
            ...
            _tree->create<double>(rx_dsp_path / "rate/value")
                ...
                .add_coerced_subscriber(boost::bind(&smini2_impl::update_rx_samp_rate, this, mb, dspno, _1));
        ...
        _mbc[mb].tx_dsp = tx_dsp_core_200::make(
            _mbc[mb].wbiface, U2_REG_SR_ADDR(SR_TX_DSP), U2_REG_SR_ADDR(SR_TX_CTRL), SMINI2_TX_ASYNC_SID
        );
        _mbc[mb].tx_dsp->set_link_rate(SMINI2_LINK_RATE_BPS);
        _tree->access<double>(mb_path / "tick_rate")
        ...
        _tree->create<double>(mb_path / "tx_dsps/0/rate/value")
            ...
            .add_coerced_subscriber(boost::bind(&smini2_impl::update_tx_samp_rate, this, mb, 0, _1));
        ...
        _mbc[mb].tx_dsp->set_updates(
            ...
            (ups_per_fifo > 0.0)? size_t(SMINI2_SRAM_BYTES/ups_per_fifo/send_frame_size) : 0
        );
        ...
        _tree->create<std::string>(mb_path / "clock_source/value")
            .add_coerced_subscriber(boost::bind(&smini2_impl::update_clock_source, this, mb, _1))
            .set("internal");
        ...
        case smini2_iface::SMINI_N210_R4:
            _tree->create<time_spec_t>(mb_path / "time/cmd")
                .add_coerced_subscriber(boost::bind(&smini2_fifo_ctrl::set_time, _mbc[mb].fifo_ctrl, _1));
        default: break; //otherwise, do not register
        }
        _tree->access<double>(mb_path / "tick_rate")
            .add_coerced_subscriber(boost::bind(&smini2_fifo_ctrl::set_tick_rate, _mbc[mb].fifo_ctrl, _1));
        ...
        rx_db_eeprom.load(*_mbc[mb].iface, SMINI2_I2C_ADDR_RX_DB);
        tx_db_eeprom.load(*_mbc[mb].iface, SMINI2_I2C_ADDR_TX_DB);
        gdb_eeprom.load(*_mbc[mb].iface, SMINI2_I2C_ADDR_TX_DB ^ 5);
        ...
        _tree->create<dboard_eeprom_t>(mb_path / "dboards/A/rx_eeprom")
            .set(rx_db_eeprom)
            .add_coerced_subscriber(boost::bind(&smini2_impl::set_db_eeprom, this, mb, "rx", _1));
        _tree->create<dboard_eeprom_t>(mb_path / "dboards/A/tx_eeprom")
            .set(tx_db_eeprom)
            .add_coerced_subscriber(boost::bind(&smini2_impl::set_db_eeprom, this, mb, "tx", _1));
        _tree->create<dboard_eeprom_t>(mb_path / "dboards/A/gdb_eeprom")
            .set(gdb_eeprom)
            .add_coerced_subscriber(boost::bind(&smini2_impl::set_db_eeprom, this, mb, "gdb", _1));

        //create a new dboard interface and manager
        _mbc[mb].dboard_manager = dboard_manager::make(
            rx_db_eeprom.id, tx_db_eeprom.id, gdb_eeprom.id,
            make_smini2_dboard_iface(_mbc[mb].wbiface, _mbc[mb].iface/*i2c*/, _mbc[mb].spiface, _mbc[mb].clock),
        ...
        BOOST_FOREACH(const std::string &name, _tree->list(db_tx_fe_path)){
            _tree->access<double>(db_tx_fe_path / name / "freq" / "value")
                .add_coerced_subscriber(boost::bind(&smini2_impl::set_tx_fe_corrections, this, mb, _1));
        }
        const fs_path db_rx_fe_path = mb_path / "dboards" / "A" / "rx_frontends";
        BOOST_FOREACH(const std::string &name, _tree->list(db_rx_fe_path)){
            _tree->access<double>(db_rx_fe_path / name / "freq" / "value")
                .add_coerced_subscriber(boost::bind(&smini2_impl::set_rx_fe_corrections, this, mb, _1));
        }
    }
    ...
    BOOST_FOREACH(const std::string &mb, _mbc.keys()){
        ...
        if (_mbc[mb].gps and _mbc[mb].gps->gps_detected()){
            ...
            SHD_MSG(status) << "Setting references to the internal GPSDO" << std::endl;
...
smini2_impl::~smini2_impl(void){SHD_SAFE_CALL(
    BOOST_FOREACH(const std::string &mb, _mbc.keys()){
        _mbc[mb].tx_dsp->set_updates(0, 0);
    }
)}

void smini2_impl::set_mb_eeprom(const std::string &mb, const shd::smini::mboard_eeprom_t &mb_eeprom){
    mb_eeprom.commit(*(_mbc[mb].iface), SMINI2_EEPROM_MAP_KEY);
}

void smini2_impl::set_db_eeprom(const std::string &mb, const std::string &type, const shd::smini::dboard_eeprom_t &db_eeprom){
    if (type == "rx") db_eeprom.store(*_mbc[mb].iface, SMINI2_I2C_ADDR_RX_DB);
    if (type == "tx") db_eeprom.store(*_mbc[mb].iface, SMINI2_I2C_ADDR_TX_DB);
    if (type == "gdb") db_eeprom.store(*_mbc[mb].iface, SMINI2_I2C_ADDR_TX_DB ^ 5);
}

sensor_value_t smini2_impl::get_mimo_locked(const std::string &mb){
    const bool lock = (_mbc[mb].wbiface->peek32(U2_REG_IRQ_RB) & (1<<10)) != 0;
    return sensor_value_t("MIMO", lock, "locked", "unlocked");
}

sensor_value_t smini2_impl::get_ref_locked(const std::string &mb){
    const bool lock = (_mbc[mb].wbiface->peek32(U2_REG_IRQ_RB) & (1<<11)) != 0;
    return sensor_value_t("Ref", lock, "locked", "unlocked");
}

void smini2_impl::set_rx_fe_corrections(const std::string &mb, const double lo_freq){
    if(not _ignore_cal_file){
        apply_rx_fe_corrections(this->get_tree()->subtree("/mboards/" + mb), "A", lo_freq);
    }
}

void smini2_impl::set_tx_fe_corrections(const std::string &mb, const double lo_freq){
    if(not _ignore_cal_file){
...
void smini2_impl::update_clock_source(const std::string &mb, const std::string &source){
    ...
    case smini2_iface::SMINI_N210_R4:
        ...
        else throw shd::value_error("unhandled clock configuration reference source: " + source);
        _mbc[mb].clock->enable_external_ref(true); //SMINI2P has an internal 10MHz TCXO
        break;
    ...
    case smini2_iface::SMINI_NXXX: break;
    }
    ...
    if (source != "mimo"){
        ...
        case smini2_iface::SMINI_N200_R4:
        case smini2_iface::SMINI_N210_R4:
            _mbc[mb].clock->set_mimo_clock_delay(mimo_clock_delay_smini_n2xx);
            break;

        case smini2_iface::SMINI2_REV4:
            _mbc[mb].clock->set_mimo_clock_delay(mimo_clock_delay_smini2_rev4);
            break;
