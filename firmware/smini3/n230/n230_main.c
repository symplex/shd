...
#ifndef SHD_VERSION_HASH
#define SHD_VERSION_HASH 0xFFFFFFFF
#endif
...
int main(void)
{
    ...
    g_host_shared_mem.data.fw_version_hash  = SHD_VERSION_HASH;
    ...
    while(true)
    {
        ...
        if (cron_job_run_due(PER_SECOND_CRON_JOBID)) {
            ...
            if (heart_beat % 10 == 0) {
                SHD_FW_TRACE_FSTR(INFO, "0.1Hz Heartbeat (%u)", heart_beat);
            }
            heart_beat++;
        }
