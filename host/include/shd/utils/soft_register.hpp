...
#ifndef INCLUDED_SHD_UTILS_SOFT_REGISTER_HPP
#define INCLUDED_SHD_UTILS_SOFT_REGISTER_HPP
 ...
 *   registers that can be accessed using the SHD register API.
 */
...
#define SHD_DEFINE_SOFT_REG_FIELD(name, width, shift) \
    static const shd::soft_reg_field_t name = (((shift & 0xFF) << 8) | (width & 0xFF))
...
SHD_INLINE bool likely(bool expr)
{
#ifdef __GNUC__
...
/** \brief hint for the branch prediction */
SHD_INLINE bool unlikely(bool expr)
{
#ifdef __GNUC__
...
namespace soft_reg_field {
    SHD_INLINE size_t width(const soft_reg_field_t field) {
        return (field & 0xFF);
    }

    SHD_INLINE size_t shift(const soft_reg_field_t field) {
        return ((field >> 8) & 0xFF);
    }

    template<typename data_t>
    SHD_INLINE data_t mask(const soft_reg_field_t field) {
...
public:
    ...
    SHD_INLINE static soft_reg_t& cast(soft_register_base& reg) {
        soft_reg_t* ptr = dynamic_cast<soft_reg_t*>(&reg);
        if (ptr) {
            return *ptr;
        } else {
            throw shd::type_error("failed to cast register to specified type");
        }
    }
};
...
class SHD_API soft_register_t : public soft_register_base {
public:
    ...
    SHD_DEFINE_SOFT_REG_FIELD(REGISTER, sizeof(reg_data_t)*8, 0);  //[WIDTH-1:0]
     ...
     */
    SHD_INLINE void initialize(wb_iface& iface, bool sync = false)
    {
        _iface = &iface;
     ...
     */
    SHD_INLINE void set(const soft_reg_field_t field, const reg_data_t value)
    {
     ...
     */
    SHD_INLINE reg_data_t get(const soft_reg_field_t field)
    {
        ...
        if (writable && _iface) {
            ...
            if (_flush_mode == ALWAYS_FLUSH || _soft_copy.is_dirty()) {
                if (get_bitwidth() <= 16) {
                    ...
                    throw shd::not_implemented_error("soft_register only supports up to 64 bits.");
                }
                _soft_copy.mark_clean();
            }
        } else {
            throw shd::not_implemented_error("soft_register is not writable or uninitialized.");
        }
    }
    ...
    SHD_INLINE void refresh()
    {
        if (readable && _iface) {
            ...
            throw shd::not_implemented_error("soft_register is not readable or uninitialized.");
        }
    }
     ...
     */
    SHD_INLINE void write(const soft_reg_field_t field, const reg_data_t value)
    {
        set(field, value);
        flush();
    }
     ...
     */
    SHD_INLINE reg_data_t read(const soft_reg_field_t field)
    {
        refresh();
        return get(field);
    }
     ...
     */
    SHD_INLINE size_t get_bitwidth()
    {
 ...
 */
template<typename reg_data_t, bool readable, bool writable>
class SHD_API soft_register_sync_t : public soft_register_t<reg_data_t, readable, writable> {
public:
 ...
 *
  ...
  public:
    ...
    SHD_DEFINE_SOFT_REG_FIELD(FIELD0,  1,  0);  //[0]
    SHD_DEFINE_SOFT_REG_FIELD(FIELD1, 15,  1);  //[15:1]
    SHD_DEFINE_SOFT_REG_FIELD(FIELD2, 16, 16);  //[31:16]
...
class SHD_API soft_regmap_accessor_t {
public:
 ...
 * to do read-modify-write operations.
 */
class SHD_API soft_regmap_t : public soft_regmap_accessor_t, public boost::noncopyable {
public:
     ...
     */
    virtual SHD_INLINE const std::string& get_name() const { return _name; }
    ...
    virtual soft_register_base& lookup(const std::string& name) const {
        ...
        if (iter != _regmap.end()) {
            ...
            throw shd::runtime_error("register not found in map: " + name);
        }
    }
...
protected:
     ...
     */
    SHD_INLINE void add_to_map(soft_register_base& reg, const std::string& name, const visibility_t visible = PRIVATE) {
        boost::lock_guard<boost::mutex> lock(_mutex);
        if (visible == PUBLIC) {
            //Only add to the map if this register is publicly visible
            if (not _regmap.insert(regmap_t::value_type(name, &reg)).second) {
                throw shd::assertion_error("cannot add two registers with the same name to regmap: " + name);
            }
        }
 ...
 */
class SHD_API soft_regmap_db_t : public soft_regmap_accessor_t, public boost::noncopyable {
public:
    ...
    void add(soft_regmap_db_t& db) {
        boost::lock_guard<boost::mutex> lock(_mutex);
        if (&db == this) {
            throw shd::assertion_error("cannot add regmap db to itself" + _name);
        } else {
    ...
    soft_register_base& lookup(const std::string& path) const
    {
        ...
        if ((tokens.size() > 2 && tokens.front() == _name) ||   //If this is a nested DB
            ...
            if (tokens.size() == 2) {                   //2 tokens => regmap/register path
                ...
                throw shd::runtime_error("could not find register map: " + path);
        ...
        throw shd::runtime_error("could not find register: " + path);
    }
...
#endif /* INCLUDED_SHD_UTILS_SOFT_REGISTER_HPP */
