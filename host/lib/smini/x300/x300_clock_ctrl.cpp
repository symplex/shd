...
public:
    ...
    x300_clock_ctrl_impl(shd::spi_iface::sptr spiface,
    ...
    void set_dboard_rate(const x300_clock_which_t which, double rate) {
        ...
        if (not math::frequencies_are_equal(_vco_freq / div, rate))
            throw shd::value_error("invalid dboard rate requested");
        ...
        throw shd::not_implemented_error("x3xx set dboard clock rate does not support changing the clock rate");
    ...
    double set_clock_delay(const x300_clock_which_t which, const double delay_ns, const bool resync = true) {
        ...
        if (ddly_value > DDLY_MAX_TAPS) {
            throw shd::value_error("set_clock_delay: Requested delay is out of range.");
        }
        ...
        case X300_CLOCK_WHICH_ADC1:
            ...
            break;
        default:
            throw shd::value_error("set_clock_delay: Requested source is invalid.");
        }
    ...
    double get_clock_delay(const x300_clock_which_t which) {
        ...
        default:
            throw shd::value_error("get_clock_delay: Requested source is invalid.");
        }
    }
...
private:
    ...
    void init() {
        ...
        enum opmode_t { INVALID,
                        ...
                        m10M_120M_ZDEL };       // NI SMINI 120 MHz Clocking
        ...
        SHD_ASSERT_THROW(clocking_mode != INVALID);
...
x300_clock_ctrl::sptr x300_clock_ctrl::make(shd::spi_iface::sptr spiface,
        const size_t slaveno,
        const size_t hw_rev,
        const double master_clock_rate,
        const double dboard_clock_rate,
