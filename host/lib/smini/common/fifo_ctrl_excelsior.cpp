...
public:
    ...
    void handle_msg1(void){
        ...
        try{
            ...
            SHD_MSG(error) << "FIFO ctrl bad VITA packet: " << ex.what() << std::endl;
        }
        if (packet_info.has_sid and packet_info.sid == _config.ctrl_sid_base){
            ctrl_result_t res = ctrl_result_t();
            res.msg[0] = shd::wtohx(pkt[packet_info.num_header_words32+0]);
            res.msg[1] = shd::wtohx(pkt[packet_info.num_header_words32+1]);
            ...
            load_metadata_from_buff(shd::wtohx<uint32_t>, metadata, packet_info, pkt, _tick_rate, packet_info.sid - _config.async_sid_base);
            _async_fifo.push_with_pop_on_full(metadata);
            standard_async_msg_prints(metadata);
        }
        else{
            SHD_MSG(error) << "FIFO ctrl got unknown SID: " << packet_info.sid << std::endl;
        }
    }
    ...
    void poke16(const wb_addr_type, const uint16_t){
        throw shd::not_implemented_error("poke16 not implemented in fifo ctrl module");
    }

    uint16_t peek16(const wb_addr_type){
        throw shd::not_implemented_error("peek16 not implemented in fifo ctrl module");
    }
...
private:
     ...
     ******************************************************************/
    SHD_INLINE void send_pkt(wb_addr_type addr, uint32_t data, int cmd){
        ...
        pkt[packet_info.num_header_words32+0] = shd::htowx(ctrl_word);
        pkt[packet_info.num_header_words32+1] = shd::htowx(data);

        //send the buffer over the interface
        buff->commit(sizeof(uint32_t)*(packet_info.num_packet_words32));
    }

    SHD_INLINE bool wraparound_lt16(const int16_t i0, const int16_t i1){
        if (((i0 ^ i1) & 0x8000) == 0) //same sign bits
            return uint16_t(i0) < uint16_t(i1);
        return int16_t(i1 - i0) > 0;
    }

    SHD_INLINE uint32_t wait_for_ack(const uint16_t seq_to_ack){

        while (wraparound_lt16(_seq_ack, seq_to_ack)){
            ctrl_result_t res = ctrl_result_t();
            if (not _ctrl_fifo.pop_with_timed_wait(res, _timeout)){
                throw shd::runtime_error("fifo ctrl timed out looking for acks");
            }
