...
//  Throws a shd::runtime_error if the USB PID and the product ID stored
...
b200_product_t get_b200_product(const usb_device_handle::sptr& handle, const mboard_eeprom_t &mb_eeprom)
{
    ...
    if (mb_eeprom["product"].empty()) {
        throw shd::runtime_error("B200: Missing product ID on EEPROM.");
    }
...
std::vector<usb_device_handle::sptr> get_b200_device_handles(const device_addr_t &hint)
{
    ...
    if(hint.has_key("vid") && hint.has_key("pid") && hint.has_key("type") && hint["type"] == "b200") {
        vid_pid_pair_list.push_back(usb_device_handle::vid_pid_pair_t(shd::cast::hexstr_cast<uint16_t>(hint.get("vid")),
                                                                      shd::cast::hexstr_cast<uint16_t>(hint.get("pid"))));
    } else {
...
static device_addrs_t b200_find(const device_addr_t &hint)
{
    ...
    BOOST_FOREACH(usb_device_handle::sptr handle, get_b200_device_handles(hint)) {
        ...
        try{
            ...
            b200_fw_image = shd::find_image_path(b200_fw_image, STR(SHD_IMAGES_DIR)); // FIXME
        }
        catch(shd::exception &e){
            SHD_MSG(warning) << e.what();
            return b200_addrs;
        }
        SHD_LOG << "the firmware image: " << b200_fw_image << std::endl;
...
static device::sptr b200_make(const device_addr_t &device_addr)
{
    ...
    catch (const shd::usb_error &) {
        SHD_MSG(status) << "Detected bad USB state; resetting." << std::endl;
...
SHD_STATIC_BLOCK(register_b200_device)
{
    device::register_device(&b200_find, &b200_make, device::SMINI);
}

/***********************************************************************
 * Structors
 **********************************************************************/
b200_impl::b200_impl(const shd::device_addr_t& device_addr, usb_device_handle::sptr &handle) :
    ...
    _tick_rate(0.0) // Forces a clock initialization at startup
{
    ...
    {
        vid = shd::cast::hexstr_cast<uint16_t>(device_addr.get("vid"));
        specified_vid = true;
    }

    if (device_addr.has_key("pid"))
    {
        pid = shd::cast::hexstr_cast<uint16_t>(device_addr.get("pid"));
        specified_pid = true;
    }
    ...
    BOOST_FOREACH(usb_device_handle::sptr dev_handle, device_list) {
        ...
        } catch (const shd::exception&) { continue; }
    }
    ...
    SHD_MSG(status) << "Detected Device: " << B2XX_STR_NAMES[_product] << std::endl;
    ...
    if(status != 0) {
        throw shd::runtime_error(str(boost::format("fx3 is in state %1%") % status));
    }
    ...
    SHD_MSG(status) << "Operating over USB " << (int) usb_speed << "." << std::endl;
    ...
    // This may throw a shd::usb_error, which will be caught by b200_make().
    ...
    _async_task = shd::msg_task::make(boost::bind(&b200_impl::handle_async_task, this, _ctrl_transport, _async_task_data));
    ...
    ////////////////////////////////////////////////////////////////////
    SHD_MSG(status) << "Initialize CODEC control..." << std::endl;
    ...
    // setup radio control
    ////////////////////////////////////////////////////////////////////
    SHD_MSG(status) << "Initialize Radio control..." << std::endl;
    const size_t num_radio_chains = ((_local_ctrl->peek32(RB32_CORE_STATUS) >> 8) & 0xff);
    SHD_ASSERT_THROW(num_radio_chains > 0);
    SHD_ASSERT_THROW(num_radio_chains <= 2);
    ...
    if (not device_addr.has_key("master_clock_rate")) {
        SHD_MSG(status) << "Setting master clock rate selection to 'automatic'." << std::endl;
    }
...
void b200_impl::setup_radio(const size_t dspno)
{
    ...
    perif.ddc->set_mux(smini::fe_connection_t(dspno == 1 ? "IbQb" : "IQ"));
...
void b200_impl::register_loopback_self_test(wb_iface::sptr iface)
{
    ...
    SHD_MSG(status) << "Performing register loopback test... " << std::flush;
    ...
    SHD_MSG(status) << ((test_fail)? "fail" : "pass") << std::endl;
}
...
void b200_impl::enforce_tick_rate_limits(size_t chan_count, double tick_rate, const std::string &direction /*= ""*/)
{
    ...
    {
        throw shd::value_error(boost::str(
...
{
    SHD_MSG(status) << (boost::format("Asking for clock rate %.6f MHz... ") % (new_tick_rate/1e6)) << std::flush;
    ...
    if (std::abs(new_tick_rate - _tick_rate) < 1.0) {
        SHD_MSG(status) << "OK" << std::endl;
        return _tick_rate;
    }

    _tick_rate = _codec_ctrl->set_clock_rate(new_tick_rate);
    SHD_MSG(status) << std::endl << (boost::format("Actually got clock rate %.6f MHz.") % (_tick_rate/1e6)) << std::endl;
...
void b200_impl::check_fpga_compat(void)
{
    ...
    if (signature != 0xACE0BA5E) throw shd::runtime_error(
    ...
    if (compat_major != expected)
    {
        throw shd::runtime_error(str(boost::format(
            ...
            "%s"
        ) % int(expected) % compat_major % print_utility_error("shd_images_downloader.py")));
    }
    _tree->create<std::string>("/mboards/0/fpga_version").set(str(boost::format("%u.%u")
                % compat_major % compat_minor));
}

void b200_impl::set_mb_eeprom(const shd::smini::mboard_eeprom_t &mb_eeprom)
{
...
void b200_impl::update_clock_source(const std::string &source)
{
    ...
    if (_product == B200MINI or _product == B205MINI)
    {
        ...
        {
            throw shd::value_error("external reference cannot be both a clock source and a time source");
        }
    ...
    else if (source == "gpsdo")
    {
        if (not _gps or not _gps->gps_detected()) {
            throw shd::key_error("update_clock_source: gpsdo selected, but no gpsdo detected!");
        }
        if (_gpio_state.ref_sel != 1)
        {
            _gpio_state.ref_sel = 1;
...
void b200_impl::update_time_source(const std::string &source)
{
    ...
    {
        throw shd::value_error("external reference cannot be both a time source and a clock source");
    }
...
void b200_impl::set_time(const shd::time_spec_t& t)
{
    BOOST_FOREACH(radio_perifs_t &perif, _radio_perifs)
...
{
    if (ant != "TX/RX" and ant != "RX2") throw shd::value_error("b200: unknown RX antenna option: " + ant);
...
void b200_impl::update_enables(void)
{
    ...
    if ((num_rx + num_tx) == 3)
    {
        throw shd::runtime_error("b200: 2 RX 1 TX and 1 RX 2 TX configurations not possible");
    }
