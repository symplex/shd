...
#ifndef INCLUDED_SHD_SMINI_MULTI_SMINI_HPP
#define INCLUDED_SHD_SMINI_MULTI_SMINI_HPP

//define API capabilities for compile time detection of new features
#define SHD_SMINI_MULTI_SMINI_REF_SOURCES_API
#define SHD_SMINI_MULTI_SMINI_GET_RATES_API
#define SHD_SMINI_MULTI_SMINI_FRONTEND_CAL_API
#define SHD_SMINI_MULTI_SMINI_FRONTEND_IQ_AUTO_API
#define SHD_SMINI_MULTI_SMINI_COMMAND_TIME_API
#define SHD_SMINI_MULTI_SMINI_BW_RANGE_API
#define SHD_SMINI_MULTI_SMINI_USER_REGS_API
#define SHD_SMINI_MULTI_SMINI_GET_SMINI_INFO_API
#define SHD_SMINI_MULTI_SMINI_NORMALIZED_GAIN
#define SHD_SMINI_MULTI_SMINI_GPIO_API
#define SHD_SMINI_MULTI_SMINI_REGISTER_API
#define SHD_SMINI_MULTI_SMINI_FILTER_API
#define SHD_SMINI_MULTI_SMINI_LO_CONFIG_API
 ...
 * This class can be used to interface with a single SMINI with
 * one or more channels, or multiple SMINIs in a homogeneous setup.
 ...
 * <pre>
 *
 * //create a multi_smini with two boards in the configuration
 * device_addr_t dev_addr;
 * dev_addr["addr0"] = "192.168.10.2"
 * dev_addr["addr1"] = "192.168.10.3";
 * multi_smini::sptr dev = multi_smini::make(dev_addr);
 *
 ...
 * dev->set_tx_subdev_spec("A:AB", multi_smini::ALL_MBOARDS);
 *
 * //now that all the channels are mapped, continue with configuration...
 *
 * </pre>
 */
class SHD_API multi_smini : boost::noncopyable{
public:
    typedef boost::shared_ptr<multi_smini> sptr;

    virtual ~multi_smini(void) = 0;
    ...
    /*!
     * Make a new multi smini from the device address.
     * \param dev_addr the device address
     * \return a new single smini object
     * \throws shd::key_error no device found
     * \throws shd::index_error fewer devices found than expected
     */
     ...
     * \return the device object within this SMINI
     */
    virtual device::sptr get_device(void) = 0;

    //! Convenience method to get a RX streamer. See also shd::device::get_rx_stream().
    virtual rx_streamer::sptr get_rx_stream(const stream_args_t &args) = 0;

    //! Convenience method to get a TX streamer. See also shd::device::get_rx_stream().
    virtual tx_streamer::sptr get_tx_stream(const stream_args_t &args) = 0;

    /*!
     * Returns identifying information about this SMINI's configuration.
     * Returns motherboard ID, name, and serial.
     * Returns daughterboard RX ID, subdev name and spec, serial, and antenna.
     * \param chan channel index 0 to N-1
     * \return RX info
     */
    virtual dict<std::string, std::string> get_smini_rx_info(size_t chan = 0) = 0;
     ...
     * \return TX info
     */
     virtual dict<std::string, std::string> get_smini_tx_info(size_t chan = 0) = 0;
    ...
    /*!
     * Get a printable summary for this SMINI configuration.
     * \return a printable string
     */
    virtual std::string get_pp_string(void) = 0;

    /*!
     * Get canonical name for this SMINI motherboard.
    ...
    /*!
     * Get the current time in the smini time registers.
     * \param mboard which motherboard to query
     * \return a timespec representing current smini time
     */
    ...
    /*!
     * Sets the time registers on the smini immediately.
     *
     ...
     *
     * \param time_spec the time to latch into the smini device
     * \param mboard the motherboard index 0 to M-1
     */
    virtual void set_time_now(const time_spec_t &time_spec, size_t mboard = ALL_MBOARDS) = 0;

    /*!
     * Set the time registers on the smini at the next pps tick.
     ...
     */
    virtual void set_command_time(const shd::time_spec_t &time_spec, size_t mboard = ALL_MBOARDS) = 0;
    ...
    /*!
     * Issue a stream command to the smini device.
     * This tells the smini to send samples into the host.
    ...
    /*!
     * Set the clock configuration for the smini device.
     * DEPRECATED in favor of set time and clock source calls.
     * This tells the smini how to get a 10MHz reference and PPS clock.
    ...
    /*!
     * Set the time source for the smini device.
    ...
    /*!
     * Set the clock source for the smini device.
    ...
    /*!
     * Get the number of SMINI motherboards in this configuration.
     */
     ...
     * \param mboard the motherboard index 0 to M-1
     */
    virtual void set_rx_subdev_spec(const shd::smini::subdev_spec_t &spec, size_t mboard = ALL_MBOARDS) = 0;
     ...
     */
    virtual shd::smini::subdev_spec_t get_rx_subdev_spec(size_t mboard = 0) = 0;

    /*!
     * Get the number of RX channels in this configuration.
     * This is the number of SMINIs times the number of RX channels per board,
     * where the number of RX channels per board is homogeneous among all SMINIs.
     */
     ...
     * For sminis that support selectable LOs, this function
     ...
     * For sminis that support exportable LOs, this function
     ...
     * \param chan the channel index 0 to N-1
     * \throws A shd::runtime_error if the gain value is outside [0, 1].
     */
     ...
     * \param mboard the motherboard index 0 to M-1
     */
    virtual void set_tx_subdev_spec(const shd::smini::subdev_spec_t &spec, size_t mboard = ALL_MBOARDS) = 0;
     ...
     * \return the frontend specification in use
     */
    virtual shd::smini::subdev_spec_t get_tx_subdev_spec(size_t mboard = 0) = 0;

    /*!
     * Get the number of TX channels in this configuration.
     * This is the number of SMINIs times the number of TX channels per board,
     * where the number of TX channels per board is homogeneous among all SMINIs.
     */
    ...
    /*!
     * Enumerate the full paths of all low-level SMINI registers accessible to read/write
     * \param mboard the motherboard index 0 to M-1
    ...
    /*!
     * Write a low-level register field for a register in the SMINI hardware
     * \param path the full path to the register
    ...
    /*!
     * Read a low-level register field from a register in the SMINI hardware
     * \param path the full path to the register
...
#endif /* INCLUDED_SHD_SMINI_MULTI_SMINI_HPP */
