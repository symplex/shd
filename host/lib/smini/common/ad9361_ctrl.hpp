...
public:
    ...
    virtual void set_timed_spi(shd::spi_iface::sptr spi_iface, uint32_t slave_num) = 0;
    virtual void set_safe_spi(shd::spi_iface::sptr spi_iface, uint32_t slave_num) = 0;
    ...
    static shd::meta_range_t get_gain_range(const std::string &which)
    {
        if(which[0] == 'R') {
            return shd::meta_range_t(0.0, 76.0, 1.0);
        } else {
            return shd::meta_range_t(0.0, 89.75, 0.25);
        }
    }

    //! get the freq range for the frontend which
    static shd::meta_range_t get_rf_freq_range(void)
    {
        return shd::meta_range_t(50e6, 6e9);
    }

    //! get the filter range for the frontend which
    static shd::meta_range_t get_bw_filter_range(const std::string &/*which*/)
    {
        return shd::meta_range_t(200e3, 56e6);
    }

    //! get the clock rate range for the frontend
    static shd::meta_range_t get_clock_rate_range(void)
    {
    ...
    void set_dc_offset(const std::string &, const std::complex<double>)
    {
        ...
        throw shd::runtime_error("ad9361_ctrl::set_dc_offset this feature is not supported on this device.");
    }
    ...
    {
        //This feature should not be used according to Analog Devices
        throw shd::runtime_error("ad9361_ctrl::set_iq_balance this feature is not supported on this device.");
    }
