...
const twinrx_gain_config_t& twinrx_gain_table::find_by_index(size_t index) const {
    if (index >= get_num_entries()) throw shd::value_error("invalid gain table index");
    return _tbl.at(index);
}

shd::gain_range_t twinrx_gain_table::get_gain_range() const {
    ...
    return shd::gain_range_t(min, max, 1.0);
}
