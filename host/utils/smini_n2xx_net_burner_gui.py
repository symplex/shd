...
import smini_n2xx_net_burner #import implementation
try:
...
class DeviceEntryWidget(tkinter.Frame):
    ...
    def _reload_cb(self):
        ...
        for hint in smini_n2xx_net_burner.enumerate_devices():
...
class SMININ2XXNetBurnerApp(tkinter.Frame):
    """
    The top level gui application for the smini-n2xx network burner.
    ...
    def _burn(self):
        ...
        try:
            ...
            burner = smini_n2xx_net_burner.burner_socket(addr=addr,quiet=False)
...
if __name__=='__main__':
    options = smini_n2xx_net_burner.get_options()
    root = tkinter.Tk()
    root.title('SMINI-N2XX Net Burner')
    SMININ2XXNetBurnerApp(root, addr=options.addr, fw=options.fw, fpga=options.fpga).pack()
    root.mainloop()
    exit()
