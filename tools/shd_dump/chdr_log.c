...
int main(int argc, char *argv[])
{
  ...
  struct in_addr smini_addr;             // Apparent SMINI IP addr in this capture
  ...
  double size_average[2];               // Average size of packets Host to SMINI
  int size_histogram[90][2];            // Array captures histogram of packet sizes Host to SMINI in 100 byte bins
  ...
  while ((c = getopt(argc, argv, "h:u:")) != -1) {
	...
	usage();
      break;
    case 'u':
      // Explicit IP address for SMINI on command line
      if (*optarg == '\0')
	usage();
      smini_addr.s_addr = strtol(strtok(optarg,"."),conversion_error,10) ;
      if  (**conversion_error != '\0')
      	usage();
      smini_addr.s_addr = smini_addr.s_addr | strtol(strtok(NULL,"."),conversion_error,10) << 8;
      if  (**conversion_error != '\0')
       	usage();
      smini_addr.s_addr = smini_addr.s_addr | strtol(strtok(NULL,"."),conversion_error,10) << 16;
      if  (**conversion_error != '\0')
       	usage();
      smini_addr.s_addr = smini_addr.s_addr | strtol(strtok(NULL,"\0"),conversion_error,10) << 24;
      if  (**conversion_error != '\0')
	usage();
      break;
  ...
  // Determine host and SMINI IP addresses so that we can classify direction of packet flow later.
 if (host_addr.s_addr == 0x0)
   get_connection_endpoints(packet_buffer,&host_addr,&smini_addr);
  ...
  /* 	  swapint((int) *(dump_header+5))); */
    ...
    print_direction(packet_buffer,&host_addr,&smini_addr);
    fprintf(stdout,"\t");
