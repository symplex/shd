...
#define SHD_DEVICE3_LOG() SHD_LOGV(never)
...
void device3_impl::merge_channel_defs(
    const std::vector<shd::rfnoc::block_id_t> &chan_ids,
    const std::vector<shd::device_addr_t> &chan_args,
    const shd::direction_t dir
) {
    SHD_ASSERT_THROW(chan_ids.size() == chan_args.size());
    if (dir == shd::DX_DIRECTION) {
        merge_channel_defs(chan_ids, chan_args, RX_DIRECTION);
        merge_channel_defs(chan_ids, chan_args, TX_DIRECTION);
        return;
    }

    shd::fs_path chans_root = shd::fs_path("/channels/") / (dir == RX_DIRECTION ? "rx" : "tx");
    ...
    for (size_t i = 0; i < chan_ids.size(); i++) {
        ...
        if (not _tree->exists(chans_root / chan_idxs[i] / "args")) {
            _tree->create<shd::device_addr_t>(chans_root / chan_idxs[i] / "args");
        }
        _tree->access<shd::device_addr_t>(chans_root / chan_idxs[i] / "args").set(chan_args[i]);
    }
}
...
void device3_impl::enumerate_rfnoc_blocks(
    ...
    shd::sid_t ctrl_sid = base_sid;
    shd::property_tree::sptr subtree = _tree->subtree(shd::fs_path("/mboards") / device_index);
    ...
    for (size_t i = 0; i < n_blocks; i++) {
        SHD_DEVICE3_LOG() << "[RFNOC] ------- Block Setup -----------" << std::endl;
        ...
        SHD_DEVICE3_LOG() << str(boost::format("Setting up NoC-Shell Control for port #0 (SID: %s)...") % xport.send_sid.to_pp_string_hex());
        shd::rfnoc::ctrl_iface::sptr ctrl = shd::rfnoc::ctrl_iface::make(
        ...
        SHD_DEVICE3_LOG() << "OK" << std::endl;
        uint64_t noc_id = ctrl->peek64(shd::rfnoc::SR_READBACK_REG_ID);
        SHD_DEVICE3_LOG() << str(boost::format("Port %d: Found NoC-Block with ID %016X.") % int(ctrl_sid.get_dst_endpoint()) % noc_id) << std::endl;
        shd::rfnoc::make_args_t make_args;
        shd::rfnoc::blockdef::sptr block_def = shd::rfnoc::blockdef::make_from_noc_id(noc_id);
        if (not block_def) {
            SHD_DEVICE3_LOG() << "Using default block configuration." << std::endl;
            block_def = shd::rfnoc::blockdef::make_from_noc_id(shd::rfnoc::DEFAULT_NOC_ID);
        }
        SHD_ASSERT_THROW(block_def);
        ...
        BOOST_FOREACH(const size_t port_number, block_def->get_all_port_numbers()) {
            ...
            SHD_DEVICE3_LOG() << str(boost::format("Setting up NoC-Shell Control for port #%d (SID: %s)...") % port_number % xport1.send_sid.to_pp_string_hex());
            shd::rfnoc::ctrl_iface::sptr ctrl1 = shd::rfnoc::ctrl_iface::make(
                    endianness == ENDIANNESS_BIG,
                    xport1.send,
                    xport1.recv,
                    xport1.send_sid,
        ...
        _rfnoc_block_ctrl.push_back(shd::rfnoc::block_ctrl_base::make(make_args, noc_id));
    }
}


shd::rfnoc::graph::sptr device3_impl::create_graph(const std::string &name)
{
    return boost::make_shared<shd::rfnoc::graph_impl>(
            name,
            shared_from_this()
    );
}
