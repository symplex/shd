...
#define SHD_STREAMER_LOG() SHD_LOGV(never)
 ...
 **********************************************************************/
static shd::stream_args_t sanitize_stream_args(const shd::stream_args_t &args_)
{
...
void generate_channel_list(
        ...
        std::vector<shd::rfnoc::block_id_t> &chan_list,
        std::vector<device_addr_t> &chan_args
) {
    shd::stream_args_t args = args_;
    std::vector<shd::rfnoc::block_id_t> chan_list_(args.channels.size());
...
static size_t get_rx_flow_control_window(
    ...
    double fullness_factor = rx_args.cast<double>(
            "recv_buff_fullness",
            shd::rfnoc::DEFAULT_FC_RX_SW_BUFF_FULL_FACTOR
    );

    if (fullness_factor < 0.01 || fullness_factor > 1) {
        throw shd::value_error("recv_buff_fullness must be in [0.01, 1] inclusive (1% to 100%)");
    }
    ...
    if (window_in_pkts == 0) {
        throw shd::value_error("recv_buff_size must be larger than the recv_frame_size.");
    }
    SHD_ASSERT_THROW(size_t(sw_buff_size * fullness_factor) >= pkt_size * window_in_pkts);
...
static void handle_rx_flowctrl(
    ...
    if (not buff) {
        throw shd::runtime_error("handle_rx_flowctrl timed out getting a send buffer");
    }
    ...
    //SHD_MSG(status) << "sending flow ctrl packet " << fc_pkt_count++ << ", acking " << str(boost::format("%04d\tseq_sw==0x%08x") % last_seq % seq32) << std::endl;
    ...
    if (endianness == ENDIANNESS_BIG) {
        ...
        pkt[packet_info.num_header_words32+RXFC_CMD_CODE_OFFSET] = shd::htonx<uint32_t>(0);
        pkt[packet_info.num_header_words32+RXFC_SEQ_NUM_OFFSET]  = shd::htonx<uint32_t>(seq32);
    } else {
        // Load Header:
        vrt::chdr::if_hdr_pack_le(pkt, packet_info);
        // Load Payload: (the sequence number)
        pkt[packet_info.num_header_words32+RXFC_CMD_CODE_OFFSET] = shd::htowx<uint32_t>(0);
        pkt[packet_info.num_header_words32+RXFC_SEQ_NUM_OFFSET]  = shd::htowx<uint32_t>(seq32);
    }
...
static size_t get_tx_flow_control_window(
    ...
    if (window_in_pkts == 0) {
        throw shd::value_error("send_buff_size must be larger than the send_frame_size.");
    }
    return window_in_pkts;
}
...
static bool tx_flow_ctrl(
    ...
    while (true)
    {
		...
		if (buff)
		{
			...
			{
				SHD_MSG(error) << "Error unpacking async flow control packet: " << ex.what() << std::endl;
				continue;
			}

			if (if_packet_info.packet_type != vrt::if_packet_info_t::PACKET_TYPE_FC)
			{
				SHD_MSG(error) << "Unexpected packet type received by flow control handler: " << if_packet_info.packet_type << std::endl;
				continue;
			}
 ...
 *
 * This is run inside a shd::task as long as this streamer lives.
 */
static void handle_tx_async_msgs(
    ...
    //unpacking can fail
    uint32_t (*endian_conv)(uint32_t) = shd::ntohx;
    try
    {
        if (endianness == ENDIANNESS_BIG)
        {
            vrt::chdr::if_hdr_unpack_be(packet_buff, if_packet_info);
            endian_conv = shd::ntohx;
        }
        else
        {
            vrt::chdr::if_hdr_unpack_le(packet_buff, if_packet_info);
            endian_conv = shd::wtohx;
        }
    }
    catch(const std::exception &ex)
    {
        SHD_MSG(error) << "Error parsing async message packet: " << ex.what() << std::endl;
        return;
    }
    ...
    {
        SHD_MSG(error) << "Unexpected flow control message found in async message handling" << std::endl;
    } else {
...
void device3_impl::update_rx_streamers(double /* rate */)
{
    BOOST_FOREACH(const std::string &block_id, _rx_streamers.keys()) {
        SHD_STREAMER_LOG() << "[Device3] updating RX streamer to " << block_id << std::endl;
        ...
        if (my_streamer) {
            ...
            SHD_STREAMER_LOG() << "  New tick_rate == " << tick_rate << "  New samp_rate == " << samp_rate << " New scaling == " << scaling << std::endl;
...
rx_streamer::sptr device3_impl::get_rx_stream(const stream_args_t &args_)
{
    ...
    for (size_t stream_i = 0; stream_i < chan_list.size(); stream_i++) {
        ...
        shd::rfnoc::block_id_t block_id = chan_list[stream_i];
        SHD_STREAMER_LOG() << "[RX Streamer] chan " << stream_i << " connecting to " << block_id << std::endl;
        ...
        shd::rfnoc::source_block_ctrl_base::sptr blk_ctrl =
            boost::dynamic_pointer_cast<shd::rfnoc::source_block_ctrl_base>(get_block_ctrl(block_id));
        ...
        shd::sid_t stream_address = blk_ctrl->get_address(block_port);
        SHD_STREAMER_LOG() << "[RX Streamer] creating rx stream " << rx_hints.to_string() << std::endl;
        both_xports_t xport = make_transport(stream_address, RX_DATA, rx_hints);
        SHD_STREAMER_LOG() << std::hex << "[RX Streamer] data_sid = " << xport.send_sid << std::dec << " actual recv_buff_size = " << xport.recv_buff_size << std::endl;

        // Configure the block
        blk_ctrl->set_destination(xport.send_sid.get_src(), block_port);

        blk_ctrl->sr_write(shd::rfnoc::SR_RESP_OUT_DST_SID, xport.send_sid.get_src(), block_port);
        SHD_STREAMER_LOG() << "[RX Streamer] resp_out_dst_sid == " << xport.send_sid.get_src() << std::endl;

        // Find all upstream radio nodes and set their response in SID to the host
        std::vector<boost::shared_ptr<shd::rfnoc::radio_ctrl> > upstream_radio_nodes = blk_ctrl->find_upstream_node<shd::rfnoc::radio_ctrl>();
        SHD_STREAMER_LOG() << "[RX Streamer] Number of upstream radio nodes: " << upstream_radio_nodes.size() << std::endl;
        BOOST_FOREACH(const boost::shared_ptr<shd::rfnoc::radio_ctrl> &node, upstream_radio_nodes) {
            node->sr_write(shd::rfnoc::SR_RESP_OUT_DST_SID, xport.send_sid.get_src(), block_port);
        }
        ...
        SHD_STREAMER_LOG() << "[RX Streamer] spp == " << spp << std::endl;
        ...
        SHD_STREAMER_LOG()<< "[RX Streamer] Flow Control Window (minus one) = " << fc_window-1 << ", Flow Control Handler Window = " << fc_handle_window << std::endl;
        ...
        my_streamer->set_overflow_handler(
              stream_i,
              boost::bind(
                  &shd::rfnoc::rx_stream_terminator::handle_overrun, recv_terminator,
                  boost::weak_ptr<shd::rx_streamer>(my_streamer), stream_i
              )
        );
        ...
        my_streamer->set_issue_stream_cmd(
            stream_i,
            boost::bind(&shd::rfnoc::source_block_ctrl_base::issue_stream_cmd, blk_ctrl, _1, block_port)
        );
...
void device3_impl::update_tx_streamers(double /* rate */)
{
    BOOST_FOREACH(const std::string &block_id, _tx_streamers.keys()) {
        SHD_STREAMER_LOG() << "[Device3] updating TX streamer: " << block_id << std::endl;
...
tx_streamer::sptr device3_impl::get_tx_stream(const shd::stream_args_t &args_)
{
    boost::mutex::scoped_lock lock(_transport_setup_mutex);
    stream_args_t args = sanitize_stream_args(args_);
    ...
    for (size_t stream_i = 0; stream_i < chan_list.size(); stream_i++) {
        ...
        size_t mb_index = block_id.get_device_no();
        size_t suggested_block_port = args.args.cast<size_t>("block_port", rfnoc::ANY_PORT);
        shd::endianness_t endianness = get_transport_endianness(mb_index);

        // Access to this channel's block control
        shd::rfnoc::sink_block_ctrl_base::sptr blk_ctrl =
            boost::dynamic_pointer_cast<shd::rfnoc::sink_block_ctrl_base>(get_block_ctrl(block_id));
        ...
        //allocate sid and create transport
        shd::sid_t stream_address = blk_ctrl->get_address(block_port);
        SHD_STREAMER_LOG() << "[TX Streamer] creating tx stream " << tx_hints.to_string() << std::endl;
        ...
        SHD_STREAMER_LOG() << std::hex << "[TX Streamer] data_sid = " << xport.send_sid << std::dec << std::endl;
        ...
        const size_t bpi = convert::get_bytes_per_item(args.otw_format); // bytes per item
        const size_t spp = std::min(args.args.cast<size_t>("spp", bpp/bpi), bpp/bpi); // samples per packet
        SHD_STREAMER_LOG() << "[TX Streamer] spp == " << spp << std::endl;
        ...
        SHD_STREAMER_LOG() << "[TX Streamer] Flow Control Window = " << fc_window << ", Flow Control Handler Window = " << fc_handle_window << std::endl;
        ...
        blk_ctrl->sr_write(shd::rfnoc::SR_CLEAR_RX_FC, 0xc1ea12, block_port);
        blk_ctrl->sr_write(shd::rfnoc::SR_RESP_IN_DST_SID, my_streamer->_async_xport.recv_sid.get_dst(), block_port);
        SHD_STREAMER_LOG() << "[TX Streamer] resp_in_dst_sid == " << boost::format("0x%04X") % xport.recv_sid.get_dst() << std::endl;
        ...
        if (args.args.has_key("radio_id") and args.args.has_key("radio_port"))
        {
            ...
            shd::rfnoc::block_id_t radio_id(args.args["radio_id"]);
            size_t radio_port = args.args.cast<size_t>("radio_port", 0);
            std::vector<boost::shared_ptr<shd::rfnoc::radio_ctrl> > downstream_radio_nodes = blk_ctrl->find_downstream_node<shd::rfnoc::radio_ctrl>();
            SHD_STREAMER_LOG() << "[TX Streamer] Number of downstream radio nodes: " << downstream_radio_nodes.size() << std::endl;
            BOOST_FOREACH(const boost::shared_ptr<shd::rfnoc::radio_ctrl> &node, downstream_radio_nodes) {
                if (node->get_block_id() == radio_id) {
                    node->sr_write(shd::rfnoc::SR_RESP_IN_DST_SID, my_streamer->_async_xport.recv_sid.get_dst(), radio_port);
                }
            }
        } else {
            ...
            SHD_STREAMER_LOG() << "[TX Streamer] Number of downstream radio nodes: " << downstream_radio_nodes.size() << std::endl;
            BOOST_FOREACH(const boost::shared_ptr<shd::rfnoc::radio_ctrl> &node, downstream_radio_nodes) {
                node->sr_write(shd::rfnoc::SR_RESP_IN_DST_SID, my_streamer->_async_xport.recv_sid.get_dst(), block_port);
            }
        }
        ...
        my_streamer->_xport.send = zero_copy_flow_ctrl::make(
            ...
            boost::bind(
				...
				(endianness == ENDIANNESS_BIG ? shd::ntohx<uint32_t> : shd::wtohx<uint32_t>),
