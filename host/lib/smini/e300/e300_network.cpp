...
static void e300_recv_tunnel(
    ...
    shd::transport::zero_copy_if::sptr recver,
...
)
{
    ...
    try
    {
        while (*running)
        {
            ...
            if (E300_NETWORK_DEBUG) SHD_MSG(status) << name << " got " << buff->size() << std::endl;
    ...
    {
        SHD_MSG(error) << "e300_recv_tunnel exit " << name << " " << ex.what() << std::endl;
    }
    catch(...)
    {
        SHD_MSG(error) << "e300_recv_tunnel exit " << name << std::endl;
    }
    SHD_MSG(status) << "e300_recv_tunnel exit " << name << std::endl;
    *running = false;
}
...
static void e300_send_tunnel(
    ...
    shd::transport::zero_copy_if::sptr sender,
    asio::ip::udp::endpoint *endpoint,
    bool *running
)
{
    ...
    try
    {
        while (*running)
        {
            ...
            if (E300_NETWORK_DEBUG) SHD_MSG(status) << name << " got " << num_bytes << std::endl;
    ...
    {
        SHD_MSG(error) << "e300_send_tunnel exit " << name << " " << ex.what() << std::endl;
    }
    catch(...)
    {
        SHD_MSG(error) << "e300_send_tunnel exit " << name << std::endl;
    }
    SHD_MSG(status) << "e300_send_tunnel exit " << name << std::endl;
    *running = false;
}
...
)
{
    ...
    try
    {
        while (*running)
        {
            ...
            switch (shd::ntohx<uint32_t>(in->which)) {
            ...
            switch (shd::ntohx<uint32_t>(in->action)) {
            ...
            case codec_xact_t::ACTION_SET_ACTIVE_CHANS:
                _codec_ctrl->set_active_chains(
                    shd::ntohx<uint32_t>(in->bits) & (1<<0),
                    shd::ntohx<uint32_t>(in->bits) & (1<<1),
                    shd::ntohx<uint32_t>(in->bits) & (1<<2),
                    shd::ntohx<uint32_t>(in->bits) & (1<<3));
                break;
            ...
            case codec_xact_t::ACTION_SET_BW:
                ...
                break;
            default:
                SHD_MSG(status) << "Got unknown request?!" << std::endl;
                //Zero out actions to fail this request on client
                out->action = shd::htonx<uint32_t>(0);
            }
    ...
    {
        SHD_MSG(error) << "e300_ctrl_tunnel exit " << name << " " << ex.what() << std::endl;
    }
    catch(...)
    {
        SHD_MSG(error) << "e300_ctrl_tunnel exit " << name << std::endl;
    }
    SHD_MSG(status) << "e300_ctrl_tunnel exit " << name << std::endl;
    *running = false;
}
...
)
{
    ...
    try
    {
        while (*running)
        {
            ...
            if(shd::ntohx<uint32_t>(in->is_poke)) {
                regs->poke32(shd::ntohx<uint32_t>(in->addr), shd::ntohx<uint32_t>(in->data));
            }
            else {
                in->data = shd::htonx<uint32_t>(regs->peek32(shd::ntohx<uint32_t>(in->addr)));
    ...
    {
        SHD_MSG(error) << "e300_gregs_tunnel exit " << name << " " << ex.what() << std::endl;
    }
    catch(...)
    {
        SHD_MSG(error) << "e300_gregs_tunnel exit " << name << std::endl;
    }
    SHD_MSG(status) << "e300_gregs_tunnel exit " << name << std::endl;
    *running = false;
}
...
)
{
    ...
    try
    {
        while (*running)
        {
            ...
            shd::smini::e300::sensor_transaction_t *in =
                reinterpret_cast<shd::smini::e300::sensor_transaction_t *>(in_buff);

            if (shd::ntohx(in->which) == ZYNQ_TEMP) {
                sensor_value_t temp = sensor_manager->get_mb_temp();
                // TODO: This is ugly ... use proper serialization
                in->value = shd::htonx<uint32_t>(
                    e300_sensor_manager::pack_float_in_uint32_t(temp.to_real()));
            } else if (shd::ntohx(in->which) == REF_LOCK) {
                in->value = shd::htonx<uint32_t>(
    ...
    {
        SHD_MSG(error) << "e300_sensor_tunnel exit " << name << " " << ex.what() << std::endl;
    }
    catch(...)
    {
        SHD_MSG(error) << "e300_sensor_tunnel exit " << name << std::endl;
    }
    SHD_MSG(status) << "e300_sensor_tunnel exit " << name << std::endl;
    *running = false;
}
...
static void e300_i2c_tunnel(
    const std::string &name,
    boost::shared_ptr<asio::ip::udp::socket> socket,
    shd::smini::e300::i2c::sptr i2c,
    asio::ip::udp::endpoint *endpoint,
    bool *running
)
{
    SHD_ASSERT_THROW(i2c);
    asio::ip::udp::endpoint _endpoint;
    try
    {
        while (*running)
        {
            uint8_t in_buff[sizeof(shd::smini::e300::i2c_transaction_t)];

            const size_t num_bytes = socket->receive_from(asio::buffer(in_buff), *endpoint);

            if (num_bytes < sizeof(shd::smini::e300::i2c_transaction_t)) {
                std::cout << "Received short packet: " << num_bytes << std::endl;
                continue;
            }

            shd::smini::e300::i2c_transaction_t *in =
                reinterpret_cast<shd::smini::e300::i2c_transaction_t *>(in_buff);
            ...
            if(in->type & i2c::ONEBYTE) {
                if(in->type & i2c::WRITE) {
                    i2c->set_i2c_reg8(
                        in->addr,
                        shd::ntohx<uint16_t>(in->reg), in->data);
                } else {
                    in->data = i2c->get_i2c_reg8(in->addr, shd::ntohx<uint16_t>(in->reg));
            ...
            // 2 byte addressed accesses go through here
                ...
                if(in->type & i2c::WRITE) {
                    ...
                    in->data = i2c->get_i2c_reg16(in->addr, shd::ntohx<uint16_t>(in->reg));
                    socket->send_to(asio::buffer(in_buff, sizeof(in_buff)), *endpoint);
                }

            } else {
                SHD_MSG(error) << "e300_i2c_tunnel could not handle message." << std::endl;
            }
        }
    }
    catch(const std::exception &ex)
    {
        SHD_MSG(error) << "e300_i2c_tunnel exit " << name << " " << ex.what() << std::endl;
    }
    catch(...)
    {
        SHD_MSG(error) << "e300_i2c_tunnel exit " << name << std::endl;
    }
    SHD_MSG(status) << "e300_i2c_tunnel exit " << name << std::endl;
    *running = false;
}
...
{
public:
    network_server_impl(const shd::device_addr_t &device_addr);
...
private:
    struct xports_t
    {
        shd::transport::zero_copy_if::sptr send_ctrl_xport;
        shd::transport::zero_copy_if::sptr recv_ctrl_xport;
        shd::transport::zero_copy_if::sptr tx_data_xport;
        shd::transport::zero_copy_if::sptr tx_flow_xport;
        shd::transport::zero_copy_if::sptr rx_data_xport;
        shd::transport::zero_copy_if::sptr rx_flow_xport;
    };
    ...
    const size_t fe)
{
    ...
    {
        SHD_MSG(status) << "e300 run server on port " << port << " for " << what << std::endl;
        try
        {
            ...
            SHD_MSG(status) << "e300 socket accept on port " << port << " for " << what << std::endl;
...
network_server_impl::network_server_impl(const shd::device_addr_t &device_addr)
{
    ...
    if (not device_addr.has_key("no_reload_fpga")) {
        ...
        // Else load the FPGA image based on the product ID
            ...
            case e300_eeprom_manager::SMINI_E310_SG1_MB:
                fpga_image = find_image_path(E310_SG1_FPGA_FILE_NAME);
                break;
            case e300_eeprom_manager::SMINI_E310_SG3_MB:
                fpga_image = find_image_path(E310_SG3_FPGA_FILE_NAME);
                break;
            case e300_eeprom_manager::SMINI_E300_MB:
                fpga_image = find_image_path(E300_FPGA_FILE_NAME);
                break;
            case e300_eeprom_manager::UNKNOWN:
            default:
                SHD_MSG(warning) << "Unknown motherboard type, loading e300 image."
                                 << std::endl;
                fpga_image = find_image_path(E300_FPGA_FILE_NAME);
                break;
            }
    ...
    shd::transport::zero_copy_xport_params ctrl_xport_params;
    ...
    shd::transport::zero_copy_xport_params data_xport_params;
    ...
    try {
        ...
        throw shd::runtime_error("Failed to get driver parameters from sysfs.");
    }
...
network_server::sptr network_server::make(const shd::device_addr_t &device_addr)
{
...
network_server::sptr network_server::make(const shd::device_addr_t &)
{
    throw shd::assertion_error("network_server::make() !E300_NATIVE");
}
#endif
