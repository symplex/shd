...
smini2_codec_ctrl::~smini2_codec_ctrl(void){
    /* NOP */
}

/*!
 * A smini2 codec control specific to the ad9777 ic.
 */
class smini2_codec_ctrl_impl : public smini2_codec_ctrl{
public:
    smini2_codec_ctrl_impl(smini2_iface::sptr iface, shd::spi_iface::sptr spiface){
        _iface = iface;
        _spiface = spiface;
    ...
    ~smini2_codec_ctrl_impl(void){SHD_SAFE_CALL(
        //power-down dac
    ...
    void set_tx_mod_mode(int mod_mode){
        ...
        default: throw shd::value_error("unknown modulation mode for ad9777");
        }
...
private:
    ...
    void send_ad9777_reg(uint8_t addr){
        ...
        SHD_LOGV(always) << "send_ad9777_reg: " << std::hex << reg << std::endl;
        _spiface->write_spi(
...
/***********************************************************************
 * Public make function for the smini2 codec control
 **********************************************************************/
smini2_codec_ctrl::sptr smini2_codec_ctrl::make(smini2_iface::sptr iface, shd::spi_iface::sptr spiface){
    return sptr(new smini2_codec_ctrl_impl(iface, spiface));
}
