 ...
 * Transmit thread
 **********************************************************************/
static void tx_thread(shd::smini::multi_smini::sptr smini, const double tx_wave_freq, const double tx_wave_ampl)
{
    shd::set_thread_priority_safe();

    // set max TX gain
    smini->set_tx_gain(smini->get_tx_gain_range().stop());
    ...
    size_t index = 0;
    const double tx_rate = smini->get_tx_rate();
 ...
 **********************************************************************/
static double tune_rx_and_tx(shd::smini::multi_smini::sptr smini, const double tx_lo_freq, const double rx_offset)
{
    ...
    //tune the receiver
    double rx_freq = smini->get_tx_freq() - rx_offset;
    double min_fe_rx_freq = smini->get_fe_rx_freq_range().start();
    double max_fe_rx_freq = smini->get_fe_rx_freq_range().stop();
    shd::tune_request_t rx_tune_req(rx_freq);
...
int SHD_SAFE_MAIN(int argc, char *argv[])
{
    ...
    if (vm.count("help"))
    {
        std::cout << boost::format("SMINI Generate TX IQ Balance Calibration Table %s") % desc << std::endl;
        std::cout <<
    ...
    threads.create_thread(boost::bind(&tx_thread, smini, tx_wave_freq, tx_wave_ampl));
    ...
    if (not vm.count("freq_start")) freq_start = smini->get_fe_tx_freq_range().start();
    if (not vm.count("freq_stop")) freq_stop = smini->get_fe_tx_freq_range().stop();

    //check start and stop frequencies
    if (freq_start < smini->get_fe_tx_freq_range().start())
    {
        std::cerr << "freq_start must be " << smini->get_fe_tx_freq_range().start() << " or greater for this daughter board" << std::endl;
        return EXIT_FAILURE;
    }
    if (freq_stop > smini->get_fe_tx_freq_range().stop())
    {
        std::cerr << "freq_stop must be " << smini->get_fe_tx_freq_range().stop() << " or less for this daughter board" << std::endl;
        return EXIT_FAILURE;
    }

    //check rx_offset
    double min_rx_offset = smini->get_rx_freq_range().start() - smini->get_fe_tx_freq_range().start();
    double max_rx_offset = smini->get_rx_freq_range().stop() - smini->get_fe_tx_freq_range().stop();
    ...
    {
        const double tx_lo = tune_rx_and_tx(smini, tx_lo_i, rx_offset);
        ...
        //reset TX IQ balance
        smini->set_tx_iq_balance(0.0);

        //set optimal RX gain setting for this frequency
        set_optimal_rx_gain(smini, rx_stream, tx_wave_freq);
        ...
        while (phase_corr_step >= precision or ampl_corr_step >= precision)
        {
            for (double phase_corr = phase_corr_start + phase_corr_step; phase_corr <= phase_corr_stop - phase_corr_step; phase_corr += phase_corr_step)
            {
                for (double ampl_corr = ampl_corr_start + ampl_corr_step; ampl_corr <= ampl_corr_stop - ampl_corr_step; ampl_corr += ampl_corr_step)
                {
                    ...
                    smini->set_tx_iq_balance(correction);
