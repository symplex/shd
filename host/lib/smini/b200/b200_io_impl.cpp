...
void b200_impl::check_streamer_args(const shd::stream_args_t &args, double tick_rate, const std::string &direction /*= ""*/)
{
...
void b200_impl::set_auto_tick_rate(
    ...
    try {
        ...
        if (!shd::math::frequencies_are_equal(_tree->access<double>("/mboards/0/tick_rate").get(), new_rate)) {
            _tree->access<double>("/mboards/0/tick_rate").set(new_rate);
        }
    } catch (const shd::value_error &) {
        SHD_MSG(warning)
...
void b200_impl::update_rx_dsp_tick_rate(const double tick_rate, rx_dsp_core_3000::sptr ddc, shd::fs_path rx_dsp_path)
{
...
void b200_impl::update_tx_dsp_tick_rate(const double tick_rate, tx_dsp_core_3000::sptr duc, shd::fs_path tx_dsp_path)
{
...
#define CHECK_RATE_AND_THROW(rate)  \
        if (shd::math::fp_compare::fp_compare_delta<double>(rate, shd::math::FREQ_COMPARISON_DELTA_HZ) > \
            shd::math::fp_compare::fp_compare_delta<double>(ad9361_device_t::AD9361_MAX_CLOCK_RATE, shd::math::FREQ_COMPARISON_DELTA_HZ)) { \
            throw shd::value_error(str( \
 ...
 * frontend selection
 **********************************************************************/
shd::smini::subdev_spec_t b200_impl::coerce_subdev_spec(const shd::smini::subdev_spec_t &spec_)
{
    shd::smini::subdev_spec_t spec = spec_;
...
void b200_impl::update_subdev_spec(const std::string &tx_rx, const shd::smini::subdev_spec_t &spec)
{
    //sanity checking
    if (spec.size()) {
 ...
 */
boost::optional<shd::msg_task::msg_type_t> b200_impl::handle_async_task(
...
)
{
    ...
    const uint32_t sid = shd::wtohx(buff->cast<const uint32_t *>()[1]);
    switch (sid) {
    ...
    case B200_LOCAL_RESP_SID:
    {
        ...
        if (ctrl){
            ...
            return std::make_pair(sid, shd::msg_task::buff_to_vector(buff->cast<uint8_t *>(), buff->size() ) );
        }
        break;
    }
    ...
    case B200_TX_MSG1_SID:
    {
        ...
        {
            SHD_MSG(error) << "Error parsing ctrl packet: " << ex.what() << std::endl;
            break;
        }

        //fill in the async metadata
        async_metadata_t metadata;
        load_metadata_from_buff(shd::wtohx<uint32_t>, metadata, if_packet_info, packet_buff, _tick_rate, i);
    ...
    //doh!
    default:
        SHD_MSG(error) << "Got a ctrl packet with unknown SID " << sid << std::endl;
    }
    return boost::none;
}
 ...
 * Receive streamer
 **********************************************************************/
rx_streamer::sptr b200_impl::get_rx_stream(const shd::stream_args_t &args_)
{
 ...
 * Transmit streamer
 **********************************************************************/
tx_streamer::sptr b200_impl::get_tx_stream(const shd::stream_args_t &args_)
{
    boost::mutex::scoped_lock lock(_transport_setup_mutex);
