...
static void eth_addrs_init(void){
    ...
    #define eth_addrs_init_x(addr, x){ \
        const bool ok = eeprom_read(SMINI2_I2C_ADDR_MBOARD, addr, &current_eth_addrs.x, sizeof(current_eth_addrs.x)); \
    ...
    eth_addrs_init_x(SMINI2_EE_MBOARD_MAC_ADDR, mac_addr);
    eth_addrs_init_x(SMINI2_EE_MBOARD_IP_ADDR,  ip_addr);
    eth_addrs_init_x(SMINI2_EE_MBOARD_GATEWAY,  gateway);
    eth_addrs_init_x(SMINI2_EE_MBOARD_SUBNET,   subnet);
...
bool set_ip_addr(const struct ip_addr *t){
    const bool ok = eeprom_write(SMINI2_I2C_ADDR_MBOARD, SMINI2_EE_MBOARD_IP_ADDR, t, sizeof(struct ip_addr));
