...
BOOST_AUTO_TEST_CASE(test_time_spec_compare){
    ...
    BOOST_CHECK(shd::time_spec_t(2.0) == shd::time_spec_t(2.0));
    BOOST_CHECK(shd::time_spec_t(2.0) > shd::time_spec_t(1.0));
    BOOST_CHECK(shd::time_spec_t(1.0) < shd::time_spec_t(2.0));

    BOOST_CHECK(shd::time_spec_t(1.1) == shd::time_spec_t(1.1));
    BOOST_CHECK(shd::time_spec_t(1.1) > shd::time_spec_t(1.0));
    BOOST_CHECK(shd::time_spec_t(1.0) < shd::time_spec_t(1.1));

    BOOST_CHECK(shd::time_spec_t(0.1) == shd::time_spec_t(0.1));
    BOOST_CHECK(shd::time_spec_t(0.2) > shd::time_spec_t(0.1));
    BOOST_CHECK(shd::time_spec_t(0.1) < shd::time_spec_t(0.2));
}
...
BOOST_AUTO_TEST_CASE(test_time_spec_arithmetic){
    ...
    CHECK_TS_EQUAL(shd::time_spec_t(2.3) + shd::time_spec_t(1.0), shd::time_spec_t(3.3));
    CHECK_TS_EQUAL(shd::time_spec_t(2.3) - shd::time_spec_t(1.0), shd::time_spec_t(1.3));
    CHECK_TS_EQUAL(shd::time_spec_t(1.0) + shd::time_spec_t(2.3), shd::time_spec_t(3.3));
    CHECK_TS_EQUAL(shd::time_spec_t(1.0) - shd::time_spec_t(2.3), shd::time_spec_t(-1.3));
}

BOOST_AUTO_TEST_CASE(test_time_spec_parts){
    std::cout << "Testing time specification parts..." << std::endl;

    BOOST_CHECK_EQUAL(shd::time_spec_t(1.1).get_full_secs(), 1);
    BOOST_CHECK_CLOSE(shd::time_spec_t(1.1).get_frac_secs(), 0.1, 0.001);
    BOOST_CHECK_EQUAL(shd::time_spec_t(1.1).to_ticks(100), 110);

    BOOST_CHECK_EQUAL(shd::time_spec_t(-1.1).get_full_secs(), -2);
    BOOST_CHECK_CLOSE(shd::time_spec_t(-1.1).get_frac_secs(), 0.9, 0.001);
    BOOST_CHECK_EQUAL(shd::time_spec_t(-1.1).to_ticks(100), -110);
}
...
BOOST_AUTO_TEST_CASE(test_time_spec_get_system_time){
    ...
    shd::time_spec_t start = shd::time_spec_t::get_system_time();
    boost::this_thread::sleep(boost::posix_time::milliseconds(500));
    shd::time_spec_t stop = shd::time_spec_t::get_system_time();

    shd::time_spec_t diff = stop - start;
...
BOOST_AUTO_TEST_CASE(test_time_spec_neg_values){
    shd::time_spec_t ts1(0.3);
    shd::time_spec_t ts2(1, -0.9);
    std::cout << "ts1 " << ts1.get_real_secs() << std::endl;
    std::cout << "ts2 " << ts2.get_real_secs() << std::endl;
    BOOST_CHECK(ts1 > ts2);

    shd::time_spec_t tsa(430.001083);
    shd::time_spec_t tsb(429.999818);
    shd::time_spec_t tsc(0.3);
    shd::time_spec_t tsd = tsa - tsb;
...
BOOST_AUTO_TEST_CASE(test_time_large_ticks_to_time_spec)
{
    ...
    const shd::time_spec_t t0 = shd::time_spec_t::from_ticks(ticks0, 100e6);
...
BOOST_AUTO_TEST_CASE(test_time_error_irrational_rate)
{
    ...
    const shd::time_spec_t ts = shd::time_spec_t::from_ticks(tick_in, rate);
