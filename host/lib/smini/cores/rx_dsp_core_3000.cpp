...
public:
    ...
    void set_mux(const shd::smini::fe_connection_t& fe_conn){
        uint32_t reg_val = 0;
        switch (fe_conn.get_sampling_mode()) {
        case shd::smini::fe_connection_t::REAL:
        case shd::smini::fe_connection_t::HETERODYNE:
        ...
        if (fe_conn.get_sampling_mode() == shd::smini::fe_connection_t::HETERODYNE) {
    ...
    shd::meta_range_t get_freq_range(void){
        ...
        return shd::meta_range_t(-(_tick_rate-offset)/2, +(_tick_rate-offset)/2, _tick_rate/std::pow(2.0, 32));
    }
