...
void e100_codec_ctrl_impl::send_reg(uint8_t addr){
    ...
    SHD_LOGV(often) << "codec control write reg: " << std::hex << reg << std::endl;
    _iface->write_spi(
        UE_SPI_SS_AD9862,
...
void e100_codec_ctrl_impl::recv_reg(uint8_t addr){
    ...
    SHD_LOGV(often) << "codec control read reg: " << std::hex << reg << std::endl;
    uint32_t ret = _iface->read_spi(
        UE_SPI_SS_AD9862,
        spi_config_t::EDGE_RISE,
        reg, 16
    );
    SHD_LOGV(often) << "codec control read ret: " << std::hex << ret << std::endl;
