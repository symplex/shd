...
# Test case base
#--------------------------------------------------------------------------
class shd_test_case(unittest.TestCase):
    """
    Base class for SHD test cases.
    """
    test_name = '--TEST--'
    ...
    def setUp(self):
        ...
        self.results = {}
        self.results_file = os.getenv('_SHD_TEST_RESULTSFILE', "")
        if self.results_file and os.path.isfile(self.results_file):
            self.results = yaml.safe_load(open(self.results_file).read()) or {}
        self.args_str = os.getenv('_SHD_TEST_ARGS_STR', "")
        self.smini_info = get_smini_list(self.args_str)[0]
        if not self.results.has_key(self.smini_info['serial']):
            self.results[self.smini_info['serial']] = {}
        if not self.results[self.smini_info['serial']].has_key(self.name):
            self.results[self.smini_info['serial']][self.name] = {}
        self.setup_logger()
        self.set_up()
    ...
    def setup_logger(self):
        ...
        self.log_file = os.getenv('_SHD_TEST_LOGFILE', "devtest.log")
        #self.log_level = int(os.getenv('_SHD_TEST_LOG_LEVEL', logging.DEBUG))
        #self.print_level = int(os.getenv('_SHD_TEST_PRINT_LEVEL', logging.WARNING))
    ...
    def report_result(self, testname, key, value):
        ...
        """
        if not self.results[self.smini_info['serial']][self.name].has_key(testname):
            self.results[self.smini_info['serial']][self.name][testname] = {}
        self.results[self.smini_info['serial']][self.name][testname][key] = value
    ...
    def filter_warnings(self, errstr):
        """ Searches errstr for SHD warnings, removes them, and puts them into a separate string.
        Returns (errstr, warnstr), where errstr no longer has warning. """
        warn_re = re.compile("SHD Warning:\n(?:    .*\n)+")
...
class shd_example_test_case(shd_test_case):
    """
    ...
    def run_example(self, example, args):
        """
        Run `example' (which has to be a SHD example or utility) with `args'.
    ...
    def test_all(self):
        ...
        for test_name, test_args in self.test_params.iteritems():
            ...
            if not test_args.has_key('products') or (self.smini_info['product'] in test_args.get('products', [])):
