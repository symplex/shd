...
#ifndef INCLUDED_SHD_UTILS_MATH_HPP
#define INCLUDED_SHD_UTILS_MATH_HPP
 ...
 * be used in SHD when portable / `std` options are not available.
 */
namespace math {
...
namespace fp_compare {
    ...
    template<typename float_t> class fp_compare_epsilon {
        public:
            SHD_INLINE fp_compare_epsilon(float_t value);
            SHD_INLINE fp_compare_epsilon(float_t value, float_t epsilon);
            SHD_INLINE fp_compare_epsilon(const fp_compare_epsilon<float_t>& copy);
            SHD_INLINE ~fp_compare_epsilon();
            SHD_INLINE void operator=(const fp_compare_epsilon& copy);
     ...
     * equality, and in the end it all comes down to the domain at hand. SHD's
     ...
     *      [(|u - v| / |u|) <= e] && [(|u - v| / |v|) <= e]
     *
     * SHD's modification to this algorithm is using the denominator's epsilon
...
/*!
    ...
    template<typename float_t> class fp_compare_delta {
        public:
            SHD_INLINE fp_compare_delta(float_t value);
            SHD_INLINE fp_compare_delta(float_t value, float_t delta);
            SHD_INLINE fp_compare_delta(const fp_compare_delta<float_t>& copy);
            SHD_INLINE ~fp_compare_delta();
            SHD_INLINE void operator=(const fp_compare_delta& copy);
    ...
    SHD_INLINE bool frequencies_are_equal(double lhs, double rhs) {
...
#include <shd/utils/fp_compare_epsilon.ipp>
#include <shd/utils/fp_compare_delta.ipp>

#endif /* INCLUDED_SHD_UTILS_MATH_HPP */
