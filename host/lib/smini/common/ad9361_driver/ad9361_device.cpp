...
void ad9361_device_t::_setup_rx_fir(size_t num_taps, int32_t decimation)
{
    if (not (decimation == 1 or decimation == 2 or decimation == 4)) {
        throw shd::runtime_error("[ad9361_device_t] Invalid Rx FIR decimation.");
    }
    ...
    for (size_t i = 0; i < num_taps; i++) {
        ...
        case 48:
            ...
            break;
        default:
            throw shd::runtime_error("[ad9361_device_t] Unsupported number of Rx FIR taps.");
        }
    }
...
void ad9361_device_t::_setup_tx_fir(size_t num_taps, int32_t interpolation)
{
    if (not (interpolation == 1 or interpolation == 2 or interpolation == 4)) {
        throw shd::runtime_error("[ad9361_device_t] Invalid Tx FIR interpolation.");
    }
    if (interpolation == 1 and num_taps > 64) {
        throw shd::runtime_error("[ad9361_device_t] Too many Tx FIR taps for interpolation value.");
    }
    boost::scoped_array<uint16_t> coeffs(new uint16_t[num_taps]);
    for (size_t i = 0; i < num_taps; i++) {
        switch (num_taps) {
        ...
        case 48:
            ...
            break;
        default:
            throw shd::runtime_error("[ad9361_device_t] Unsupported number of Tx FIR taps.");
        }
    }
...
void ad9361_device_t::_calibrate_lock_bbpll()
{
    ...
    while (!(_io_iface->peek8(0x05e) & 0x80)) {
        if (count > 1000) {
            throw shd::runtime_error("[ad9361_device_t] BBPLL not locked");
            break;
        }
        count++;
...
void ad9361_device_t::_calibrate_synth_charge_pumps()
{
    ...
    if ((_io_iface->peek8(0x017) & 0x0F) != 5) {
        throw shd::runtime_error("[ad9361_device_t] AD9361 not in ALERT during cal");
    }
    ...
    while (!(_io_iface->peek8(0x244) & 0x80)) {
        if (count > 5) {
            throw shd::runtime_error("[ad9361_device_t] RX charge pump cal failure");
            break;
        }
        count++;
        boost::this_thread::sleep(boost::posix_time::milliseconds(1));
    ...
    while (!(_io_iface->peek8(0x284) & 0x80)) {
        if (count > 5) {
            throw shd::runtime_error("[ad9361_device_t] TX charge pump cal failure");
            break;
        }
        count++;
        boost::this_thread::sleep(boost::posix_time::milliseconds(1));
...
double ad9361_device_t::_calibrate_baseband_rx_analog_filter(double req_rfbw)
{
    ...
    {
        SHD_LOG << "baseband bandwidth too large for current sample rate. Setting bandwidth to: "<<_baseband_bw;
    ...
    while (_io_iface->peek8(0x016) & 0x80) {
        if (count > 100) {
            throw shd::runtime_error("[ad9361_device_t] RX baseband filter cal FAILURE");
            break;
        }
        count++;
        boost::this_thread::sleep(boost::posix_time::milliseconds(1));
...
double ad9361_device_t::_calibrate_baseband_tx_analog_filter(double req_rfbw)
{
    ...
    while (_io_iface->peek8(0x016) & 0x40) {
        if (count > 100) {
            throw shd::runtime_error("[ad9361_device_t] TX baseband filter cal FAILURE");
            break;
        }
...
double ad9361_device_t::_calibrate_secondary_tx_filter(double req_rfbw)
{
    ...
    if ((bbbw_mhz * 2) <= 9) {
        ...
        reg0d0 = 0x00;
        throw shd::runtime_error("[ad9361_device_t] Cal2ndTxFil: INVALID_CODE_PATH bad bbbw_mhz");
    }
...
double ad9361_device_t::_calibrate_rx_TIAs(double req_rfbw)
{
    ...
    if (ceil_bbbw_mhz <= 3) {
        ...
        throw shd::runtime_error("[ad9361_device_t] CalRxTias: INVALID_CODE_PATH bad bbbw_mhz");
    }
...
void ad9361_device_t::_calibrate_baseband_dc_offset()
{
    ...
    while (_io_iface->peek8(0x016) & 0x01) {
        if (count > 100) {
            throw shd::runtime_error("[ad9361_device_t] Baseband DC Offset Calibration Failure");
            break;
        }
        count++;
...
void ad9361_device_t::_calibrate_rf_dc_offset()
{
    ...
    while (_io_iface->peek8(0x016) & 0x02) {
        if (count > 200) {
            throw shd::runtime_error("[ad9361_device_t] RF DC Offset Calibration Failure");
            break;
        }
        count++;
...
void ad9361_device_t::_calibrate_rx_quadrature()
{
    ...
    while (_io_iface->peek8(0x016) & 0x20) {
        if (count > 1000) {
            throw shd::runtime_error("[ad9361_device_t] Rx Quadrature Calibration Failure");
            break;
        }
        count++;
        boost::this_thread::sleep(boost::posix_time::milliseconds(5));
...
void ad9361_device_t::_tx_quadrature_cal_routine() {
    ...
    if (max_cal_freq > bbbw)
        throw shd::runtime_error("[ad9361_device_t] max_cal_freq > bbbw");
    ...
    while (_io_iface->peek8(0x016) & 0x10) {
        if (count > 100) {
            throw shd::runtime_error("[ad9361_device_t] TX Quadrature Calibration Failure");
            break;
        }
        count++;
...
void ad9361_device_t::_calibrate_tx_quadrature()
{
    ...
    if ((_io_iface->peek8(0x017) & 0x0F) != 5) {
        throw shd::runtime_error("[ad9361_device_t] TX Quad Cal started, but not in ALERT");
    }
...
void ad9361_device_t::_program_gain_table() {
    ...
    if (_rx_freq < 1300e6) {
        ...
        new_gain_table = 1;
        throw shd::runtime_error("[ad9361_device_t] Wrong _rx_freq value");
    }
...
void ad9361_device_t::_setup_synth(direction_t direction, double vcorate)
{
    ...
    if (vcoindex > 53)
        throw shd::runtime_error("[ad9361_device_t] vcoindex > 53");
    ...
    if (direction == RX) {
        ...
        throw shd::runtime_error("[ad9361_device_t] [_setup_synth] INVALID_CODE_PATH");
    }
}
...
{
    SHD_LOG << boost::format("[ad9361_device_t::_tune_bbvco] rate=%.10f\n") % rate;
    ...
    if (i == 7)
        throw shd::runtime_error("[ad9361_device_t] _tune_bbvco: wrong vcorate");

    SHD_LOG << boost::format("[ad9361_device_t::_tune_bbvco] vcodiv=%d vcorate=%.10f\n") % vcodiv % vcorate;
    /* Fo = Fref * (Nint + Nfrac / mod) */
    int nint = static_cast<int>(vcorate / fref);
    SHD_LOG << boost::format("[ad9361_device_t::_tune_bbvco] (nint)=%.10f\n") % (vcorate / fref);
    int nfrac = static_cast<int>(boost::math::round(((vcorate / fref) - (double) nint) * (double) modulus));
    SHD_LOG << boost::format("[ad9361_device_t::_tune_bbvco] (nfrac)=%.10f\n") % (((vcorate / fref) - (double) nint) * (double) modulus);
    SHD_LOG << boost::format("[ad9361_device_t::_tune_bbvco] nint=%d nfrac=%d\n") % nint % nfrac;
...
double ad9361_device_t::_tune_helper(direction_t direction, const double value)
{
    ...
    if (i == 7)
        throw shd::runtime_error("[ad9361_device_t] RFVCO can't find valid VCO rate!");
    ...
    if (direction == RX) {
        ...
        if (value < _client_params->get_band_edge(AD9361_RX_BAND0)) {
            ...
            throw shd::runtime_error("[ad9361_device_t] [_tune_helper] INVALID_CODE_PATH");
        }
        ...
        if ((_io_iface->peek8(0x247) & 0x02) == 0) {
            throw shd::runtime_error("[ad9361_device_t] RX PLL NOT LOCKED");
        }
        ...
        if ((_io_iface->peek8(0x287) & 0x02) == 0) {
            throw shd::runtime_error("[ad9361_device_t] TX PLL NOT LOCKED");
        }
...
double ad9361_device_t::_setup_rates(const double rate)
{
    ...
    SHD_LOG << boost::format("[ad9361_device_t::_setup_rates] rate=%.6d\n") % rate;
    ...
    if (rate < 0.33e6) {
        ...
        throw shd::runtime_error("[ad9361_device_t] [_setup_rates] INVALID_CODE_PATH");
    }

    SHD_LOG << boost::format("[ad9361_device_t::_setup_rates] divfactor=%d\n") % divfactor;
    ...
    SHD_LOG << boost::format("[ad9361_device_t::_setup_rates] adcclk=%f\n") % adcclk;
...
void ad9361_device_t::initialize()
{
    ...
    if (device_id != 0x8) {
        throw shd::runtime_error(str(boost::format("[ad9361_device_t::initialize] Device ID readback failure. Expected: 0x8, Received: 0x%x") % device_id));
    }
    ...
    default:
        throw shd::runtime_error("[ad9361_device_t] NOT IMPLEMENTED");
    }
    boost::this_thread::sleep(boost::posix_time::milliseconds(20));
...
double ad9361_device_t::set_clock_rate(const double req_rate)
{
    ...
    if (req_rate > 61.44e6) {
        throw shd::runtime_error("[ad9361_device_t] Requested master clock rate outside range");
    }

    SHD_LOG << boost::format("[ad9361_device_t::set_clock_rate] req_rate=%.10f\n") % req_rate;

    /* SHD has a habit of requesting the same rate like four times when it
    ...
    default:
        throw shd::runtime_error("[ad9361_device_t] [set_clock_rate:1] AD9361 in unknown state");
        break;
    };
    ...
    SHD_LOG << boost::format("[ad9361_device_t::set_clock_rate] rate=%.10f\n") % rate;
    ...
    default:
        throw shd::runtime_error("[ad9361_device_t] [set_clock_rate:2] AD9361 in unknown state");
        break;
    };
 ...
 * provides one TX antenna, and one RX antenna. The B200 maintains the SMINI
...
double ad9361_device_t::tune(direction_t direction, const double value)
{
    ...
    if (direction == RX) {
        ...
        throw shd::runtime_error("[ad9361_device_t] [tune] INVALID_CODE_PATH");
    }
...
double ad9361_device_t::_get_temperature(const double cal_offset, const double timeout)
{
    ...
    while(((_io_iface->peek8(0x00C) >> 1) & 0x01) == 0) {
        ...
        {
            throw shd::runtime_error("[ad9361_device_t] timeout while reading temperature");
        }
    }
...
void ad9361_device_t::set_dc_offset_auto(direction_t direction, const bool on)
{
    if (direction == RX) {
        ...
        throw shd::runtime_error("[ad9361_device_t] [set_dc_offset_auto] Tx DC tracking not supported");
    }
}
...
void ad9361_device_t::set_iq_balance_auto(direction_t direction, const bool on)
{
    if (direction == RX) {
        ...
        throw shd::runtime_error("[ad9361_device_t] [set_iq_balance_auto] Tx IQ tracking not supported");
    }
}
...
void ad9361_device_t::_setup_agc(chain_t chain, gain_mode_t gain_mode)
{
    ...
    {
        throw shd::runtime_error("[ad9361_device_t] Wrong value for chain");
    }
    ...
    switch (gain_mode) {
        ...
        case GAIN_MODE_FAST_AGC:
            ...
            break;
        default:
            throw shd::runtime_error("[ad9361_device_t] Gain mode does not exist");
    }
...
filter_info_base::sptr ad9361_device_t::get_filter(direction_t direction, chain_t chain, const std::string &name)
{
    if(direction == RX) {
        ...
        {
            throw shd::runtime_error("ad9361_device_t::get_filter this filter can not be read.");
        }
    ...
    throw shd::runtime_error("ad9361_device_t::get_filter wrong direction parameter.");
}
...
void ad9361_device_t::set_filter(direction_t direction, chain_t chain, const std::string &name, filter_info_base::sptr filter)
{
    ...
    if(direction == RX) {
        ...
        {
            throw shd::runtime_error("ad9361_device_t::set_filter this filter can not be written.");
        }
...
void ad9361_device_t::_set_fir_taps(direction_t direction, chain_t chain, const std::vector<int16_t>& taps)
{
    ...
    if(num_taps == num_taps_avail)
    {
        ...
        throw shd::runtime_error("ad9361_device_t::_set_fir_taps not enough coefficients.");
    } else {
        throw shd::runtime_error("ad9361_device_t::_set_fir_taps too many coefficients.");
    }
}
...
std::vector<int16_t> ad9361_device_t::_get_fir_taps(direction_t direction, chain_t chain)
{
    ...
    if(chain == CHAIN_1)
    {
        ...
        throw shd::runtime_error("[ad9361_device_t] Can not read both chains synchronously");
    }
