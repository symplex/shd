...
static const std::string X300_FW_FILE_NAME  = "smini_x300_fw.bin";
...
static const size_t X300_ETH_MSG_FRAME_SIZE         = shd::transport::udp_simple::mtu;  //bytes
...
static const size_t X300_MAX_RATE_10GIGE            = (size_t)(  // bytes/s
        ...
        ( float(X300_10GE_DATA_FRAME_MAX_SIZE - shd::smini::DEVICE3_TX_MAX_HDR_LEN) /
...
static const size_t X300_MAX_RATE_1GIGE            = (size_t)(  // bytes/s
        10e9 / 8 *                                               // wire speed multiplied by percentage of packets that is sample data
        ( float(X300_1GE_DATA_FRAME_MAX_SIZE - shd::smini::DEVICE3_TX_MAX_HDR_LEN) /
...
shd::uart_iface::sptr x300_make_uart_iface(shd::wb_iface::sptr iface);

shd::wb_iface::sptr x300_make_ctrl_iface_enet(shd::transport::udp_simple::sptr udp, bool enable_errors = true);
shd::wb_iface::sptr x300_make_ctrl_iface_pcie(shd::nisminirio::niriok_proxy::sptr drv_proxy, bool enable_errors = true);

shd::device_addrs_t x300_find(const shd::device_addr_t &hint_);

class x300_impl : public shd::smini::device3_impl
{
public:

    x300_impl(const shd::device_addr_t &);
    void setup_mb(const size_t which, const shd::device_addr_t &);
    ~x300_impl(void);

    // device claim functions
    enum claim_status_t {UNCLAIMED, CLAIMED_BY_US, CLAIMED_BY_OTHER};
    static claim_status_t claim_status(shd::wb_iface::sptr iface);
    static void claim(shd::wb_iface::sptr iface);
    static bool try_to_claim(shd::wb_iface::sptr iface, long timeout = 2000);
    static void release(shd::wb_iface::sptr iface);

    enum x300_mboard_t {
        SMINI_X300_MB, SMINI_X310_MB, UNKNOWN
    };
    static x300_mboard_t get_mb_type_from_pcie(const std::string& resource, const std::string& rpc_port);
    static x300_mboard_t get_mb_type_from_eeprom(const shd::smini::mboard_eeprom_t& mb_eeprom);

protected:
    void subdev_to_blockid(
            const shd::smini::subdev_spec_pair_t &spec, const size_t mb_i,
            shd::rfnoc::block_id_t &block_id, shd::device_addr_t &block_args
    );
    shd::smini::subdev_spec_pair_t blockid_to_subdev(
            const shd::rfnoc::block_id_t &blockid, const shd::device_addr_t &block_args
    );
...
private:
    ...
    struct mboard_members_t
    {
        ...
        void discover_eth(const shd::smini::mboard_eeprom_t mb_eeprom,
        ...
        shd::smini::x300::fw_regmap_t::sptr fw_regmap;
        ...
        std::vector<shd::rfnoc::x300_radio_ctrl_impl::sptr> radios;
        ...
        shd::transport::muxed_zero_copy_if::sptr ctrl_dma_xport;
        //! Async message transport
        shd::transport::muxed_zero_copy_if::sptr async_msg_dma_xport;
         ...
         */
        uint32_t allocate_pcie_dma_chan(const shd::sid_t &tx_sid, const xport_type_t xport_type);
    };
    std::vector<mboard_members_t> _mb;

    //task for periodically reclaiming the device from others
    void claimer_loop(shd::wb_iface::sptr);

    size_t _sid_framer;

    shd::sid_t allocate_sid(
        mboard_members_t &mb,
        const shd::sid_t &address,
        const uint32_t src_addr,
        const uint32_t src_dst);
    shd::both_xports_t make_transport(
        const shd::sid_t &address,
    ...
    //
    shd::dict<std::string, shd::smini::recv_packet_demuxer_3000::sptr> _demux_cache;
    //
    ...
    //
    shd::dict<std::string, shd::transport::zero_copy_if::sptr> _send_cache;
    //
    ////////////////////////////////////////////////////////////////////

    shd::dict<std::string, shd::smini::dboard_manager::sptr> _dboard_managers;
    ...
    void sync_times(mboard_members_t&, const shd::time_spec_t&);

    shd::sensor_value_t get_ref_locked(mboard_members_t& mb);
    bool wait_for_clk_locked(mboard_members_t& mb, uint32_t which, double timeout);
    bool is_pps_present(mboard_members_t& mb);

    void set_mb_eeprom(shd::i2c_iface::sptr i2c, const shd::smini::mboard_eeprom_t &);

    void check_fw_compat(const shd::fs_path &mb_path, shd::wb_iface::sptr iface);
    void check_fpga_compat(const shd::fs_path &mb_path, const mboard_members_t &members);

    /// More IO stuff
    shd::device_addr_t get_tx_hints(size_t mb_index);
    shd::device_addr_t get_rx_hints(size_t mb_index);
    shd::endianness_t get_transport_endianness(size_t mb_index) {
        return _mb[mb_index].if_pkt_is_big_endian ? shd::ENDIANNESS_BIG : shd::ENDIANNESS_LITTLE;
    };

    void post_streamer_hooks(shd::direction_t dir);
};
