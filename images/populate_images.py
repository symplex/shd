...
def get_md5_and_zipfilename():
    ...
    cmakef = open(shdimgs.get_cmake_main_file(), 'r').read()
    md5_regex = re.compile(r'SHD_IMAGES_MD5SUM\s*"(?P<md5>[0-9a-f]{32})', flags=re.MULTILINE)
    md5 = md5_regex.search(cmakef).groups('md5')[0]
    filename_regex = re.compile(r'SHD_IMAGES_DOWNLOAD_SRC\s*"(?P<filename>[^"]*\.zip)', flags=re.MULTILINE)
...
def main():
    ...
    print "== Reading MD5 and ZIP filename for current commit from {0}...".format(shdimgs.get_cmake_main_file())
    (md5, filename) = get_md5_and_zipfilename()
    print "== Starting download..."
    try:
        downloader_cmd = [
                'python',
                '../host/utils/shd_images_downloader.py.in',
                '-i', img_root_dir,
                '-f', filename,
                '-c', md5
        ]
