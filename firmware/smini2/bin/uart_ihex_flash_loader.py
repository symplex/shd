...
def main():
	...
	if(options.image == 3):
		print "Are you *really* sure you want to write to the failsafe FPGA image? If you mess this up your SMINI2+ will become a brick. Press 'y' to continue, any other key to abort."
		if(raw_input().rstrip() is not "y"):
			print "Good choice."
			raise SystemExit(0)

	elif(options.image == 1):
		print "Are you *really* sure you want to write to the failsafe firmware image? If you mess this up your SMINI2+ will only be able to be reprogrammed via the UART RAM loader.\nPress 'y' to continue, any other key to abort."
		if(raw_input().rstrip() is not "y"):
			print "Good choice."
			raise SystemExit(0)
...
#test to see if a valid SMINI2+ in flash load mode is connected
	ser.write("garbage\n")
	ser.readline()
	ser.write("!SECTORSIZE\n")
	reply = ser.readline().rstrip()
	if("NOK" in reply):
		sys.stderr.write("Error writing to SMINI2+. Try again.\n")
		raise SystemExit(1)
	elif("OK" in reply):
		sectorsize = int(reply[3:5], 16)
		print("SMINI2+ found with sector size %i. Erasing old image." % 2**sectorsize)
	else:
		sys.stderr.write("Invalid response or no SMINI2+ connected.\n")
		raise SystemExit(1)
