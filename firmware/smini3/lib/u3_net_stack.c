...
{
    SHD_FW_TRACE(DEBUG, "handle_arp_packet");
    ...
    {
        SHD_FW_TRACE(DEBUG, "ARPOP_REPLY");
    ...
    {
        SHD_FW_TRACE(DEBUG, "ARPOP_REQUEST");
...
)
{
    ...
    {
        SHD_FW_TRACE(WARN, "u3_net_stack_send_udp_pkt arp_cache_lookup fail");
        return;
    }
...
static void handle_udp_packet(
    ...
    SHD_FW_TRACE_FSTR(ERROR, "Unhandled UDP packet src=%u, dest=%u", udp->src, udp->dest);
...
static void handle_icmp_packet(
    ...
    SHD_FW_TRACE_FSTR(ERROR, "Unhandled ICMP packet type=%u", icmp->type);
}
...
)
{
    ...
    if (!resolve_ip(dst, &dst_mac_addr))
    {
        SHD_FW_TRACE(WARN, "u3_net_stack_send_echo_request arp_cache_lookup fail");
        return;
    }
...
static void handle_eth_packet(const void *buff, const size_t num_bytes)
{
    ...
    SHD_FW_TRACE_FSTR(DEBUG, "handle_eth_packet got ethertype 0x%x", (unsigned)eth_hdr->ethertype);

    if (eth_hdr->ethertype == ETHERTYPE_ARP)
    {
        SHD_FW_TRACE(DEBUG, "eth_hdr->ethertype == ETHERTYPE_ARP");
    ...
    {
        SHD_FW_TRACE(DEBUG, "eth_hdr->ethertype == ETHERTYPE_IPV4");
...
void u3_net_stack_handle_one(void)
{
    ...
    if (ptr != NULL)
    {
        SHD_FW_TRACE_FSTR(DEBUG, "u3_net_stack_handle_one got %u bytes", (unsigned)num_bytes);
