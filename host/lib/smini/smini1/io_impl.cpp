 ...
 **********************************************************************/
static void smini1_bs_vrt_packer(
    uint32_t *,
...
static void smini1_bs_vrt_unpacker(
    const uint32_t *,
    vrt::if_packet_info_t &if_packet_info
){
 ...
 **********************************************************************/
struct smini1_impl::io_impl{
    io_impl(zero_copy_if::sptr data_transport):
        data_transport(data_transport),
        curr_buff(offset_send_buffer(data_transport->get_send_buff())),
        omsb(boost::bind(&smini1_impl::io_impl::commit_send_buff, this, _1, _2, _3))
    {
        /* NOP */
    }

    ~io_impl(void){
        SHD_SAFE_CALL(flush_send_buff();)
    }
 ...
 */
void smini1_impl::io_impl::commit_send_buff(
 ...
 */
void smini1_impl::io_impl::flush_send_buff(void){
 ...
 **********************************************************************/
void smini1_impl::io_init(void){

    _io_impl = SHD_PIMPL_MAKE(io_impl, (_data_transport));
    ...
    _io_impl->vandal_task = task::make(boost::bind(
        &smini1_impl::vandal_conquest_loop, this
    ));
}

void smini1_impl::rx_stream_on_off(bool enb){
    this->restore_rx(enb);
...
void smini1_impl::tx_stream_on_off(bool enb){
 ...
 */
void smini1_impl::vandal_conquest_loop(void){
    ...
    try{ while (not boost::this_thread::interruption_requested()){
        ...
        SHD_MSG(error) << "The vandal caught an unexpected exception " << e.what() << std::endl;
    }
}
 ...
 **********************************************************************/
class smini1_recv_packet_streamer : public sph::recv_packet_handler, public rx_streamer{
public:
    smini1_recv_packet_streamer(const size_t max_num_samps, soft_time_ctrl::sptr stc){
 ...
 **********************************************************************/
class smini1_send_packet_streamer : public sph::send_packet_handler, public tx_streamer{
public:
    smini1_send_packet_streamer(const size_t max_num_samps, soft_time_ctrl::sptr stc, boost::function<void(bool)> tx_enb_fcn){
        _max_num_samps = max_num_samps;
 ...
 **********************************************************************/
void smini1_impl::update_rx_subdev_spec(const shd::smini::subdev_spec_t &spec){
...
void smini1_impl::update_tx_subdev_spec(const shd::smini::subdev_spec_t &spec){
...
void smini1_impl::update_tick_rate(const double rate){
...
shd::meta_range_t smini1_impl::get_rx_dsp_host_rates(void){
    meta_range_t range;
...
shd::meta_range_t smini1_impl::get_tx_dsp_host_rates(void){
    meta_range_t range;
...
double smini1_impl::update_rx_samp_rate(size_t dspno, const double samp_rate){

    const size_t div = this->has_rx_halfband()? 2 : 1;
    const size_t rate = boost::math::iround(_master_clock_rate/this->get_rx_dsp_host_rates().clip(samp_rate, true));

    if (rate < 8 and this->has_rx_halfband()) SHD_MSG(warning) <<
        "SMINI1 cannot achieve decimations below 8 when the half-band filter is present.\n"
        "The smini1_fpga_4rx.rbf file is a special FPGA image without RX half-band filters.\n"
        "To load this image, set the device address key/value pair: fpga=smini1_fpga_4rx.rbf\n"
    << std::endl;
    ...
    if (dspno == 0){ //only care if dsp0 is set since its homogeneous
        ...
        boost::shared_ptr<smini1_recv_packet_streamer> my_streamer =
            boost::dynamic_pointer_cast<smini1_recv_packet_streamer>(_rx_streamer.lock());
...
double smini1_impl::update_tx_samp_rate(size_t dspno, const double samp_rate){
    ...
    if (dspno == 0){ //only care if dsp0 is set since its homogeneous
        ...
        //update the streamer if created
        boost::shared_ptr<smini1_send_packet_streamer> my_streamer =
            boost::dynamic_pointer_cast<smini1_send_packet_streamer>(_tx_streamer.lock());
        if (my_streamer.get() != NULL){
            my_streamer->set_samp_rate(_master_clock_rate / rate);
        }
    }
...
void smini1_impl::update_rates(void){
...
double smini1_impl::update_rx_dsp_freq(const size_t dspno, const double freq_){
    ...
    SHD_ASSERT_THROW(std::abs(freq) <= _master_clock_rate/2.0);
...
double smini1_impl::update_tx_dsp_freq(const size_t dspno, const double freq){
 ...
 * Async Data
 **********************************************************************/
bool smini1_impl::recv_async_msg(
 ...
 * Receive streamer
 **********************************************************************/
rx_streamer::sptr smini1_impl::get_rx_stream(const shd::stream_args_t &args_){
    ...
    if (args.otw_format == "sc16"){
        ...
        throw shd::value_error("SMINI1 RX cannot handle requested wire format: " + args.otw_format);
    }
    ...
    boost::shared_ptr<smini1_recv_packet_streamer> my_streamer =
        boost::make_shared<smini1_recv_packet_streamer>(spp, _soft_time_ctrl);

    //init some streamer stuff
    my_streamer->set_tick_rate(_master_clock_rate);
    my_streamer->set_vrt_unpacker(&smini1_bs_vrt_unpacker);
    my_streamer->set_xport_chan_get_buff(0, boost::bind(
        &shd::transport::zero_copy_if::get_recv_buff, _io_impl->data_transport, _1
    ));

    //set the converter
    shd::convert::id_type id;
    id.input_format = args.otw_format + "_item16_smini1";
    id.num_inputs = 1;
 ...
 * Transmit streamer
 **********************************************************************/
tx_streamer::sptr smini1_impl::get_tx_stream(const shd::stream_args_t &args_){
    stream_args_t args = args_;
    ...
    if (args.otw_format != "sc16"){
        throw shd::value_error("SMINI1 TX cannot handle requested wire format: " + args.otw_format);
    }
    ...
    //make the new streamer given the samples per packet
    boost::function<void(bool)> tx_fcn = boost::bind(&smini1_impl::tx_stream_on_off, this, _1);
    boost::shared_ptr<smini1_send_packet_streamer> my_streamer =
        boost::make_shared<smini1_send_packet_streamer>(spp, _soft_time_ctrl, tx_fcn);

    //init some streamer stuff
    my_streamer->set_tick_rate(_master_clock_rate);
    my_streamer->set_vrt_packer(&smini1_bs_vrt_packer);
    my_streamer->set_xport_chan_get_buff(0, boost::bind(
        &smini1_impl::io_impl::get_send_buff, _io_impl.get(), _1
    ));
    ...
    id.output_format = args.otw_format + "_item16_smini1";
    id.num_outputs = 1;
    my_streamer->set_converter(id);
