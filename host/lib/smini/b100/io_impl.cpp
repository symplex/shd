...
void b100_impl::update_rx_subdev_spec(const shd::smini::subdev_spec_t &spec){
...
void b100_impl::update_tx_subdev_spec(const shd::smini::subdev_spec_t &spec){
    fs_path root = "/mboards/0/dboards";
 ...
 * Receive streamer
 **********************************************************************/
rx_streamer::sptr b100_impl::get_rx_stream(const shd::stream_args_t &args_){
 ...
 * Transmit streamer
 **********************************************************************/
tx_streamer::sptr b100_impl::get_tx_stream(const shd::stream_args_t &args_){
    stream_args_t args = args_;
    ...
    for (size_t chan_i = 0; chan_i < args.channels.size(); chan_i++){
        const size_t dsp = args.channels[chan_i];
        SHD_ASSERT_THROW(dsp == 0); //always 0
        _tx_dsp->setup(args);
        my_streamer->set_xport_chan_get_buff(chan_i, boost::bind(
