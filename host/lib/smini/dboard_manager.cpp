...
public:
    ...
    dboard_id_t xx_id(void) const{
        SHD_ASSERT_THROW(not this->is_xcvr());
        return this->_rx_id;
    }

    dboard_id_t rx_id(void) const{
        SHD_ASSERT_THROW(this->is_xcvr());
        return this->_rx_id;
    }
...
typedef shd::dict<dboard_key_t, args_t> id_to_args_map_t;
SHD_SINGLETON_FCN(id_to_args_map_t, get_id_to_args_map)
...
static void register_dboard_key(
    ...
    SHD_LOGV(always) << "registering: " << name << std::endl;
    if (get_id_to_args_map().has_key(dboard_key)){

        if (dboard_key.is_xcvr()) throw shd::key_error(str(boost::format(
        ...
        else throw shd::key_error(str(boost::format(
...
private:
    ...
    shd::dict<std::string, dboard_base::sptr> _rx_dboards;
    shd::dict<std::string, dboard_base::sptr> _tx_dboards;
...
dboard_manager_impl::~dboard_manager_impl(void){SHD_SAFE_CALL(
    set_nice_dboard_if();
)}
