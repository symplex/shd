...
#ifndef INCLUDED_SHD_DEVICE3_HPP
#define INCLUDED_SHD_DEVICE3_HPP
...
/*!
 * \brief Extends shd::device for third-generation SMINI devices.
 *
 ...
 */
class SHD_API device3 : public shd::device {

  public:
    typedef boost::shared_ptr<device3> sptr;

    //! Same as shd::device::make(), but will fail if not actually a device3
     ...
     * on this device), it will throw a shd::lookup_error.
     *
     * \param block_id Canonical block name (e.g. "0/FFT_1").
     */
     ...
     * If the given block ID is not valid (i.e. such a block does not exist
     * on this device) or if the type does not match, it will throw a shd::lookup_error.
     *
     * \code{.cpp}
     * // Assume DEV is a device3::sptr
     * shd::rfnoc::my_block_ctrl::sptr block_controller = get_block_ctrl<my_block_ctrl>("0/MyBlock_0");
    ...
    boost::shared_ptr<T> get_block_ctrl(const rfnoc::block_id_t &block_id) const
    {
        ...
        if (blk) {
            ...
            throw shd::lookup_error(str(boost::format("This device does not have a block of type %s with ID: %s")
...
#endif /* INCLUDED_SHD_DEVICE3_HPP */
// vim: sw=4 et:
