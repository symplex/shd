...
typedef std::map<std::string, shd::image_loader::loader_fcn_t> loader_fcn_map_t;
typedef std::map<std::string, std::string> string_map_t;

// Nice typedefs for iterating over std::map
typedef std::pair<std::string, shd::image_loader::loader_fcn_t> loader_fcn_pair_t;
typedef std::pair<std::string, std::string> string_pair_t;

SHD_SINGLETON_FCN(loader_fcn_map_t, get_image_loaders);
SHD_SINGLETON_FCN(string_map_t,     get_recovery_strings);

/*
 * Registration
 */
void shd::image_loader::register_image_loader(const std::string &device_type,
                                              const loader_fcn_t &loader_fcn,
                                              const std::string &recovery_instructions){
    SHD_LOGV(always) << "Registering image loader and recovery instructions for "
 ...
 * Actual loading
 */
bool shd::image_loader::load(const shd::image_loader::image_loader_args_t &image_loader_args){
    ...
    if(image_loader_args.args.has_key("type")){
        ...
        if(get_image_loaders().find(type) == get_image_loaders().end()){
            throw shd::runtime_error(str(boost::format("There is no image loader registered for given type \"%s\".")
                                         % type));
        }
 ...
 */
std::string shd::image_loader::get_recovery_instructions(const std::string &device_type){
