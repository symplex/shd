...
static void octoclock_read_bin(octoclock_session_t &session)
{
    ...
    if (not bin_file.is_open()) {
        throw shd::io_error("Could not read image file.");
    }
    ...
    if(size_t(bin_file.gcount()) != filesize) {
        throw shd::io_error("Failed to read firmware image.");
    }
...
static void octoclock_validate_firmware_image(octoclock_session_t &session){
    ...
    else throw shd::runtime_error(str(boost::format("Invalid extension \"%s\". Extension must be .hex or .bin.")
                                      % extension));
...
static void octoclock_setup_session(octoclock_session_t &session,
    ...
    size_t len = 0;
    SHD_OCTOCLOCK_SEND_AND_RECV(session.ctrl_xport, OCTOCLOCK_FW_COMPAT_NUM, OCTOCLOCK_QUERY_CMD, pkt_out, len, session.data_in);
    if(SHD_OCTOCLOCK_PACKET_MATCHES(OCTOCLOCK_QUERY_ACK, pkt_out, pkt_in, len)){
        session.starting_firmware_version = shd::htonx<uint32_t>(pkt_in->proto_ver);
    } else {
        throw shd::runtime_error("Failed to communicate with OctoClock.");
    }
}
...
static void octoclock_reset_into_bootloader(octoclock_session_t &session){
    ...
    octoclock_packet_t pkt_out;
    pkt_out.sequence = shd::htonx<uint32_t>(++session.sequence);
    pkt_out.proto_ver = shd::htonx<uint32_t>(session.starting_firmware_version);
    ...
    if(SHD_OCTOCLOCK_PACKET_MATCHES(RESET_ACK, pkt_out, pkt_in, len)){
        // Make sure this device is now in its bootloader
        boost::this_thread::sleep(boost::posix_time::milliseconds(500));
        shd::device_addrs_t octoclocks = shd::device::find(
                                             shd::device_addr_t(str(boost::format("addr=%s")
                                                                    % session.dev_addr["addr"]
                                                               )));
        if(octoclocks.size() == 0){
            std::cout << "failed." << std::endl;
            throw shd::runtime_error("Failed to reset OctoClock.");
        }
...
static void octoclock_burn(octoclock_session_t &session){
    ...
    SHD_OCTOCLOCK_SEND_AND_RECV(session.fw_xport, OCTOCLOCK_FW_COMPAT_NUM, PREPARE_FW_BURN_CMD, pkt_out, len, session.data_in);
    if(SHD_OCTOCLOCK_PACKET_MATCHES(FW_BURN_READY_ACK, pkt_out, pkt_in, len)){
        std::cout << "successful." << std::endl;
    }
    else{
        std::cout << "failed." << std::endl;
        throw shd::runtime_error("Failed to prepare OctoClock for firmware load.");
    }
    ...
    for(size_t i = 0; i < session.num_blocks; i++){
        ...
        SHD_OCTOCLOCK_SEND_AND_RECV(session.fw_xport, OCTOCLOCK_FW_COMPAT_NUM, FILE_TRANSFER_CMD, pkt_out, len, session.data_in);
        if(not SHD_OCTOCLOCK_PACKET_MATCHES(FILE_TRANSFER_ACK, pkt_out, pkt_in, len)){
            std::cout << std::endl;
            throw shd::runtime_error("Failed to load firmware.");
        }
    }
...
static void octoclock_verify(octoclock_session_t &session){
    ...
    for(size_t i = 0; i < session.num_blocks; i++){
        ...
        SHD_OCTOCLOCK_SEND_AND_RECV(session.fw_xport, OCTOCLOCK_FW_COMPAT_NUM, READ_FW_CMD, pkt_out, len, session.data_in);
...
static void octoclock_finalize(octoclock_session_t &session){
    ...
    SHD_OCTOCLOCK_SEND_AND_RECV(session.fw_xport, OCTOCLOCK_FW_COMPAT_NUM, FINALIZE_BURNING_CMD, pkt_out, len, session.data_in);
    if(SHD_OCTOCLOCK_PACKET_MATCHES(FINALIZE_BURNING_ACK, pkt_out, pkt_in, len)){
        std::cout << "successful." << std::endl;
    }
    else{
        std::cout << "failed." << std::endl;
        throw shd::runtime_error("Failed to finalize OctoClock firmware load.");
    }
}
...
SHD_STATIC_BLOCK(register_octoclock_image_loader){
