...
bool spif_spsn_s25flxx_erase_sector_dispatch(const spi_flash_dev_t* flash, uint32_t offset)
{
    ...
    if (offset % flash->sector_size) {
        SHD_FW_TRACE(ERROR, "spif_spsn_s25flxx_erase_sector: Erase offset not a multiple of sector size.");
        return false;
    }

    if (!_spif_wait_ready(flash, S25FLXX_SECTOR_ERASE_TIME_MS)) {
        SHD_FW_TRACE_FSTR(ERROR, "spif_spsn_s25flxx_erase_sector: Timeout. Sector at 0x%X was not ready for erase.", offset);
        return false;
    }
...
bool spif_spsn_s25flxx_erase_sector_commit(const spi_flash_dev_t* flash, uint32_t offset)
{
    ...
    if (!status) {
        SHD_FW_TRACE_FSTR(ERROR, "spif_spsn_s25flxx_erase_sector_commit: Timeout. Sector at 0x%X did not finish erasing in time.", offset);
    }
...
bool spif_spsn_s25flxx_write_page_dispatch(const spi_flash_dev_t* flash, uint32_t offset, const void *buf, uint32_t num_bytes)
{
    if (num_bytes == 0 || num_bytes > flash->page_size) {
        SHD_FW_TRACE(ERROR, "spif_spsn_s25flxx_write_page: Invalid size. Must be > 0 and <= Page Size.");
        return false;
    }
    if (num_bytes > (flash->sector_size * flash->num_sectors)) {
        SHD_FW_TRACE(ERROR, "spif_spsn_s25flxx_write_page: Cannot write past flash boundary.");
        return false;
    }

    //Wait until ready and enable write enabled
    if (!_spif_wait_ready(flash, S25FLXX_PAGE_WRITE_TIME_MS)) {
        SHD_FW_TRACE_FSTR(ERROR, "spif_spsn_s25flxx_write_page: Timeout. Page at 0x%X was not ready for write.", offset);
        return false;
    }
    _spi_flash_set_write_enabled(flash, true);
...
bool spif_spsn_s25flxx_write_page_commit(const spi_flash_dev_t* flash, uint32_t offset, const void *buf, uint32_t num_bytes)
{
    ...
    if (!_spif_wait_ready(flash, S25FLXX_PAGE_WRITE_TIME_MS)) {
        SHD_FW_TRACE(ERROR, "spif_spsn_s25flxx_commit_write: Timeout. Page did not finish writing in time.");
        return false;
    }
    _spi_flash_set_write_enabled(flash, false);
    return true;
