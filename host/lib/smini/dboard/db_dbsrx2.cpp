...
static const shd::dict<std::string, gain_range_t> dbsrx2_gain_ranges = map_list_of
...
private:
    ...
    void send_reg(uint8_t start_reg, uint8_t stop_reg){
        start_reg = uint8_t(shd::clip(int(start_reg), 0x0, 0xB));
        stop_reg = uint8_t(shd::clip(int(stop_reg), 0x0, 0xB));
    ...
    void read_reg(uint8_t start_reg, uint8_t stop_reg){
        ...
        start_reg = uint8_t(shd::clip(int(start_reg), 0x0, 0xD));
        stop_reg = uint8_t(shd::clip(int(stop_reg), 0x0, 0xD));
 ...
 **********************************************************************/
// FIXME 0x67 is the default i2c address on SMINI2
//       need to handle which side for SMINI1 with different address
static dboard_base::sptr make_dbsrx2(dboard_base::ctor_args_t args){
    return dboard_base::sptr(new dbsrx2(args));
}

SHD_STATIC_BLOCK(reg_dbsrx2_dboard){
