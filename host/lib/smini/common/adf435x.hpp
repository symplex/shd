...
public:
    ...
    virtual shd::range_t get_int_range() = 0;
...
public:
    ...
    shd::range_t get_int_range()
    {
        if (_N_min < 0) throw shd::runtime_error("set_prescaler must be called before get_int_range");
        return shd::range_t(_N_min, 4095);
    }
    ...
    double set_frequency(double target_freq, bool int_n_mode, bool flush = false)
    {
        ...
        shd::range_t rf_divider_range = _get_rfdiv_range();
        shd::range_t int_range = get_int_range();
        ...
        SHD_ASSERT_THROW((_regs.frac_12_bit          & ((uint16_t)~0xFFF)) == 0);
        SHD_ASSERT_THROW((_regs.mod_12_bit           & ((uint16_t)~0xFFF)) == 0);
        SHD_ASSERT_THROW((_regs.clock_divider_12_bit & ((uint16_t)~0xFFF)) == 0);
        SHD_ASSERT_THROW((_regs.r_counter_10_bit     & ((uint16_t)~0x3FF)) == 0);

        SHD_ASSERT_THROW(vco_freq >= VCO_FREQ_MIN and vco_freq <= VCO_FREQ_MAX);
        SHD_ASSERT_THROW(RFdiv >= static_cast<uint16_t>(rf_divider_range.start()));
        SHD_ASSERT_THROW(RFdiv <= static_cast<uint16_t>(rf_divider_range.stop()));
        SHD_ASSERT_THROW(_regs.int_16_bit >= static_cast<uint16_t>(int_range.start()));
        SHD_ASSERT_THROW(_regs.int_16_bit <= static_cast<uint16_t>(int_range.stop()));
...
template <>
inline shd::range_t adf435x_impl<adf4350_regs_t>::_get_rfdiv_range()
{
    return shd::range_t(1, 16);
}

template <>
inline shd::range_t adf435x_impl<adf4351_regs_t>::_get_rfdiv_range()
{
    return shd::range_t(1, 64);
}
