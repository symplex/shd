...
xge_read_sfpp_type(const uint32_t base, const uint32_t delay_ms)
{
 ...
 // Delay read of SFPP
  ...
  // I2C Error?
  if (x < 0) {
    SHD_FW_TRACE(ERROR, "I2C error in SFPP_TYPE.");
    return x;
    }
  ...
  if (x & 0x10)
    {
      SHD_FW_TRACE(DEBUG, "SFFP_TYPE_SR.");
      return SFFP_TYPE_SR;
    }
  if (x & 0x20)
    {
      SHD_FW_TRACE(DEBUG, "SFFP_TYPE_LR.");
      return SFFP_TYPE_LR;
    }
  if (x & 0x40)
    {
      SHD_FW_TRACE(DEBUG, "SFFP_TYPE_LRM.");
  ...
  if (x & 0x01) {
    SHD_FW_TRACE(DEBUG, "SFFP_TYPE_1000BASE_SX.");
    return SFFP_TYPE_1000BASE_SX;
  }
  if (x & 0x02) {
    SHD_FW_TRACE(DEBUG, "SFFP_TYPE_1000BASE_LX.");
    return SFFP_TYPE_1000BASE_LX;
  }
  if (x & 0x08) {
    SHD_FW_TRACE(DEBUG, "SFFP_TYPE_1000BASE_T.");
...
//   SHD_FW_TRACE(DEBUG, "SFP+ reg6 read as %x",x);
//  if (x < 0)
//    return x;
...
{
    SHD_FW_TRACE(DEBUG, "Begining XGE MAC init sequence.");
...
static void xge_phy_init(const uint8_t eth, const uint32_t mdio_port)
{
    ...
    SHD_FW_TRACE(DEBUG, "Begining XGE PHY init sequence.");
    // Software reset
...
void poll_sfpp_status(const uint32_t sfp)
{
    ...
    if (status & SFPP_STATUS_MODABS_CHG) {
        ...
        if (status & SFPP_STATUS_MODABS) {
            ...
            SHD_FW_TRACE_FSTR(INFO, "An SFP+ module has been removed from eth port %d.", sfp);
        } else {
            // MODDET is low, module currently inserted.
            // Return status.
            SHD_FW_TRACE_FSTR(INFO, "A new SFP+ module has been inserted into eth port %d.", sfp);
    ...
    if (status & SFPP_STATUS_RXLOS_CHG) {
        SHD_FW_TRACE_FSTR(DEBUG, "SFP%1d RXLOS changed state: %d", sfp, (status & SFPP_STATUS_RXLOS));
    }
    if (status & SFPP_STATUS_TXFAULT_CHG) {
        SHD_FW_TRACE_FSTR(DEBUG, "SFP%1d TXFAULT changed state: %d", sfp, ((status & SFPP_STATUS_TXFAULT) >> 1));
    }
    if (status & SFPP_STATUS_MODABS_CHG) {
        SHD_FW_TRACE_FSTR(DEBUG, "SFP%1d MODABS changed state: %d", sfp, ((status & SFPP_STATUS_MODABS) >> 2));
    }
    ...
    if (old_link_up != links_up[sfp]) {
        SHD_FW_TRACE_FSTR(INFO, "The link on SFP port %u is %s", sfp, links_up[sfp]?"up":"down");
    }
}
...
void ethernet_init(const uint32_t sfp)
{
    ...
    SHD_FW_TRACE_FSTR(DEBUG, "SFP%1d SFP initial state: RXLOS: %d  TXFAULT: %d  MODABS: %d",
        sfp,
        (x & SFPP_STATUS_RXLOS),
...
{
    SHD_FW_TRACE_FSTR(DEBUG,
        "[MDIO Register Dump for Addr=%x, Device=%x]\n- Raw Value = %x",
        address, device, data);
  int x;
  switch(address) {
  case XGE_MDIO_CONTROL1:
    SHD_FW_TRACE_FSTR_SHORT(DEBUG, "CONTROL1: %x = ", data);
    for (x=15; x >= 0 ; x--)
      if ((data & (1 << x)) != 0)
	// Bits set.
	switch(x) {
	case 15: SHD_FW_TRACE_SHORT(DEBUG, "Reset,"); break;
	case 14: SHD_FW_TRACE_SHORT(DEBUG, "Loopback,"); break;
	case 11: SHD_FW_TRACE_SHORT(DEBUG, "Low Power Mode,"); break;
	case 5:case 4:case 3:case 2: SHD_FW_TRACE_SHORT(DEBUG, "RESERVED speed value,"); break;
	case 0: SHD_FW_TRACE_SHORT(DEBUG, "PMA loopback,"); break;
	} //else
	// Bits clear.
	//switch (x) {
	//case 13: case 6: SHD_FW_TRACE_SHORT(DEBUG, " None 10Gb/s speed set!"); break;
	//}
    SHD_FW_TRACE_SHORT(DEBUG, " \n");
    break;
  case XGE_MDIO_STATUS1:
    SHD_FW_TRACE_FSTR_SHORT(DEBUG, "STATUS1: %x = ", data);
    for (x=15; x >= 0 ; x--)
      if ((data & (1 << x)) != 0)
	// Bits set.
	switch(x) {
	case 7: SHD_FW_TRACE_SHORT(DEBUG, "Fault Detected,"); break;
	case 2: SHD_FW_TRACE_SHORT(DEBUG, "Link is Up,"); break;
	case 1: SHD_FW_TRACE_SHORT(DEBUG, "Supports Low Power,"); break;
	} else
	// Bits Clear
	switch(x) {
	case 2: SHD_FW_TRACE_SHORT(DEBUG, "Link is Down,"); break;
	}
    SHD_FW_TRACE_SHORT(DEBUG, " \n");
    break;
  case XGE_MDIO_SPEED:
    SHD_FW_TRACE_FSTR_SHORT(DEBUG, "SPEED ABILITY: %x = ", data);
    for (x=15; x >= 0 ; x--)
      if ((data & (1 << x)) != 0)
	// Bits set.
	switch(x) {
	case 15:case 14:case 13:case 12:case 11:case 10:case 9:
	case 8:case 7:case 6:case 5:case 4:case 3:case 2:case 1: SHD_FW_TRACE_SHORT(DEBUG, "RESERVED bits set!,"); break;
	case 0: SHD_FW_TRACE_SHORT(DEBUG, "Capable of 10Gb/s,");
	} else
	// Bits clear.
	switch(x) {
	case 0: SHD_FW_TRACE_SHORT(DEBUG, "Incapable of 10Gb/s,"); break;
	}
    SHD_FW_TRACE_SHORT(DEBUG, " \n");
    break;
  case XGE_MDIO_DEVICES1:
    SHD_FW_TRACE_FSTR_SHORT(DEBUG, "DEVICES IN PACKAGE: %x = ", data);
    for (x=15; x >= 0 ; x--)
      if ((data & (1 << x)) != 0)
	// Bits set.
	switch(x) {
	case 7: SHD_FW_TRACE_SHORT(DEBUG, "Auto-Negotiation,"); break;
	case 6: SHD_FW_TRACE_SHORT(DEBUG, "TC,"); break;
 	case 5: SHD_FW_TRACE_SHORT(DEBUG, "DTE XS,"); break;
	case 4: SHD_FW_TRACE_SHORT(DEBUG, "PHY XS,"); break;
	case 3: SHD_FW_TRACE_SHORT(DEBUG, "PCS,"); break;
	case 2: SHD_FW_TRACE_SHORT(DEBUG, "WIS,"); break;
	case 1: SHD_FW_TRACE_SHORT(DEBUG, "PMD/PMA,"); break;
	case 0: SHD_FW_TRACE_SHORT(DEBUG, "Clause 22 registers,"); break;
	}
    SHD_FW_TRACE_SHORT(DEBUG, " \n");
    break;
  case XGE_MDIO_DEVICES2:
    SHD_FW_TRACE_FSTR_SHORT(DEBUG, "DEVICES IN PACKAGE (cont): %x = ", data);
    for (x=15; x >= 0 ; x--)
      if ((data & (1 << x)) != 0)
	// Bits set.
	switch(x) {
	case 15: SHD_FW_TRACE_SHORT(DEBUG, "Vendor device 2,"); break;
	case 14: SHD_FW_TRACE_SHORT(DEBUG, "Vendor device 1,"); break;
	case 13: SHD_FW_TRACE_SHORT(DEBUG, "Clause 22 extension,"); break;
	}
    SHD_FW_TRACE_SHORT(DEBUG, " \n");
    break;
  case XGE_MDIO_CONTROL2:
    SHD_FW_TRACE_FSTR_SHORT(DEBUG, "CONTROL2: %x = ", data);
    // PMA/PMD
    if (device == XGE_MDIO_DEVICE_PMA)
      switch((data & 0xf)) {
      case 0xF: SHD_FW_TRACE_SHORT(DEBUG, "10BASE-T,"); break;
      case 0xE: SHD_FW_TRACE_SHORT(DEBUG, "100BASE-TX,"); break;
      case 0xD: SHD_FW_TRACE_SHORT(DEBUG, "1000BASE-KX,"); break;
      case 0xC: SHD_FW_TRACE_SHORT(DEBUG, "1000BASE-T,"); break;
      case 0xB: SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-KR,"); break;
      case 0xA: SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-KX4,"); break;
      case 0x9: SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-T,"); break;
      case 0x8: SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-LRM,"); break;
      case 0x7: SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-SR,"); break;
      case 0x6: SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-LR,"); break;
      case 0x5: SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-ER,"); break;
      case 0x4: SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-LX4,"); break;
	//     case 0x3: SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-SW,"); break;
	//      case 0x2: SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-LW,"); break;
	//     case 0x1: SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-EW,"); break;
      case 0x0: SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-CX4,"); break;
      } else if (device == XGE_MDIO_DEVICE_PCS)
      // PCS
      switch((data & 0x3)) {
      case 0x3: SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-T PCS,"); break;
      case 0x2: SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-W PCS,"); break;
      case 0x1: SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-X PCS,"); break;
      case 0x0: SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-R PCS,"); break;
      }
    SHD_FW_TRACE_SHORT(DEBUG, " \n");
    break;
  case XGE_MDIO_STATUS2:
    SHD_FW_TRACE_FSTR_SHORT(DEBUG, "STATUS2: %x = ", data);
    for (x=15; x >= 0 ; x--)
      if ((data & (1 << x)) != 0)
	// Bits set.
	switch(x) {
	case 15: if ((data & (1 << 14)) == 0) SHD_FW_TRACE_SHORT(DEBUG, "Device responding,"); break;
	case 13: if (device == XGE_MDIO_DEVICE_PMA) SHD_FW_TRACE_SHORT(DEBUG, "Able detect a Tx fault,"); break;
	case 12: if (device == XGE_MDIO_DEVICE_PMA) SHD_FW_TRACE_SHORT(DEBUG, "Able detect an Rx fault,"); break;
	case 11: SHD_FW_TRACE_SHORT(DEBUG, "Fault on Tx path,"); break;
	case 10: SHD_FW_TRACE_SHORT(DEBUG, "Fault on Rx path,"); break;
	case 9:  if (device == XGE_MDIO_DEVICE_PMA) SHD_FW_TRACE_SHORT(DEBUG, "Extended abilities in Reg1.11,"); break;
	case 8:  if (device == XGE_MDIO_DEVICE_PMA) SHD_FW_TRACE_SHORT(DEBUG, "Able to disable TX,"); break;
	case 7:  if (device == XGE_MDIO_DEVICE_PMA) SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-SR,"); break;
	case 6:  if (device == XGE_MDIO_DEVICE_PMA) SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-LR,"); break;
	case 5:  if (device == XGE_MDIO_DEVICE_PMA) SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-ER,"); break;
	case 4:  if (device == XGE_MDIO_DEVICE_PMA) SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-LX4,"); break;
	case 3:  if (device == XGE_MDIO_DEVICE_PMA) SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-SW,"); break;
	case 2:  if (device == XGE_MDIO_DEVICE_PMA) SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-LW,"); break;
	case 1:  if (device == XGE_MDIO_DEVICE_PMA) SHD_FW_TRACE_SHORT(DEBUG, "10GBASE-EW,"); break;
	case 0:  if (device == XGE_MDIO_DEVICE_PMA) SHD_FW_TRACE_SHORT(DEBUG, "loopback,"); break;
	}
    SHD_FW_TRACE_SHORT(DEBUG, " \n");
    break;
  case XGE_MDIO_LANESTATUS:
    SHD_FW_TRACE_FSTR_SHORT(DEBUG, "LANE STATUS: %x = ", data);
    for (x=15; x >= 0 ; x--)
      if ((data & (1 << x)) != 0)
	// Bits set.
	switch(x) {
	case 12: SHD_FW_TRACE_SHORT(DEBUG, "Lanes aligned,"); break;
	case 11: SHD_FW_TRACE_SHORT(DEBUG, "Able to generate test patterns,"); break;
	case 3:  SHD_FW_TRACE_SHORT(DEBUG, "Lane 3 synced,"); break;
	case 2:  SHD_FW_TRACE_SHORT(DEBUG, "Lane 2 synced,"); break;
	case 1:  SHD_FW_TRACE_SHORT(DEBUG, "Lane 1 synced,"); break;
	case 0:  SHD_FW_TRACE_SHORT(DEBUG, "Lane 0 synced,"); break;
	} else
	// Bits clear
	switch(x) {
 	case 3:  SHD_FW_TRACE_SHORT(DEBUG, "Lane 3 not synced,"); break;
	case 2:  SHD_FW_TRACE_SHORT(DEBUG, "Lane 2 not synced,"); break;
	case 1:  SHD_FW_TRACE_SHORT(DEBUG, "Lane 1 not synced,"); break;
	case 0:  SHD_FW_TRACE_SHORT(DEBUG, "Lane 0 not synced,"); break;
	}
    SHD_FW_TRACE_SHORT(DEBUG, " \n");
    break;
  case XILINX_CORE_VERSION:
    SHD_FW_TRACE_FSTR_SHORT(DEBUG, "XILINX CORE VERSION: %x  ",data);
    SHD_FW_TRACE_FSTR_SHORT(DEBUG, "Version: %d.%d ",((data&0xf000)>>12),((data&0xf00)>>8));
    SHD_FW_TRACE_FSTR_SHORT(DEBUG, "Patch: %d ",((data&0xE)>>1));
    SHD_FW_TRACE_SHORT(DEBUG, " \n");
    if (data&0x1) SHD_FW_TRACE(WARN, "Evaluation Version of core");
    break;
  default:
    SHD_FW_TRACE_SHORT(DEBUG, "Register @ address: ");
    SHD_FW_TRACE_FSTR_SHORT(DEBUG, "%x",address);
    SHD_FW_TRACE_SHORT(DEBUG, " has value: ");
    SHD_FW_TRACE_FSTR_SHORT(DEBUG, "%x\n",data);
    break;
  }
}
