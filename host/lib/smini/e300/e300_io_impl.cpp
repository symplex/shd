...
void e300_impl::_update_subdev_spec(
        ...
        const shd::smini::subdev_spec_t &spec)
{
    //sanity checking
    if (spec.size())
    ...
    if (spec.size() >= 1)
    {
        ...
        SHD_ASSERT_THROW(spec[0].sd_name == "A" or spec[0].sd_name == "B");
    }
    if (spec.size() == 2)
    {
    ...
    if (txrx == "tx") {
        ...
        for (size_t i = 0; i < spec.size(); i++)
        {
            ...
            _radio_perifs[i].ddc->set_mux(smini::fe_connection_t(conn));
...
void e300_impl::_handle_overflow(
    ...
    boost::weak_ptr<shd::rx_streamer> streamer)
{
    boost::shared_ptr<sph::recv_packet_streamer> my_streamer =
...
static size_t get_rx_flow_control_window(size_t frame_size, size_t sw_buff_size, double fullness_factor)
{
    if (fullness_factor < 0.01 || fullness_factor > 1) {
        throw shd::value_error("recv_buff_fullness must be between 0.01 and 1 inclusive (1% to 100%)");
    }
    ...
    const size_t last_seq)
{
    ...
    pkt[packet_info.num_header_words32+RXFC_SEQ_NUM_OFFSET] = shd::htowx<uint32_t>(seq_sw);
                                 ...
                                 boost::function<double(void)> get_tick_rate)
{
    ...
    if (shd::wtohx(packet_buff[if_packet_info.num_header_words32+0]) == 0)
    {
        const size_t seq = shd::wtohx(packet_buff[if_packet_info.num_header_words32+1]);
    ...
    load_metadata_from_buff(shd::wtohx<uint32_t>,
 ...
 * Receive streamer
 **********************************************************************/
rx_streamer::sptr e300_impl::get_rx_stream(const shd::stream_args_t &args_)
{
    ...
    {
        throw shd::value_error("e300_impl::get_rx_stream only supports otw_format sc16");
    }
    ...
    for (size_t stream_i = 0; stream_i < args.channels.size(); stream_i++)
    {
        ...
        SHD_LOG << "RX Flow Control Window = " << fc_window
        ...
        my_streamer->set_overflow_handler(stream_i,
            ...
            boost::weak_ptr<shd::rx_streamer>(my_streamer))
        );
 ...
 * Transmit streamer
 **********************************************************************/
tx_streamer::sptr e300_impl::get_tx_stream(const shd::stream_args_t &args_)
{
    boost::mutex::scoped_lock lock(_stream_spawn_mutex);
    stream_args_t args = args_;
    ...
    if (not args.otw_format.empty() and args.otw_format != "sc16")
    {
        throw shd::value_error("e300_impl::get_tx_stream only supports otw_format sc16");
    }
    args.otw_format = "sc16";
    args.channels = args.channels.empty()? std::vector<size_t>(1, 0) : args.channels;
    ...
    for (size_t stream_i = 0; stream_i < args.channels.size(); stream_i++)
    {
        ...
        SHD_LOG << "TX Flow Control Window = " << fc_window
