...
static SHD_INLINE size_t S2H_BASE(const size_t base)
{
    return base + ZF_PAGE_SIZE * 0;
}

static SHD_INLINE size_t H2S_BASE(const size_t base)
{
    return base + ZF_PAGE_SIZE * 1;
}

static SHD_INLINE size_t REG_BASE(const size_t base)
{
    return base + ZF_PAGE_SIZE * 2;
}

static SHD_INLINE size_t DST_BASE(const size_t base)
{
    return base + ZF_PAGE_SIZE * 3;
}

static SHD_INLINE size_t ZF_STREAM_OFF(const size_t which)
{
    return which * 32;
}
...
#include <shd/types/time_spec.hpp> //timeout
#include <shd/utils/log.hpp>
 ...
 **********************************************************************/
SHD_INLINE void zf_poke32(const uint32_t addr, const uint32_t data)
{
    volatile uint32_t *p = reinterpret_cast<uint32_t *>(addr);
    *p = data;
}

SHD_INLINE uint32_t zf_peek32(const uint32_t addr)
{
...
struct e300_fifo_mb : managed_buffer
{
    ...
    void release(void)
    {
        SHD_ASSERT_THROW(zf_peek32(ctrl_base+ARBITER_RB_ADDR_SPACE) > 0);
        SHD_ASSERT_THROW(zf_peek32(ctrl_base+ARBITER_RB_SIZE_SPACE) > 0);
        zf_poke32(ctrl_base + ARBITER_WR_ADDR, phys_mem);
        zf_poke32(ctrl_base + ARBITER_WR_SIZE, this->size());
    }

    template <typename T>
    SHD_INLINE typename T::sptr get_new(void)
    {
...
public:
    e300_transport(
        ...
        _waiter(waiter)
    {
        //SHD_MSG(status) << boost::format("phys 0x%x") % addrs.phys << std::endl;
        //SHD_MSG(status) << boost::format("data 0x%x") % addrs.data << std::endl;
        //SHD_MSG(status) << boost::format("ctrl 0x%x") % addrs.ctrl << std::endl;

        const uint32_t sig = zf_peek32(_addrs.ctrl + ARBITER_RD_SIG);
        SHD_ASSERT_THROW((sig >> 16) == 0xACE0);
    ...
    template <typename T>
    SHD_INLINE typename T::sptr get_buff(const double timeout)
    {
        ...
        while (1)
        {
            if (zf_peek32(_addrs.ctrl + ARBITER_RB_STATUS_OCC))
            {
                ...
                SHD_ASSERT_THROW((sts >> 7) & 0x1); //assert OK
                SHD_ASSERT_THROW((sts & 0xf) == _addrs.which); //expected tag
...
public:
        ...
        _send_entries_in_use(std::vector<size_t>(H2S_NUM_STREAMS, 0))
    {
        ...
        if (_fd < 0)
        {
            throw shd::runtime_error("e300: failed to open " + dev);
        }

        //mmap the control and data regions into virtual space
        //SHD_VAR(_config.ctrl_length);
        //SHD_VAR(_config.buff_length);
        //SHD_VAR(_config.phys_addr);
        _buff = ::mmap(NULL, _config.ctrl_length + _config.buff_length, PROT_READ|PROT_WRITE, MAP_SHARED, _fd, 0);
        if (_buff == MAP_FAILED)
        {
            ::close(_fd);
            throw shd::runtime_error("e300: failed to mmap " + dev);
        }
    ...
    {
        delete _waiter;
        SHD_LOG << "cleanup: munmap" << std::endl;
        ::munmap(_buff, _config.ctrl_length + _config.buff_length);
        ::close(_fd);
    }

    shd::transport::zero_copy_if::sptr make_recv_xport(
    ...
    shd::transport::zero_copy_if::sptr make_send_xport(
        const size_t which_stream,
        const shd::transport::zero_copy_xport_params &params)
    {
...
private:
    shd::transport::zero_copy_if::sptr _make_xport(
        const size_t which_stream,
        const shd::transport::zero_copy_xport_params &params,
        const bool is_recv)
    {
        ...
        SHD_ASSERT_THROW(_recv_entries_in_use.at(which_stream) <= S2H_NUM_CMDS);
        SHD_ASSERT_THROW(_send_entries_in_use.at(which_stream) <= H2S_NUM_CMDS);
        SHD_ASSERT_THROW(_bytes_in_use <= _config.buff_length);
...
{
    throw shd::assertion_error("e300_fifo_interface::make() !E300_NATIVE");
}
