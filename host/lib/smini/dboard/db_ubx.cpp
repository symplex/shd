...
public:
    ubx_xcvr(ctor_args_t args) : xcvr_dboard_base(args)
    {
        ...
        if (_rev >= 1)
        {
            ...
            if (_iface->get_clock_rate(dboard_iface::UNIT_RX) > pfd_freq_max)
            {
                ...
                try {
                    ...
                    SHD_MSG(warning) << "Unable to set dboard clock rate - phase will vary" << std::endl;
        ...
        get_tx_subtree()->create<std::string>("power_mode/value")
            .add_coerced_subscriber(boost::bind(&shd::property<std::string>::set, &get_rx_subtree()->access<std::string>("power_mode/value"), _1))
            .set_publisher(boost::bind(&shd::property<std::string>::get, &get_rx_subtree()->access<std::string>("power_mode/value")));
        get_tx_subtree()->create<std::vector<std::string> >("xcvr_mode/options")
            .set(ubx_xcvr_modes);
        get_tx_subtree()->create<std::string>("xcvr_mode/value")
            .add_coerced_subscriber(boost::bind(&shd::property<std::string>::set, &get_rx_subtree()->access<std::string>("xcvr_mode/value"), _1))
            .set_publisher(boost::bind(&shd::property<std::string>::get, &get_rx_subtree()->access<std::string>("xcvr_mode/value")));
...
private:
    ...
    **********************************************************************/
    ...
    void sync_phase(shd::time_spec_t cmd_time, shd::direction_t dir)
    {
        // Send phase sync signal only if the command time is set
        if (cmd_time != shd::time_spec_t(0.0))
        {
            // Delay 400 microseconds to allow LOs to lock
            cmd_time += shd::time_spec_t(0.0004);
            ...
            cmd_time = shd::time_spec_t::from_ticks(ticks, tick_rate);
            ...
            SHD_ASSERT_THROW(lo1_field_info.unit == lo2_field_info.unit);
            _iface->set_atr_reg(unit, gpio_atr::ATR_REG_IDLE, value, mask);
            cmd_time += shd::time_spec_t(1/pfd_freq);
            _iface->set_command_time(cmd_time);
    ...
    double set_tx_gain(double gain)
    {
        ...
        write_gpio();
        SHD_LOGV(rarely) << boost::format("UBX TX Gain: %f dB, Code: %d, IO Bits 0x%04x") % gain % attn_code % _ubx_tx_atten_val << std::endl;
        _tx_gain = gain;
        return gain;
    }
    ...
    double set_rx_gain(double gain)
    {
        ...
        write_gpio();
        SHD_LOGV(rarely) << boost::format("UBX RX Gain: %f dB, Code: %d, IO Bits 0x%04x") % gain % attn_code % _ubx_rx_atten_val << std::endl;
        _rx_gain = gain;
        return gain;
    }
    ...
    **********************************************************************/
    double set_tx_freq(double freq)
    {
        ...
        SHD_LOGV(rarely) << boost::format("UBX TX: the requested frequency is %f MHz") % (freq/1e6) << std::endl;
        ...
        shd::time_spec_t cmd_time = _iface->get_command_time();
        if (cmd_time != shd::time_spec_t(0.0))
        {
        ...
        if (cmd_time != shd::time_spec_t(0.0) and _txlo1->can_sync())
        {
        ...
        SHD_LOGV(rarely) << boost::format("UBX TX: the actual frequency is %f MHz") % (_tx_freq/1e6) << std::endl;
    ...
    double set_rx_freq(double freq)
    {
        ...
        SHD_LOGV(rarely) << boost::format("UBX RX: the requested frequency is %f MHz") % (freq/1e6) << std::endl;
        ...
        if (cmd_time != shd::time_spec_t(0.0) and _rxlo1->can_sync())
        {
        ...
        SHD_LOGV(rarely) << boost::format("UBX RX: the actual frequency is %f MHz") % (_rx_freq/1e6) << std::endl;
...
SHD_STATIC_BLOCK(reg_ubx_dboards)
{
