...
class global_regs : boost::noncopyable, public virtual shd::wb_iface
{
public:
...
SHD_INLINE uint32_t XB_ADDR(const uint32_t addr)
{
    return global_regs::SR_CORE_XBAR + (addr << 2);
}

SHD_INLINE uint32_t DST_ADDR(const uint32_t addr)
{
