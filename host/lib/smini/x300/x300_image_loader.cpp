...
static void x300_validate_image(x300_session_t &session){
    ...
    if(session.lvbitx){
        ...
        throw shd::runtime_error(str(boost::format("Invalid extension \"%s\". Extension must be .bin, .bit, or .lvbitx.")
                                     % extension));
    }
}
...
static void x300_setup_session(x300_session_t &session,
    ...
    if(filepath == ""){
        if(!session.dev_addr.has_key("product") or session.fpga_type == ""){
            throw shd::runtime_error("Found a device but could not auto-generate an image filename.");
        }
        else session.filepath = find_image_path(str(boost::format("smini_%s_fpga_%s.bit")
 ...
 */
static SHD_INLINE size_t x300_send_and_recv(udp_simple::sptr xport,
                                            uint32_t pkt_code,
                                            x300_fpga_update_data_t *pkt_out,
                                            uint8_t* data){
    pkt_out->flags = shd::htonx<uint32_t>(pkt_code);
    xport->send(boost::asio::buffer(pkt_out, sizeof(*pkt_out)));
    return xport->recv(boost::asio::buffer(data, udp_simple::mtu), UDP_TIMEOUT);
}

static SHD_INLINE bool x300_recv_ok(const x300_fpga_update_data_t *pkt_in,
                                    size_t len){
    return (len > 0 and
...
static SHD_INLINE void x300_bitswap(uint8_t *num){
...
static void x300_ethernet_load(x300_session_t &session){
    ...
    if(x300_recv_ok(pkt_in, len)){
        ...
        std::cout << "failed." << std::endl;
        throw shd::runtime_error("Device reported an error during initialization.");
    }
    ...
    for(size_t i = 0; i < session.size; i += X300_FLASH_SECTOR_SIZE){
        ...
        for(size_t j = i; (j < session.size and j < (i+X300_FLASH_SECTOR_SIZE)); j += X300_PACKET_SIZE_BYTES){
            ...
            if(len == 0){
                ...
                if(!session.lvbitx) image.close();
                throw shd::runtime_error("Device reported an error.");
            }
        }
    }
    if(!session.lvbitx){
    ...
    if(len == 0){
        ...
        std::cout << "failed." << std::endl;
        throw shd::runtime_error("Device reported an error during cleanup.");
    }
    ...
    if(session.configure){
        ...
        if(len == 0){
            ...
            std::cout << "failed." << std::endl;
            throw shd::runtime_error("Device reported an error while saving the image.");
        }
        else std::cout << "successful." << std::endl;
    }
    std::cout << str(boost::format("Power-cycle the SMINI %s to use the new image.") % session.dev_addr.get("product", "")) << std::endl;
}
...
static void x300_pcie_load(x300_session_t &session){
    ...
    nisminirio::nisminirio_session fpga_session(session.resource, session.rpc_port);
    nirio_status_chain(fpga_session.download_bitstream_to_flash(session.filepath), status);

    if(nirio_status_fatal(status)){
        std::cout << "failed." << std::endl;
        nisminirio::nirio_status_to_exception(status, "NI-RIO reported the following error:");
    }
    else std::cout << "successful." << std::endl;
    std::cout << str(boost::format("Power-cycle the SMINI %s to use the new image.") % session.dev_addr.get("product", "")) << std::endl;
}
...
SHD_STATIC_BLOCK(register_x300_image_loader){
    std::string recovery_instructions = "Aborting. Your SMINI X-Series device will likely be unusable. Visit\n"
                                        "http://files.ettus.com/manual/page_smini_x3x0.html#x3x0_load_fpga_imgs_jtag\n"
