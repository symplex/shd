...
#ifndef INCLUDED_LIBSHD_SMINI_COMMON_RECV_PACKET_DEMUXER_3000_HPP
#define INCLUDED_LIBSHD_SMINI_COMMON_RECV_PACKET_DEMUXER_3000_HPP
    ...
    struct recv_packet_demuxer_3000 : boost::enable_shared_from_this<recv_packet_demuxer_3000>
    {
        ...
        transport::managed_recv_buffer::sptr _internal_get_recv_buff(const uint32_t sid, const double timeout)
        {
...
#endif // RECV_PACKET_DEMUXER_3000_THREAD_SAFE
            {
                ...
                if (buff)
                {
                    const uint32_t new_sid = shd::wtohx(buff->cast<const uint32_t *>()[1]);
                    if (new_sid != sid)
                    {
                        boost::mutex::scoped_lock l(mutex);
                        if (_queues.count(new_sid) == 0) SHD_MSG(error)
...
#endif /* INCLUDED_LIBSHD_SMINI_COMMON_RECV_PACKET_DEMUXER_3000_HPP */
