...
class e100_ctrl : public shd::transport::zero_copy_if{
public:
    ...
    static shd::i2c_iface::sptr make_dev_i2c_iface(const std::string &node);

    //! Make a spi iface for the spi gpio
    static shd::spi_iface::sptr make_aux_spi_iface(void);

    //! Make a uart iface for the uart device node
    static shd::uart_iface::sptr make_gps_uart_iface(const std::string &node);
