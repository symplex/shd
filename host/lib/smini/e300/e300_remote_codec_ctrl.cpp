...
{
public:
    e300_remote_codec_ctrl_impl(shd::transport::zero_copy_if::sptr xport) : _xport(xport)
    {
    }
    ...
    void set_timed_spi(shd::spi_iface::sptr, uint32_t ) {};
    void set_safe_spi(shd::spi_iface::sptr, uint32_t ) {};

    double set_gain(const std::string &which, const double value)
    {
        _clear();
        _args.action = shd::htonx<uint32_t>(transaction_t::ACTION_SET_GAIN);
        if (which == "TX1")      _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_TX1);
        else if (which == "TX2") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_TX2);
        else if (which == "RX1") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_RX1);
        else if (which == "RX2") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_RX2);
    ...
    {
        _clear();
        _args.action = shd::htonx<uint32_t>(
            transaction_t::ACTION_SET_CLOCK_RATE);
        _args.which = shd::htonx<uint32_t>(
    ...
    {
        _clear();
        _args.action = shd::htonx<uint32_t>(transaction_t::ACTION_TUNE);
        if (which == "TX1")      _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_TX1);
        else if (which == "TX2") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_TX2);
        else if (which == "RX1") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_RX1);
        else if (which == "RX2") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_RX2);
    ...
    {
        _clear();
        _args.action = shd::htonx<uint32_t>(transaction_t::ACTION_GET_FREQ);
        if (which == "TX1")      _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_TX1);
        else if (which == "TX2") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_TX2);
        else if (which == "RX1") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_RX1);
        else if (which == "RX2") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_RX2);
    ...
    {
        _clear();
        _args.action = shd::htonx<uint32_t>(transaction_t::ACTION_SET_LOOPBACK);
        _args.which  = shd::htonx<uint32_t>(transaction_t::CHAIN_NONE);  /*Unused*/
        _args.bits = shd::htonx<uint32_t>(on ? 1 : 0);
    ...
    {
        _clear();
        _args.action = shd::htonx<uint32_t>(transaction_t::ACTION_GET_RSSI);
        if (which == "RX1") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_RX1);
        else if (which == "RX2") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_RX2);
        else throw std::runtime_error("e300_remote_codec_ctrl_impl incorrect chain string.");
        _args.bits = shd::htonx<uint32_t>(0);
    ...
    {
        _clear();
        _args.action = shd::htonx<uint32_t>(transaction_t::ACTION_GET_TEMPERATURE);
        _args.which  = shd::htonx<uint32_t>(transaction_t::CHAIN_NONE);  /*Unused*/
        _args.bits = shd::htonx<uint32_t>(0);
    ...
    {
        _clear();
        _args.action = shd::htonx<uint32_t>(transaction_t::ACTION_SET_DC_OFFSET_AUTO);
        if (which == "TX1")      _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_TX1);
        else if (which == "TX2") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_TX2);
        else if (which == "RX1") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_RX1);
        else if (which == "RX2") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_RX2);
    ...
    {
        _clear();
        _args.action = shd::htonx<uint32_t>(transaction_t::ACTION_SET_IQ_BALANCE_AUTO);
        if (which == "TX1")     _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_TX1);
        else if (which == "TX2") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_TX2);
        else if (which == "RX1") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_RX1);
        else if (which == "RX2") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_RX2);
    ...
    {
        _clear();
       _args.action = shd::htonx<uint32_t>(transaction_t::ACTION_SET_AGC);
       if (which == "TX1")      _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_TX1);
       else if (which == "TX2") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_TX2);
       else if (which == "RX1") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_RX1);
       else if (which == "RX2") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_RX2);
    ...
    {
        _clear();
       _args.action = shd::htonx<uint32_t>(transaction_t::ACTION_SET_AGC_MODE);
    ...
    {
        _clear();
        _args.action = shd::htonx<uint32_t>(transaction_t::ACTION_SET_BW);
        if (which == "TX1")      _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_TX1);
        else if (which == "TX2") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_TX2);
        else if (which == "RX1") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_RX1);
        else if (which == "RX2") _args.which = shd::htonx<uint32_t>(transaction_t::CHAIN_RX2);
    ...
    {
        SHD_MSG(warning) << "Attempting to set filter on E300 in network mode." << std::endl;
    }

    void output_digital_test_tone(SHD_UNUSED(bool enb))
    {
        SHD_THROW_INVALID_CODE_PATH();
    }
...
ad9361_ctrl::sptr e300_remote_codec_ctrl::make(shd::transport::zero_copy_if::sptr xport)
{
