...
#define SMINI2_FW_PROTO_VERSION 7 //should be unused after r6
...
shd::dict<uint32_t, std::string> filename_map = boost::assign::map_list_of
    (0,      "N2XX")
    (0xa,    "n200_r3")
    (0x100a, "n200_r4")
    (0x10a,  "n210_r3")
...
uint8_t smini2_update_data_in_mem[udp_simple::mtu];
...
void sig_int_handler(int){
    ...
    if(num_ctrl_c == 1){
        std::cout << std::endl << "Are you sure you want to abort the image burning? If you do, your "
                                  "SMINI-N Series unit will be bricked!" << std::endl
                               << "Press Ctrl+C again to abort the image burning procedure." << std::endl << std::endl;
    }
    else{
        std::cout << std::endl << "Aborting. Your SMINI-N Series unit will be bricked." << std::endl
                  << "Refer to http://files.ettus.com/manual/page_smini2.html#smini2_loadflash_brick" << std::endl
...
/***********************************************************************
 * List all connected SMINI N2XX devices
 **********************************************************************/
void list_sminis(){
    udp_simple::sptr udp_bc_transport;
    const smini2_fw_update_data_t *update_data_in = reinterpret_cast<const smini2_fw_update_data_t *>(smini2_update_data_in_mem);
    uint32_t hw_rev = 0;

    smini2_fw_update_data_t smini2_ack_pkt = smini2_fw_update_data_t();
    smini2_ack_pkt.proto_ver = htonx<uint32_t>(SMINI2_FW_PROTO_VERSION);
    smini2_ack_pkt.id = htonx<uint32_t>(SMINI2_QUERY);

    std::cout << "Available SMINI N2XX devices:" << std::endl;
    ...
    BOOST_FOREACH(const if_addrs_t &if_addrs, get_if_addrs()){
        ...
        udp_bc_transport = udp_simple::make_broadcast(if_addrs.bcast, BOOST_STRINGIZE(SMINI2_UDP_UPDATE_PORT));
        udp_bc_transport->send(boost::asio::buffer(&smini2_ack_pkt, sizeof(smini2_ack_pkt)));

        size_t len = udp_bc_transport->recv(boost::asio::buffer(smini2_update_data_in_mem), UDP_TIMEOUT);
        if(len > offsetof(smini2_fw_update_data_t, data) and ntohl(update_data_in->id) == SMINI2_ACK){
            smini2_ack_pkt.id = htonx<uint32_t>(GET_HW_REV_CMD);
            udp_bc_transport->send(boost::asio::buffer(&smini2_ack_pkt, sizeof(smini2_ack_pkt)));

            size_t len = udp_bc_transport->recv(boost::asio::buffer(smini2_update_data_in_mem), UDP_TIMEOUT);
            if(len > offsetof(smini2_fw_update_data_t, data) and ntohl(update_data_in->id) == GET_HW_REV_ACK){
...
/***********************************************************************
 * Find SMINI N2XX with specified IP address and return type
 **********************************************************************/
void print_image_loader_warning(const std::string &fw_path,
    ...
    std::string shd_image_loader = str(boost::format("shd_image_loader --args=\"type=smini2,addr=%s")
                                       % vm["addr"].as<std::string>());
    if(vm.count("auto-reboot") > 0)
        shd_image_loader += ",reset";
    if(vm.count("overwrite-safe") > 0)
        shd_image_loader += ",overwrite-safe";
    if(vm.count("dont-check-rev") > 0)
        shd_image_loader += ",dont-check-rev";

    shd_image_loader += "\"";

    if(vm.count("no-fw") == 0){
        shd_image_loader += str(boost::format("%s--fw-path=\"%s\"")
                                % nl % fw_path);
    }
    else{
        shd_image_loader += str(boost::format("%s--no-fw")
                                % nl);
    }

    if(vm.count("no-fpga") == 0){
        shd_image_loader += str(boost::format("%s--fpga-path=\"%s\"")
                                % nl % fpga_path);
    }
    else{
        shd_image_loader += str(boost::format("%s--no-fpga")
                                % nl);
    }

    std::cout << "************************************************************************************************" << std::endl
              << "WARNING: This utility will be removed in an upcoming version of SHD. In the future, use" << std::endl
              << "         this command:" << std::endl
              << std::endl
              << shd_image_loader << std::endl
              << std::endl
              << "************************************************************************************************" << std::endl
              << std::endl;
}
 ...
 * Find SMINI N2XX with specified IP address and return type
 **********************************************************************/
uint32_t find_smini(udp_simple::sptr udp_transport, bool check_rev){
    uint32_t hw_rev;
    bool found_it = false;

    // If the user chooses to not care about the rev, simply check
    // for the presence of a SMINI N2XX.
    ...
    const smini2_fw_update_data_t *update_data_in = reinterpret_cast<const smini2_fw_update_data_t *>(smini2_update_data_in_mem);
    smini2_fw_update_data_t hw_info_pkt = smini2_fw_update_data_t();
    hw_info_pkt.proto_ver = htonx<uint32_t>(SMINI2_FW_PROTO_VERSION);
    ...
    size_t len = udp_transport->recv(boost::asio::buffer(smini2_update_data_in_mem), UDP_TIMEOUT);
    if(len > offsetof(smini2_fw_update_data_t, data) and ntohl(update_data_in->id) == ack_id){
        hw_rev = ntohl(update_data_in->data.hw_rev);
        if(filename_map.has_key(hw_rev)){
            ...
            if(check_rev) throw std::runtime_error("Invalid revision found.");
                ...
                hw_rev = 0;
                std::cout << "Found SMINI N2XX." << std::endl;
                found_it = true;
            }
        }
    }
    if(not found_it) throw std::runtime_error("No SMINI N2XX found.");
...
void validate_custom_fpga_file(std::string rev_str, std::string& fpga_path, bool check_rev){
    ...
    shd::fs_path custom_fpga_path(fpga_path);
...
void validate_custom_fw_file(std::string rev_str, std::string& fw_path, bool check_rev){
    ...
    //If user cares about revision, use revision string to detect invalid image filename
    shd::fs_path custom_fw_path(fw_path);
...
uint32_t* get_flash_info(std::string& ip_addr){
    ...
    udp_simple::sptr udp_transport = udp_simple::make_connected(ip_addr, BOOST_STRINGIZE(SMINI2_UDP_UPDATE_PORT));
    smini2_fw_update_data_t get_flash_info_pkt = smini2_fw_update_data_t();
    get_flash_info_pkt.proto_ver = htonx<uint32_t>(SMINI2_FW_PROTO_VERSION);
    ...
    //Loop and receive until the timeout
    size_t len = udp_transport->recv(boost::asio::buffer(smini2_update_data_in_mem), UDP_TIMEOUT);
    if(len > offsetof(smini2_fw_update_data_t, data) and ntohl(update_data_in->id) == GET_FLASH_INFO_ACK){
...
void erase_image(udp_simple::sptr udp_transport, bool is_fw, uint32_t memory_size, bool overwrite_safe){
    ...
    smini2_fw_update_data_t erase_pkt = smini2_fw_update_data_t();
    erase_pkt.id = htonx<uint32_t>(ERASE_FLASH_CMD);
    erase_pkt.proto_ver = htonx<uint32_t>(SMINI2_FW_PROTO_VERSION);
    ...
    size_t len = udp_transport->recv(boost::asio::buffer(smini2_update_data_in_mem), UDP_TIMEOUT);
    if(len > offsetof(smini2_fw_update_data_t, data) and ntohl(update_data_in->id) == ERASE_FLASH_ACK){
    ...
    while(true){
        udp_transport->send(boost::asio::buffer(&erase_pkt, sizeof(erase_pkt)));
        size_t len = udp_transport->recv(boost::asio::buffer(smini2_update_data_in_mem), UDP_TIMEOUT);
        if(len > offsetof(smini2_fw_update_data_t, data) and ntohl(update_data_in->id) == DONE_ERASING_ACK){
...
void write_image(udp_simple::sptr udp_transport, bool is_fw, uint8_t* image,
    ...
    //Setting up UDP packet
    smini2_fw_update_data_t write_pkt = smini2_fw_update_data_t();
    write_pkt.id = htonx<uint32_t>(WRITE_FLASH_CMD);
    write_pkt.proto_ver = htonx<uint32_t>(SMINI2_FW_PROTO_VERSION);
    ...
    for(int i = 0; i < ((image_size/FLASH_DATA_PACKET_SIZE)+1); i++){
        ...
        size_t len = udp_transport->recv(boost::asio::buffer(smini2_update_data_in_mem), UDP_TIMEOUT);
        if(len > offsetof(smini2_fw_update_data_t, data) and ntohl(update_data_in->id) != WRITE_FLASH_ACK){
...
void verify_image(udp_simple::sptr udp_transport, bool is_fw, uint8_t* image,
    ...
    uint8_t from_smini[FPGA_IMAGE_SIZE_BYTES];
    ...
    //Setting up UDP packet
    smini2_fw_update_data_t verify_pkt = smini2_fw_update_data_t();
    verify_pkt.id = htonx<uint32_t>(READ_FLASH_CMD);
    verify_pkt.proto_ver = htonx<uint32_t>(SMINI2_FW_PROTO_VERSION);
    ...
    for(int i = 0; i < ((image_size/FLASH_DATA_PACKET_SIZE)+1); i++){
        ...
        size_t len = udp_transport->recv(boost::asio::buffer(smini2_update_data_in_mem), UDP_TIMEOUT);
        if(len > offsetof(smini2_fw_update_data_t, data) and ntohl(update_data_in->id) != READ_FLASH_ACK){
            throw std::runtime_error(str(boost::format("Invalid reply %d from device.")
                                         % ntohl(update_data_in->id)));
        }
        for(int j = 0; j < FLASH_DATA_PACKET_SIZE; j++) from_smini[current_index+j] = update_data_in->data.flash_args.data[j];

        current_addr += FLASH_DATA_PACKET_SIZE;
        current_index += FLASH_DATA_PACKET_SIZE;
    }
    for(int i = 0; i < image_size; i++) if(from_smini[i] != image[i]) throw std::runtime_error("Image write failed.");

    std::cout << std::flush << "\rVerifying " << type << " image (100%)." << std::endl;
    std::cout << " * Successful." << std::endl;
}

void reset_smini(udp_simple::sptr udp_transport){

    //Set up UDP transport
    const smini2_fw_update_data_t *update_data_in = reinterpret_cast<const smini2_fw_update_data_t *>(smini2_update_data_in_mem);

    //Set up UDP packet
    smini2_fw_update_data_t reset_pkt = smini2_fw_update_data_t();
    reset_pkt.id = htonx<uint32_t>(RESET_SMINI_CMD);
    reset_pkt.proto_ver = htonx<uint32_t>(SMINI2_FW_PROTO_VERSION);

    //Reset SMINI
    udp_transport->send(boost::asio::buffer(&reset_pkt, sizeof(reset_pkt)));
    size_t len = udp_transport->recv(boost::asio::buffer(smini2_update_data_in_mem), UDP_TIMEOUT);
    if(len > offsetof(smini2_fw_update_data_t, data) and ntohl(update_data_in->id) == RESET_SMINI_ACK){
        throw std::runtime_error("SMINI reset failed."); //There should be no response to this UDP packet
    }
    else std::cout << std::endl << "Resetting SMINI." << std::endl;
}
...
int SHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    desc.add_options()
        ...
        ("list", "List available N2XX SMINI devices.")
    ;
    po::variables_map vm;
    ...
    if(vm.count("help") > 0){
        ...
        std::cout << boost::format("Automatically detects and burns standard firmware and FPGA images onto SMINI N2XX devices.\n");
    ...
    //Find SMINI and establish connection
    std::cout << boost::format("Searching for SMINI N2XX with IP address %s.\n") % ip_addr;
    udp_simple::sptr udp_transport = udp_simple::make_connected(ip_addr, BOOST_STRINGIZE(SMINI2_UDP_UPDATE_PORT));
    uint32_t hw_rev = find_smini(udp_transport, check_rev);
    ...
    if(burn_fpga){
        if(use_custom_fpga){
            ...
            std::string default_fpga_filename = str(boost::format("smini_%s_fpga.bin") % filename_map[hw_rev]);
    ...
    if(burn_fw){
        if(use_custom_fw){
            ...
            std::string default_fw_filename = str(boost::format("smini_%s_fw.bin") % erase_tail_copy(filename_map[hw_rev],3));
    ...
    if(auto_reboot) reset = true;
        ...
        while(user_response != "y" and user_response != "" and user_response != "n"){
            std::cout << std::endl << "Image burning successful. Reset SMINI (Y/n)? ";
    ...
    if(reset) reset_smini(udp_transport);
