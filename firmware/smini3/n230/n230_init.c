...
void n230_init(void)
{
    ...
    SHD_FW_TRACE_FSTR(INFO, "[ZPU Init Begin -- CPU CLOCK is %d MHz]", (CPU_CLOCK_FREQ/1000000));

    //Initialize cron and the per millisecond cron job
    SHD_FW_TRACE(INFO, "Initializing cron...");
    ...
    SHD_FW_TRACE(INFO, "Initializing I2C...");
    ...
    SHD_FW_TRACE(INFO, "Initializing network stack...");
    init_network_stack();
}
...
{
    SHD_FW_TRACE_FSTR(INFO, "Initializing eth%d...", iface_num);
    ...
    SHD_FW_TRACE_FSTR(INFO, "-- MAC%u:     %s", iface_num, mac_addr_to_str(u3_net_stack_get_mac_addr(iface_num)));
    SHD_FW_TRACE_FSTR(INFO, "-- IP%u:      %s", iface_num, ip_addr_to_str(u3_net_stack_get_ip_addr(iface_num)));
    SHD_FW_TRACE_FSTR(INFO, "-- SUBNET%u:  %s", iface_num, ip_addr_to_str(u3_net_stack_get_subnet(iface_num)));
    SHD_FW_TRACE_FSTR(INFO, "-- BCAST%u:   %s", iface_num, ip_addr_to_str(u3_net_stack_get_bcast(iface_num)));
}
