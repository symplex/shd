        ...
        product   == product_id)                    //Verify device
    {
        ...
        switch (request->flags & FW_COMM_FLAGS_CMD_MASK) {
            case FW_COMM_CMD_ECHO: {
                SHD_FW_TRACE(DEBUG, "fw_comm_protocol::echo()");
                response->data_words = 1;
                response->data[0] = iface_id;
            } break;

            case FW_COMM_CMD_POKE32: {
                SHD_FW_TRACE_FSTR(DEBUG, "fw_comm_protocol::poke32(0x%x)=0x%x",
            ...
            case FW_COMM_CMD_PEEK32: {
                *(response->data) = peek_callback(request->addr);
                SHD_FW_TRACE_FSTR(DEBUG, "fw_comm_protocol::peek32(0x%x)=0x%x",
            ...
            case FW_COMM_CMD_BLOCK_POKE32: {
                ...
                SHD_FW_TRACE_FSTR(DEBUG, "fw_comm_protocol::block_poke32(0x%x,%d)",request->addr,response->data_words);
            ...
            case FW_COMM_CMD_BLOCK_PEEK32: {
                ...
                SHD_FW_TRACE_FSTR(DEBUG, "fw_comm_protocol::block_peek32(0x%x,%d)",request->addr,response->data_words);
            } break;

            default: {
                SHD_FW_TRACE(ERROR, "fw_comm_protocol got an invalid command.");
        ...
        SHD_FW_TRACE(WARN, "fw_comm_protocol ignored an unknown request.");
        send_response = false;
    }
