...
shd::time_spec_t x300_dboard_iface::get_command_time()
{
    return _config.cmd_time_ctrl->get_time();
}

void x300_dboard_iface::set_command_time(const shd::time_spec_t& t)
{
    ...
    const fe_connection_t& fe_conn)
{
    if (unit == UNIT_RX) {
        if (_rx_fes.has_key(fe_name)) {
            ...
            throw shd::assertion_error("front-end name was not registered: " + fe_name);
        }
    } else {
        throw shd::not_implemented_error("frontend connection not configurable for TX");
    }
}
