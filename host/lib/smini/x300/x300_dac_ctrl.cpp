...
{
public:
    x300_dac_ctrl_impl(shd::spi_iface::sptr iface, const size_t slaveno, const double refclk):
    ...
    void _init()
    {
        ...
        SHD_ASSERT_THROW((_refclk * N0 * N1) >= 1e9);
        SHD_ASSERT_THROW((_refclk * N0 * N1) <= 2e9);
    ...
    void _check_pll()
    {
        ...
        while (true)
        {
            ...
            if (exit_time < time_spec_t::get_system_time())
                throw shd::runtime_error("x300_dac_ctrl: timeout waiting for DAC PLL to lock");
    ...
    void _check_dac_sync()
    {
        ...
        while (true)
        {
            ...
            if ((((reg_12 >> 6) & 0x3) == 0x1) && (((reg_6 >> 4) & 0x3) == 0x1))
                break;
            if (exit_time < time_spec_t::get_system_time())
                throw shd::runtime_error("x300_dac_ctrl: timeout waiting for backend synchronization");
            if (reg_6 & (1 << 5))
                write_ad9146_reg(0x06, 0x30);   // Clear Sync event flags
 ...
 **********************************************************************/
x300_dac_ctrl::sptr x300_dac_ctrl::make(shd::spi_iface::sptr iface, const size_t slaveno, const double clock_rate)
{
