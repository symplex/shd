...
static const shd::dict<std::string, gain_range_t> rfx_rx_gain_ranges = map_list_of
    ("PGA0", gain_range_t(0, 70, 0.022))
;

static const shd::dict<std::string, gain_range_t> rfx400_rx_gain_ranges = map_list_of
...
private:
    ...
    const shd::dict<std::string, gain_range_t> _rx_gain_ranges;
    const shd::dict<dboard_iface::unit_t, bool> _div2;
    std::string  _rx_ant;
    shd::dict<std::string, double> _rx_gains;
    uint16_t _power_up;
...
SHD_STATIC_BLOCK(reg_rfx_dboards){
...
static double rx_pga0_gain_to_dac_volts(double &gain, double range){
    ...
    double dac_volts = shd::clip<double>(gain*slope + min_volts, max_volts, min_volts);
...
double rfx_xcvr::set_lo_freq(
    ...
    static const shd::dict<int, adf4360_regs_t::prescaler_value_t> prescaler_to_enum = map_list_of
    ...
    static const shd::dict<int, adf4360_regs_t::band_select_clock_div_t> bandsel_to_enum = map_list_of
