...
#include "../lib/smini/e300/e300_network.hpp"
...
int main(int argc, char *argv[])
{
    ...
    if (vm.count("help")){
        std::cout << boost::format("SHD E3x0 Network Mode %s") % desc << std::endl;
        return EXIT_FAILURE;
    }
    shd::device_addr_t args;
    if(vm.count("fpga")) {
    ...
    try {
        check_network_ok();
        shd::smini::e300::network_server::sptr server = shd::smini::e300::network_server::make(args);
        server->run();
    } catch (shd::assertion_error &e) {
        SHD_MSG(error) << "This executable is supposed to run on the device, not on the host." << std::endl
                       << "Please refer to the manual section on operating your e3x0 device in network mode." << std::endl;
        return EXIT_FAILURE;
    } catch (shd::runtime_error &e) {
