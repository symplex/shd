...
static const shd::dict<std::string, gain_range_t> dbsrx_gain_ranges = map_list_of
    ("GC1", gain_range_t(0, 56, 0.5))
    ("GC2", gain_range_t(0, 24, 1))
;

static const double smini1_gpio_clock_rate_limit = 4e6;
...
private:
    ...
    void send_reg(uint8_t start_reg, uint8_t stop_reg){
        start_reg = uint8_t(shd::clip(int(start_reg), 0x0, 0x5));
        stop_reg = uint8_t(shd::clip(int(stop_reg), 0x0, 0x5));
    ...
    void read_reg(uint8_t start_reg, uint8_t stop_reg){
        ...
        start_reg = uint8_t(shd::clip(int(start_reg), 0x0, 0x1));
        stop_reg = uint8_t(shd::clip(int(stop_reg), 0x0, 0x1));
...
dbsrx::dbsrx(ctor_args_t args) : rx_dboard_base(args){
    //warn user about incorrect DBID on SMINI1, requires R193 populated
    ...
    //warn user about incorrect DBID on non-SMINI1, requires R194 populated
    ...
    if (this->get_iface()->get_special_props().soft_clock_divider){
        this->get_iface()->set_gpio_ddr(dboard_iface::UNIT_RX, 0x1); // GPIO0 is clock when on SMINI1
    }
    else{
...
double dbsrx::set_lo_freq(double target_freq){
    ...
    const double max_clock_rate = shd::sorted(clock_rates).back();
    BOOST_FOREACH(ref_clock, shd::reversed(shd::sorted(clock_rates))){
        //SMINI1 feeds the DBSRX clock from a FPGA GPIO line.
        //make sure that this clock does not exceed rate limit.
        if (this->get_iface()->get_special_props().soft_clock_divider){
            if (ref_clock > smini1_gpio_clock_rate_limit) continue;
        }
        ...
        for(r = 0; r <= 6; r += 1) {
            ...
            R = 1 << (r+1);
            SHD_LOGV(often) << boost::format("DBSRX R:%d\n") % R << std::endl;
    ...
    SHD_ASSERT_THROW(ref_clock <= 27.0e6 and ref_clock >= 0.0);
    SHD_ASSERT_THROW(m and ref_clock/m >= 1e6 and ref_clock/m <= 2.5e6);
    SHD_ASSERT_THROW((pfd_freq >= dbsrx_pfd_freq_range.start()) and (pfd_freq <= dbsrx_pfd_freq_range.stop()));
    SHD_ASSERT_THROW((N >= 256) and (N <= 32768));
    ...
    while ((_max2118_read_regs.adc == 0) or (_max2118_read_regs.adc == 7)){
        ...
        if (_max2118_read_regs.adc == 0){
            if (_max2118_write_regs.osc_band == 0){
                ...
                SHD_ASSERT_THROW(_max2118_read_regs.adc != 0); //just to cause a throw
            }
        ...
        if (_max2118_read_regs.adc == 7){
            if (_max2118_write_regs.osc_band == 7){
                ...
                SHD_ASSERT_THROW(_max2118_read_regs.adc != 7); //just to cause a throw
            }
...
double dbsrx::set_bandwidth(double bandwidth){
    ...
    _max2118_write_regs.f_dac = shd::clip<int>(int((((bandwidth*_max2118_write_regs.m_divider)/ref_clock) - 4)/0.145),0,127);
