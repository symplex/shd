...
#test to see if a valid SMINI2+ in RAM load mode is connected

	ser.write("WOOOOO\n");
	reply = ser.readline()
	if("NOK" not in reply):
		sys.stderr.write("Valid SMINI2+ not connected or no response received\n")
		raise SystemExit(1)
	else:
		print("SMINI2+ found.")
	...
	for line in f:
		...
		if("NOK" in reply): #blocks to wait for response
			print("Received NOK reply from SMINI2+")
			raise SystemExit(1)
