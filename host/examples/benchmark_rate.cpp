...
void benchmark_rx_rate(
    ...
    ) % (smini->get_rx_rate()/1e6) % rx_stream->get_num_channels() << std::endl;

    //setup variables and allocate buffer
    shd::rx_metadata_t md;
    const size_t max_samps_per_packet = rx_stream->get_max_num_samps();
    std::vector<char> buff(max_samps_per_packet*shd::convert::get_bytes_per_item(rx_cpu));
    ...
    shd::stream_cmd_t cmd(shd::stream_cmd_t::STREAM_MODE_START_CONTINUOUS);
    cmd.time_spec = smini->get_time_now() + shd::time_spec_t(INIT_DELAY);
    ...
    while (true) {
        ...
        case shd::rx_metadata_t::ERROR_CODE_LATE_COMMAND:
            ...
            cmd.time_spec = smini->get_time_now() + shd::time_spec_t(0.05);
            cmd.stream_now = (buffs.size() == 1);
            rx_stream->issue_stream_cmd(cmd);
            break;
...
void benchmark_tx_rate(
    ...
    ) % (smini->get_tx_rate()/1e6) % tx_stream->get_num_channels() << std::endl;

    //setup variables and allocate buffer
    shd::tx_metadata_t md;
    md.time_spec = smini->get_time_now() + shd::time_spec_t(INIT_DELAY);
    md.has_time_spec = (tx_stream->get_num_channels() > 1);
    const size_t max_samps_per_packet = tx_stream->get_max_num_samps();
    std::vector<char> buff(max_samps_per_packet*shd::convert::get_bytes_per_item(tx_cpu));
...
int SHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    if (vm.count("help") or (vm.count("rx_rate") + vm.count("tx_rate")) == 0){
        std::cout << boost::format("SHD Benchmark Rate %s") % desc << std::endl;
        std::cout <<
    ...
    if (not device_addrs.empty() and device_addrs.at(0).get("type", "") == "smini1"){
        ...
        std::cerr << "Benchmark results will be inaccurate on SMINI1 due to insufficient features.\n" << std::endl;
    }
    ...
    int num_mboards = smini->get_num_mboards();
    ...
    if(vm.count("ref"))
    {
        ...
        if(ref != "internal") {
            ...
            for (int i = 0; i < num_mboards; i++) {
                ...
                while((is_locked = smini->get_mboard_sensor("ref_locked",i).to_bool()) == false and
    ...
    if (vm.count("rx_rate")){
        ...
        shd::stream_args_t stream_args(rx_cpu, rx_otw);
        stream_args.channels = rx_channel_nums;
        shd::rx_streamer::sptr rx_stream = smini->get_rx_stream(stream_args);
        thread_group.create_thread(boost::bind(&benchmark_rx_rate, smini, rx_cpu, rx_stream, random_nsamps, boost::ref(burst_timer_elapsed)));
    }
    ...
    if (vm.count("tx_rate")){
        ...
        shd::stream_args_t stream_args(tx_cpu, tx_otw);
        stream_args.channels = tx_channel_nums;
        shd::tx_streamer::sptr tx_stream = smini->get_tx_stream(stream_args);
        thread_group.create_thread(boost::bind(&benchmark_tx_rate, smini, tx_cpu, tx_stream, boost::ref(burst_timer_elapsed), random_nsamps));
