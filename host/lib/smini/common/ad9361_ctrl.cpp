...
{
public:
    ad9361_io_spi(shd::spi_iface::sptr spi_iface, uint32_t slave_num) :
    ...
    virtual uint8_t peek8(uint32_t reg)
    {
        ...
        shd::spi_config_t config;
        config.mosi_edge = shd::spi_config_t::EDGE_FALL;
        config.miso_edge = shd::spi_config_t::EDGE_FALL;    //TODO (Ashish): FPGA SPI workaround. This should be EDGE_RISE
...
public:
    ...
    void set_timed_spi(shd::spi_iface::sptr spi_iface, uint32_t slave_num)
    {
        _timed_spi = boost::make_shared<ad9361_io_spi>(spi_iface, slave_num);
        _use_timed_spi();
    }

    void set_safe_spi(shd::spi_iface::sptr spi_iface, uint32_t slave_num)
    {
    ...
    void set_agc_mode(const std::string &which, const std::string &mode)
    {
        ...
        if(mode == "slow") {
            ...
            throw shd::runtime_error("ad9361_ctrl got an invalid AGC option.");
        }
    }
    ...
    double set_clock_rate(const double rate)
    {
        ...
        if (clipped_rate != rate) {
            SHD_MSG(warning) << boost::format(
                    "The requested master_clock_rate %f MHz exceeds bounds imposed by SHD.\n"
...
private:
    static ad9361_device_t::direction_t _get_direction_from_antenna(const std::string& antenna)
    {
        ...
        if (sub == "RX") {
            ...
            throw shd::runtime_error("ad9361_ctrl got an invalid channel string.");
        }
        return ad9361_device_t::RX;
    }
    ...
    static ad9361_device_t::chain_t _get_chain_from_antenna(const std::string& antenna)
    {
        ...
        if (sub == "1") {
            ...
            throw shd::runtime_error("ad9361_ctrl::set_gain got an invalid channel string.");
        }
        return ad9361_device_t::CHAIN_1;
    }
