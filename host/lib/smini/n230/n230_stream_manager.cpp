 ...
 * Receive streamer
 **********************************************************************/
rx_streamer::sptr n230_stream_manager::get_rx_stream(const shd::stream_args_t &args_)
{
 ...
 * Transmit streamer
 **********************************************************************/
tx_streamer::sptr n230_stream_manager::get_tx_stream(const shd::stream_args_t &args_)
{
    boost::mutex::scoped_lock lock(_stream_setup_mutex);
    ...
    if (not args.otw_format.empty() and args.otw_format != "sc16") {
        throw shd::value_error("n230_impl::get_tx_stream only supports otw_format sc16");
    }
    ...
    const size_t last_seq)
{
    ...
    //load payload
    pkt[packet_info.num_header_words32 + RXFC_CMD_CODE_OFFSET] = shd::htonx<uint32_t>(N230_EVENT_CODE_FLOW_CTRL);
    pkt[packet_info.num_header_words32 + RXFC_SEQ_NUM_OFFSET] = shd::htonx<uint32_t>(seq_sw);
