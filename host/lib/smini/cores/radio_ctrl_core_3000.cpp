...
private:
     ...
     ******************************************************************/
    SHD_INLINE void send_pkt(const uint32_t addr, const uint32_t data = 0)
    {
    ...
    SHD_INLINE uint64_t wait_for_ack(const bool readback)
    {
        while (readback or (_outstanding_seqs.size() >= _resp_queue_size))
        {
            ...
            if (_resp_xport)
            {
                ...
                {
                    throw shd::io_error(str(boost::format("Radio ctrl (%s) no response packet - %s") % _name % ex.what()));
                }
            ...
            {
                SHD_MSG(error) << "Radio ctrl bad VITA packet: " << ex.what() << std::endl;
                if (buff){
                    SHD_VAR(buff->size());
                }
                else{
                ...
                SHD_MSG(status) << std::hex << pkt[0] << std::dec << std::endl;
                SHD_MSG(status) << std::hex << pkt[1] << std::dec << std::endl;
                SHD_MSG(status) << std::hex << pkt[2] << std::dec << std::endl;
                SHD_MSG(status) << std::hex << pkt[3] << std::dec << std::endl;
            }
            ...
            try
            {
                ...
                SHD_ASSERT_THROW(packet_info.sid == uint32_t((_sid >> 16) | (_sid << 16)));
                SHD_ASSERT_THROW(packet_info.packet_count == (seq_to_ack & 0xfff));
                SHD_ASSERT_THROW(packet_info.num_payload_words32 == 2);
                SHD_ASSERT_THROW(packet_info.packet_type == _packet_type);
            }
            catch(const std::exception &ex)
            {
                throw shd::io_error(str(boost::format("Radio ctrl (%s) packet parse error - %s") % _name % ex.what()));
            }
