...
static clock_settings_type get_clock_settings(double rate){
    ...
    for (size_t i = 1; i <= 100; i++){
        ...
        for ( //calculate an r divider that fits into the bounds of the vco
            ...
            SHD_MSG(status) << "SMINI-E100 clock control: " << i << std::endl << cs.to_pp_string() << std::endl;
            return cs;
        }
    }

    throw shd::value_error(str(boost::format(
        "SMINI-E100 clock control: could not calculate settings for clock rate %fMHz"
    ) % (rate/1e6)));
}
...
public:
        ...
        _dboard_clocks_diff(dboard_clocks_diff)
    {
        ...
        SHD_MSG(status) << boost::format("Initializing FPGA clock to %fMHz...") % (master_clock_rate/1e6) << std::endl;
    ...
    void set_clock_settings_with_internal_vco(double rate){
        ...
        SHD_ASSERT_THROW(cs.prescaler == 8); //assumes this below:
...
private:
    ...
    void send_reg(uint16_t addr){
        ...
        SHD_LOGV(often) << "clock control write reg: " << std::hex << reg << std::endl;
        _iface->write_spi(
            UE_SPI_SS_AD9522,
    ...
    void calibrate_now(void){
        ...
        SHD_MSG(error) << "SMINI-E100 clock control: VCO calibration timeout" << std::endl;
        wait_for_ld:
        ...
        SHD_MSG(error) << "SMINI-E100 clock control: lock detection timeout" << std::endl;
    }
