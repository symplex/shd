...
const gain_range_t smini1_codec_ctrl::tx_pga_gain_range(-20, 0, double(0.1));
const gain_range_t smini1_codec_ctrl::rx_pga_gain_range(0, 20, 1);

smini1_codec_ctrl::~smini1_codec_ctrl(void){
    /* NOP */
}
 ...
 **********************************************************************/
class smini1_codec_ctrl_impl : public smini1_codec_ctrl {
public:
    //structors
    smini1_codec_ctrl_impl(spi_iface::sptr iface, int spi_slave);
    ~smini1_codec_ctrl_impl(void);
 ...
 **********************************************************************/
smini1_codec_ctrl_impl::smini1_codec_ctrl_impl(spi_iface::sptr iface, int spi_slave){
    _iface = iface;
...
smini1_codec_ctrl_impl::~smini1_codec_ctrl_impl(void){SHD_SAFE_CALL(
...
void smini1_codec_ctrl_impl::set_tx_pga_gain(double gain){
    int gain_word = int(mtpgw*(gain - tx_pga_gain_range.start())/(tx_pga_gain_range.stop() - tx_pga_gain_range.start()));
    _ad9862_regs.tx_pga_gain = shd::clip(gain_word, 0, mtpgw);
    this->send_reg(16);
}

double smini1_codec_ctrl_impl::get_tx_pga_gain(void){
...
void smini1_codec_ctrl_impl::set_rx_pga_gain(double gain, char which){
    int gain_word = int(mrpgw*(gain - rx_pga_gain_range.start())/(rx_pga_gain_range.stop() - rx_pga_gain_range.start()));
    gain_word = shd::clip(gain_word, 0, mrpgw);
    switch(which){
    case 'A':
...
double smini1_codec_ctrl_impl::get_rx_pga_gain(char which){
    int gain_word;
    switch(which){
...
double smini1_codec_ctrl_impl::read_aux_adc(aux_adc_t which){
    switch(which){
    case AUX_ADC_A1:
 ...
 **********************************************************************/
void smini1_codec_ctrl_impl::write_aux_dac(aux_dac_t which, double volts)
{
    //special case for aux dac d (aka sigma delta word)
    if (which == AUX_DAC_D) {
        uint16_t dac_word = shd::clip(boost::math::iround(volts*0xfff/3.3), 0, 0xfff);
    ...
    uint8_t dac_word = shd::clip(boost::math::iround(volts*0xff/3.3), 0, 0xff);

    //setup a lookup table for the aux dac params (reg ref, reg addr)
    typedef boost::tuple<uint8_t*, uint8_t> dac_params_t;
    shd::dict<aux_dac_t, dac_params_t> aux_dac_to_params = boost::assign::map_list_of
    ...
    SHD_ASSERT_THROW(aux_dac_to_params.has_key(which));
 ...
 **********************************************************************/
void smini1_codec_ctrl_impl::send_reg(uint8_t addr)
{
...
void smini1_codec_ctrl_impl::recv_reg(uint8_t addr)
{
 ...
 * DUC tuning 
 **********************************************************************/
double smini1_codec_ctrl_impl::coarse_tune(double codec_rate, double freq)
{
    double coarse_freq;
...
double smini1_codec_ctrl_impl::fine_tune(double codec_rate, double target_freq)
{
...
void smini1_codec_ctrl_impl::set_duc_freq(double freq, double rate)
{
...
void smini1_codec_ctrl_impl::enable_tx_digital(bool enb){
 ...
 **********************************************************************/
void smini1_codec_ctrl_impl::bypass_adc_buffers(bool bypass) {
 ...
 * Codec Control Make
 **********************************************************************/
smini1_codec_ctrl::sptr smini1_codec_ctrl::make(spi_iface::sptr iface,
                                              int spi_slave)
{
    return sptr(new smini1_codec_ctrl_impl(iface, spi_slave));
}
