 ...
 * Structors
 ***************************************************************************/
SHD_RFNOC_RADIO_BLOCK_CONSTRUCTOR(x300_radio_ctrl)
    , _ignore_cal_file(false)
{
    SHD_RFNOC_BLOCK_TRACE() << "x300_radio_ctrl_impl::ctor() " << std::endl;
    ...
    for (size_t i = 0; i < _get_num_radios(); i++) {
        ...
        _leds[i]->set_atr_mode(smini::gpio_atr::MODE_ATR, smini::gpio_atr::gpio_atr_3000::MASK_SET_ALL);
    ...
    ////////////////////////////////////////////////////////////////
    const size_t max_bytes_header = shd::transport::vrt::chdr::max_if_hdr_words64 * sizeof(uint64_t);
...
double x300_radio_ctrl_impl::set_rate(double rate)
{
    ...
    if (not shd::math::frequencies_are_equal(rate, actual_rate)) {
        SHD_MSG(warning) << "[X300 Radio] Requesting invalid sampling rate from device: " << rate/1e6 << " MHz. Actual rate is: " << actual_rate/1e6 << " MHz." << std::endl;
    }
...
double x300_radio_ctrl_impl::set_tx_gain(const double gain, const size_t chan)
{
    ...
    if (gain_stages.size() == 1) {
        ...
        SHD_MSG(warning) << "set_tx_gain: could not apply gain for this daughterboard.";
...
double x300_radio_ctrl_impl::set_rx_gain(const double gain, const size_t chan)
{
    ...
    if (gain_stages.size() == 1) {
        ...
        SHD_MSG(warning) << "set_rx_gain: could not apply gain for this daughterboard.";
        radio_ctrl_impl::set_tx_gain(0.0, chan);
        return 0.0;
    }
}
...
size_t x300_radio_ctrl_impl::get_chan_from_dboard_fe(const std::string &fe, const shd::direction_t direction)
{
    switch (direction) {
...
std::string x300_radio_ctrl_impl::get_dboard_fe_from_chan(const size_t chan, const shd::direction_t direction)
{
    switch (direction) {
        case shd::TX_DIRECTION:
...
double x300_radio_ctrl_impl::get_output_samp_rate(size_t chan)
{
    ...
    if (chan == ANY_PORT) {
        chan = 0;
        for (size_t i = 0; i < _get_num_radios(); i++) {
            if (_is_streamer_active(shd::RX_DIRECTION, chan)) {
                chan = i;
                break;
            }
        }
        ...
        bool verbose)
{
    ...
    BOOST_FOREACH(const std::string& fe, _db_manager->get_rx_frontends()) {
        ...
        _rx_fe_map[rx_chan].core->set_fe_connection(smini::fe_connection_t(conn, if_freq));
        rx_chan++;
    }
    ...
    SHD_ASSERT_THROW(rx_chan or tx_chan);
...
void x300_radio_ctrl_impl::reset_codec()
{
    ...
    SHD_ASSERT_THROW(bool(_adc));
    SHD_ASSERT_THROW(bool(_dac));
    _adc->reset();
    _dac->reset();
}
...
void x300_radio_ctrl_impl::extended_adc_test(const std::vector<x300_radio_ctrl_impl::sptr>& radios, double duration_s)
{
    ...
    SHD_MSG(status) << boost::format("Running Extended ADC Self-Test (Duration=%.0fs, %ds/iteration)...\n")
    ...
    for (size_t iter = 0; iter < num_iters; iter++) {
        ...
        //Run self-test
        SHD_MSG(status) << boost::format("-- [%s] Iteration %06d... ") % time_strm.str() % (iter+1);
        try {
            for (size_t i = 0; i < radios.size(); i++) {
                radios[i]->self_test_adc((SECS_PER_ITER*1000)/radios.size());
            }
            SHD_MSG(status) << "passed" << std::endl;
        } catch(std::exception &e) {
            num_failures++;
            SHD_MSG(status) << e.what() << std::endl;
        }
    }
    if (num_failures == 0) {
        SHD_MSG(status) << "Extended ADC Self-Test PASSED\n";
    } else {
        throw shd::runtime_error(
...
void x300_radio_ctrl_impl::synchronize_dacs(const std::vector<x300_radio_ctrl_impl::sptr>& radios)
{
    ...
    shd::time_spec_t sync_time =
        radios[0]->_time64->get_time_now() + shd::time_spec_t(((double)t_sync_us)/1e6);
    ...
    for (size_t i = 0; i < radios.size(); i++) {
        ...
        radios[i]->set_command_time(shd::time_spec_t(0.0), IO_MASTER_RADIO);
    }
...
double x300_radio_ctrl_impl::self_cal_adc_xfer_delay(
    ...
    bool apply_delay)
{
    SHD_MSG(status) << "Running ADC transfer delay self-cal: " << std::flush;
    ...
    SHD_MSG(status) << "Measuring..." << std::flush;
    ...
    for (size_t i = 0; i < NUM_DELAY_STEPS; i++) {
        ...
        //SHD_MSG(status) << (boost::format("XferDelay=%fns, Error=%d\n") % delay % err_code);
    ...
    if (win_start_idx == -1) {
        throw shd::runtime_error("self_cal_adc_xfer_delay: Self calibration failed. Convergence error.");
    }
    ...
    if (win_length < master_clk_period/4) {
        throw shd::runtime_error("self_cal_adc_xfer_delay: Self calibration failed. Valid window too narrow.");
    }
    ...
    if (apply_delay) {
        SHD_MSG(status) << "Validating..." << std::flush;
        //Apply delay
    ...
    SHD_MSG(status) << (boost::format(" done (FPGA->ADC=%.3fns%s, Window=%.3fns)\n") %
...
{
    if (print_status) SHD_MSG(status) << "Running ADC capture delay self-cal..." << std::flush;
    ...
    while (iter++ < NUM_RETRIES) {
        for (uint32_t dly_tap = 0; dly_tap < NUM_DELAY_STEPS; dly_tap++) {
            ...
            //SHD_MSG(status) << (boost::format("CapTap=%d, Error=%d\n") % dly_tap % err_code);
        }
    ...
    if (win_start == -1) {
        throw shd::runtime_error("self_cal_adc_capture_delay: Self calibration failed. Convergence error.");
    }

    if (win_stop-win_start < MIN_WINDOW_LEN) {
        throw shd::runtime_error("self_cal_adc_capture_delay: Self calibration failed. Valid window too narrow.");
    }
    ...
    if (print_status) {
        ...
        SHD_MSG(status) << boost::format(" done (Tap=%d, Window=%d, TapDelay=%.3fps, Iter=%d)\n") % ideal_tap % (win_stop-win_start) % tap_delay % iter;
    }
}
...
void x300_radio_ctrl_impl::_set_db_eeprom(i2c_iface::sptr i2c, const size_t addr, const shd::smini::dboard_eeprom_t &db_eeprom)
{
...
{
    SHD_RFNOC_BLOCK_TRACE() << "x300_radio_ctrl_impl::check_radio_config() " << std::endl;
    ...
    for (size_t chan = 0; chan < _get_num_radios(); chan++) {
        if (_tree->exists(rx_fe_path / _rx_fe_map.at(chan).db_fe_name / "enabled")) {
            const bool chan_active = _is_streamer_active(shd::RX_DIRECTION, chan);
            if (chan_active) {
    ...
    for (size_t chan = 0; chan < _get_num_radios(); chan++) {
        if (_tree->exists(tx_fe_path / _tx_fe_map.at(chan).db_fe_name / "enabled")) {
            const bool chan_active = _is_streamer_active(shd::TX_DIRECTION, chan);
            if (chan_active) {
 ...
 * Register block
 ***************************************************************************/
SHD_RFNOC_BLOCK_REGISTER(x300_radio_ctrl, "X300Radio");
