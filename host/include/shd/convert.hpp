...
#ifndef INCLUDED_SHD_CONVERT_HPP
#define INCLUDED_SHD_CONVERT_HPP
...
namespace shd{ namespace convert{

    //! A conversion class that implements a conversion from inputs -> outputs.
    class converter{
    public:
        typedef boost::shared_ptr<converter> sptr;
        typedef shd::ref_vector<void *> output_type;
        typedef shd::ref_vector<const void *> input_type;
        ...
        SHD_INLINE void conv(const input_type &in, const output_type &out, const size_t num){
    ...
    struct SHD_API id_type : boost::equality_comparable<id_type>{
    ...
    SHD_API bool operator==(const id_type &, const id_type &);
     ...
     */
    SHD_API void register_converter(
        const id_type &id,
     ...
     */
    SHD_API function_type get_converter(
        const id_type &id,
     ...
     */
    SHD_API void register_bytes_per_item(
        const std::string &format, const size_t size
    );

    //! Convert an item format to a size in bytes
    SHD_API size_t get_bytes_per_item(const std::string &format);

}} //namespace

#endif /* INCLUDED_SHD_CONVERT_HPP */
