...
# TODO: make it autodetect SHD devices
...
SMINI2_FW_PROTO_VERSION = 7 #should be unused after r6
...
class update_id_t:
  SMINI2_FW_UPDATE_ID_WAT = ord(' ')
  SMINI2_FW_UPDATE_ID_OHAI_LOL = ord('a')
  SMINI2_FW_UPDATE_ID_OHAI_OMG = ord('A')
  SMINI2_FW_UPDATE_ID_WATS_TEH_FLASH_INFO_LOL = ord('f')
  SMINI2_FW_UPDATE_ID_HERES_TEH_FLASH_INFO_OMG = ord('F')
  SMINI2_FW_UPDATE_ID_ERASE_TEH_FLASHES_LOL = ord('e')
  SMINI2_FW_UPDATE_ID_ERASING_TEH_FLASHES_OMG = ord('E')
  SMINI2_FW_UPDATE_ID_R_U_DONE_ERASING_LOL = ord('d')
  SMINI2_FW_UPDATE_ID_IM_DONE_ERASING_OMG = ord('D')
  SMINI2_FW_UPDATE_ID_NOPE_NOT_DONE_ERASING_OMG = ord('B')
  SMINI2_FW_UPDATE_ID_WRITE_TEH_FLASHES_LOL = ord('w')
  SMINI2_FW_UPDATE_ID_WROTE_TEH_FLASHES_OMG = ord('W')
  SMINI2_FW_UPDATE_ID_READ_TEH_FLASHES_LOL = ord('r')
  SMINI2_FW_UPDATE_ID_KK_READ_TEH_FLASHES_OMG = ord('R')
  SMINI2_FW_UPDATE_ID_RESET_MAH_COMPUTORZ_LOL = ord('s')
  SMINI2_FW_UPDATE_ID_RESETTIN_TEH_COMPUTORZ_OMG = ord('S')
  SMINI2_FW_UPDATE_ID_I_CAN_HAS_HW_REV_LOL = ord('v')
  SMINI2_FW_UPDATE_ID_HERES_TEH_HW_REV_OMG = ord('V')
  SMINI2_FW_UPDATE_ID_KTHXBAI = ord('~')
...
########################################################################
# print equivalent shd_image_loader command
########################################################################
def print_image_loader_warning(fw, fpga, reset, safe, addr):
    ...
    # Generate shd_image_loader command based on given arguments
    shd_image_loader = "shd_image_loader --args=\"type=smini2,addr={0}".format(addr)
    if reset:
    ...
    if fw:
        shd_image_loader += "{0}--fw-path=\"{1}\"".format(nl, fw)
    else:
        shd_image_loader += "{0}--no-fw".format(nl)

    if fpga:
        shd_image_loader += "{0}--fpga-path=\"{1}\"".format(nl, fpga)
    else:
        shd_image_loader += "{0}--no-fpga".format(nl)

    print("")
    print("************************************************************************************************")
    print("WARNING: This utility will be removed in an upcoming version of SHD. In the future, use")
    print("         this command:")
    print("")
    print(shd_image_loader)
    print("")
    print("************************************************************************************************")
    print("")
...
def enumerate_devices():
    for bcast_addr in get_interfaces():
        ...
        sock.settimeout(0.1)
        out_pkt = pack_flash_args_fmt(SMINI2_FW_PROTO_VERSION, update_id_t.SMINI2_FW_UPDATE_ID_OHAI_LOL, 0, 0, 0)
        ...
        while(still_goin):
            try:
                ...
                if(pktid == update_id_t.SMINI2_FW_UPDATE_ID_OHAI_OMG):
...
class burner_socket(object):
    ...
    def init_update(self,quiet):
        out_pkt = pack_flash_args_fmt(SMINI2_FW_PROTO_VERSION, update_id_t.SMINI2_FW_UPDATE_ID_OHAI_LOL, seq(), 0, 0)
        ...
        if pktid == update_id_t.SMINI2_FW_UPDATE_ID_OHAI_OMG:
            if not quiet: print("SMINI-N2XX found.")
        else:
            raise Exception("Invalid reply received from device.")

    def get_hw_rev(self):
        out_pkt = pack_flash_hw_rev_fmt(SMINI2_FW_PROTO_VERSION, update_id_t.SMINI2_FW_UPDATE_ID_I_CAN_HAS_HW_REV_LOL, seq(), 0)
        in_pkt = self.send_and_recv(out_pkt)
        (proto_ver, pktid, rxseq, hw_rev) = unpack_flash_hw_rev_fmt(in_pkt)
        if(pktid != update_id_t.SMINI2_FW_UPDATE_ID_HERES_TEH_HW_REV_OMG): hw_rev = 0
    ...
    def get_flash_info(self):
        ...
        out_pkt = pack_flash_args_fmt(SMINI2_FW_PROTO_VERSION, update_id_t.SMINI2_FW_UPDATE_ID_WATS_TEH_FLASH_INFO_LOL, seq(), 0, 0)
        in_pkt = self.send_and_recv(out_pkt)

        (proto_ver, pktid, rxseq, self.sector_size_bytes, self.memory_size_bytes) = unpack_flash_info_fmt(in_pkt)

        if pktid != update_id_t.SMINI2_FW_UPDATE_ID_HERES_TEH_FLASH_INFO_OMG:
    ...
    def write_image(self, image, addr):
        ...
        while writedata:
            out_pkt = pack_flash_args_fmt(SMINI2_FW_PROTO_VERSION, update_id_t.SMINI2_FW_UPDATE_ID_WRITE_TEH_FLASHES_LOL, seq(), addr, FLASH_DATA_PACKET_SIZE, writedata[:FLASH_DATA_PACKET_SIZE])
            in_pkt = self.send_and_recv(out_pkt)

            (proto_ver, pktid, rxseq, flash_addr, rxlength, data) = unpack_flash_args_fmt(in_pkt)

            if pktid != update_id_t.SMINI2_FW_UPDATE_ID_WROTE_TEH_FLASHES_OMG:
              raise Exception("Invalid reply %c from device." % (chr(pktid)))
    ...
    def verify_image(self, image, addr):
        ...
        while readsize > 0:
            ...
            out_pkt = pack_flash_args_fmt(SMINI2_FW_PROTO_VERSION, update_id_t.SMINI2_FW_UPDATE_ID_READ_TEH_FLASHES_LOL, seq(), addr, thisreadsize)
            in_pkt = self.send_and_recv(out_pkt)

            (proto_ver, pktid, rxseq, flash_addr, rxlength, data) = unpack_flash_args_fmt(in_pkt)

            if pktid != update_id_t.SMINI2_FW_UPDATE_ID_KK_READ_TEH_FLASHES_OMG:
              raise Exception("Invalid reply %c from device." % (chr(pktid)))
    ...
    def reset_smini(self):
        out_pkt = pack_flash_args_fmt(SMINI2_FW_PROTO_VERSION, update_id_t.SMINI2_FW_UPDATE_ID_RESET_MAH_COMPUTORZ_LOL, seq(), 0, 0)
        try: in_pkt = self.send_and_recv(out_pkt)
        except socket.timeout: return

        (proto_ver, pktid, rxseq, flash_addr, rxlength, data) = unpack_flash_args_fmt(in_pkt)
        if pktid == update_id_t.SMINI2_FW_UPDATE_ID_RESETTIN_TEH_COMPUTORZ_OMG:
    ...
    def erase_image(self, addr, length):
        ...
        out_pkt = pack_flash_args_fmt(SMINI2_FW_PROTO_VERSION, update_id_t.SMINI2_FW_UPDATE_ID_ERASE_TEH_FLASHES_LOL, seq(), addr, length)
        in_pkt = self.send_and_recv(out_pkt)

        (proto_ver, pktid, rxseq, flash_addr, rxlength, data) = unpack_flash_args_fmt(in_pkt)

        if pktid != update_id_t.SMINI2_FW_UPDATE_ID_ERASING_TEH_FLASHES_OMG:
            raise Exception("Invalid reply %c from device." % (chr(pktid)))
        ...
        while(True):
            out_pkt = pack_flash_args_fmt(SMINI2_FW_PROTO_VERSION, update_id_t.SMINI2_FW_UPDATE_ID_R_U_DONE_ERASING_LOL, seq(), 0, 0)
            in_pkt = self.send_and_recv(out_pkt)

            (proto_ver, pktid, rxseq, flash_addr, rxlength, data) = unpack_flash_args_fmt(in_pkt)

            if pktid == update_id_t.SMINI2_FW_UPDATE_ID_IM_DONE_ERASING_OMG: break
            elif pktid != update_id_t.SMINI2_FW_UPDATE_ID_NOPE_NOT_DONE_ERASING_OMG:
                raise Exception("Invalid reply %c from device." % (chr(pktid)))
...
def get_options():
    ...
    parser.add_option("--addr", type="string",                 help="SMINI-N2XX device address",       default='')
...
if __name__=='__main__':
    ...
    if options.overwrite_safe and not options.read:
        ...
        print("If your image is faulty, your SMINI2+ will become a brick until reprogrammed via JTAG.")
