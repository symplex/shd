...
static const shd::dict<std::string, freq_range_t> tvrx_freq_ranges = map_list_of
...
static const shd::dict<std::string, boost::array<double, 17> > tvrx_rf_gains_db = map_list_of
...
static shd::dict<std::string, gain_range_t> get_tvrx_gain_ranges(void) {
...
SHD_STATIC_BLOCK(reg_tvrx_dboard){
...
static std::string get_band(double freq) {
    BOOST_FOREACH(const std::string &band, tvrx_freq_ranges.keys()) {
        if(freq >= tvrx_freq_ranges[band].start() && freq <= tvrx_freq_ranges[band].stop()){
            SHD_LOGV(often) << "Band: " << band << std::endl;
            return band;
        }
    }
...
static double gain_interp(double gain, const boost::array<double, 17>& db_vector, const boost::array<double, 17>& volts_vector) {
    double volts;
    gain = shd::clip<double>(gain, db_vector.front(), db_vector.back()); //let's not get carried away here
    ...
    SHD_LOGV(often) << "Gain interp: gain: " << gain << ", gain_step: " << int(gain_step) << ", slope: " << slope << ", volts: " << volts << std::endl;
...
static double rf_gain_to_voltage(double gain, double lo_freq){
    ...
    //this is the voltage at the SMINI DAC output
    double dac_volts = gain_volts / opamp_gain;

    dac_volts = shd::clip<double>(dac_volts, 0.0, 3.3);
...
double tvrx::set_freq(double freq) {
    ...
    if((divisor & ~0x7fff)) SHD_THROW_INVALID_CODE_PATH();
    ...
    SHD_LOGV(often) << boost::format("set_freq: target LO: %f f_ref: %f divisor: %i actual LO: %f") % target_lo_freq % f_ref % divisor % actual_lo_freq << std::endl;
