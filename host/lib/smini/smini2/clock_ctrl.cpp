...
#include "smini2_regs.hpp" //spi slave constants
#include "smini2_clk_regs.hpp"
...
smini2_clock_ctrl::~smini2_clock_ctrl(void){
    /* NOP */
}

/*!
 * A smini2 clock control specific to the ad9510 ic.
 */
class smini2_clock_ctrl_impl : public smini2_clock_ctrl{
public:
    smini2_clock_ctrl_impl(smini2_iface::sptr iface, shd::spi_iface::sptr spiface){
        _iface = iface;
        _spiface = spiface;
        clk_regs = smini2_clk_regs_t(_iface->get_rev());
    ...
    ~smini2_clock_ctrl_impl(void){SHD_SAFE_CALL(
    ...
    //uses output clock 6 (cmos) on SMINI2, output clock 5 (cmos) on N200/N210 r3,
    ...
    void enable_tx_dboard_clock(bool enb){
        ...
        default:
          //throw shd::not_implemented_error("enable_tx_dboard_clock: unknown hardware version");
          break;
        }
...
private:
    ...
    //this is the same between SMINI2 and SMINI2+ and doesn't get a switch statement
    ...
    //uses output clock 4 (lvds) on SMINI2 and output clock 2 (lvpecl) on SMINI2+
    ...
    smini2_iface::sptr _iface;
    shd::spi_iface::sptr _spiface;
    smini2_clk_regs_t clk_regs;
 ...
 **********************************************************************/
smini2_clock_ctrl::sptr smini2_clock_ctrl::make(smini2_iface::sptr iface, shd::spi_iface::sptr spiface){
    return sptr(new smini2_clock_ctrl_impl(iface, spiface));
}
