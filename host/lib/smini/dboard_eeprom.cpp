...
static uint8_t checksum(const byte_vector_t &bytes){
    ...
    SHD_LOGV(often) << boost::format("sum: 0x%02x") % sum << std::endl;
    return uint8_t(sum);
}
...
void dboard_eeprom_t::load(i2c_iface &iface, uint8_t addr){
    ...
    SHD_LOGV(often) << ss.str() << std::endl;

    try{
        SHD_ASSERT_THROW(bytes.size() >= DB_EEPROM_CLEN);
        SHD_ASSERT_THROW(bytes[DB_EEPROM_MAGIC] == DB_EEPROM_MAGIC_VALUE);
        SHD_ASSERT_THROW(bytes[DB_EEPROM_CHKSUM] == checksum(bytes));
    ...
    }catch(const shd::assertion_error &){
        id = dboard_id_t::none();
        serial = "";
    }
}
