...
bool find_safe_booted_flag(void) {
	...
	eeprom_read(SMINI2_I2C_ADDR_MBOARD, SMINI2_EE_MBOARD_BOOTLOADER_FLAGS, &flag_byte, 1);
	return (flag_byte == 0x5E);
}

void set_safe_booted_flag(bool flag) {
	unsigned char flag_byte = flag ? 0x5E : 0xDC;
	eeprom_write(SMINI2_I2C_ADDR_MBOARD, SMINI2_EE_MBOARD_BOOTLOADER_FLAGS, &flag_byte, 1);
}
