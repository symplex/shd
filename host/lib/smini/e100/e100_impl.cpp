...
device_addrs_t e100_find(const device_addr_t &hint){
    ...
    //return an empty list of addresses when type is set to non-smini-e
...
SHD_STATIC_BLOCK(register_e100_device){
    device::register_device(&e100_find, &e100_make, device::SMINI);
}

static const shd::dict<std::string, std::string> model_to_fpga_file_name = boost::assign::map_list_of
    ("E100", "smini_e100_fpga_v2.bin")
    ("E110", "smini_e110_fpga.bin")
;
...
std::string get_default_e1x0_fpga_image(const shd::device_addr_t &device_addr){
    ...
    shd::i2c_iface::sptr dev_i2c_iface = e100_ctrl::make_dev_i2c_iface(E100_I2C_DEV_NODE);
    ...
    if (not model_to_fpga_file_name.has_key(model)) throw shd::runtime_error(str(boost::format(
        "\n"
 ...
 * Structors
 **********************************************************************/
e100_impl::e100_impl(const shd::device_addr_t &device_addr){
    ...
    try{
        ...
        SHD_MSG(error) << boost::format("Could not find FPGA image. %s\n") % print_utility_error("shd_images_downloader.py");
        throw;
    }
    ...
    SHD_MSG(status) << "Performing control readback test... " << std::flush;
    ...
    if (test_fail) SHD_MSG(error) << boost::format(
    ...
    if (_gps and _gps->gps_detected()){
        ...
        SHD_MSG(status) << "Initializing time to the internal GPSDO" << std::endl;
...
void e100_impl::set_mb_eeprom(const shd::smini::mboard_eeprom_t &mb_eeprom){
    mb_eeprom.commit(*_dev_i2c_iface, E100_EEPROM_MAP_KEY);
}

void e100_impl::set_db_eeprom(const std::string &type, const shd::smini::dboard_eeprom_t &db_eeprom){
