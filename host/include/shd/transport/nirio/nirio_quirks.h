...
#ifndef INCLUDED_SHD_TRANSPORT_NIRIO_NIRIO_QUIRKS_H
#define INCLUDED_SHD_TRANSPORT_NIRIO_NIRIO_QUIRKS_H
...
#define SHD_NIRIO_RX_FIFO_XFER_CHECK_EN 0
...
public:
    ...
    SHD_INLINE void register_tx_streams(const uint32_t tx_stream_indices[], size_t size) {
        for (size_t i = 0; i < size; i++) {
            _tx_stream_fifo_indices.insert(tx_stream_indices[i]);
        }
    }

    SHD_INLINE void add_tx_fifo(uint32_t index) {
        if (_tx_stream_fifo_indices.find(index) != _tx_stream_fifo_indices.end()) {
            if (_tx_stream_count == 0) {
                SHD_LOG << "NI-RIO RX FIFO Transfer Check Quirk Enabled.";
            }
            _tx_stream_count++;
        }
    }

    SHD_INLINE void remove_tx_fifo(uint32_t index) {
        if (_tx_stream_fifo_indices.find(index) != _tx_stream_fifo_indices.end()) {
            _tx_stream_count--;
            if (_tx_stream_count == 0) {
                SHD_LOG << "NI-RIO RX FIFO Transfer Check Quirk Disabled.";
            }
        }
    }
    ...
    //         current shd_device configuration (dynamic)
    SHD_INLINE bool rx_fifo_xfer_check_en() const {
...
#endif /* INCLUDED_SHD_TRANSPORT_NIRIO_NIRIO_QUIRKS_H */
