...
public:
    ...
    void release(void){
        if (fp_verbose) SHD_LOGV(always) << "recv buff: release" << std::endl;
        _info->flags = RB_KERNEL; //release the frame
    }

    SHD_INLINE bool ready(void){return _info->flags & RB_USER;}

    SHD_INLINE sptr get_new(void){
        if (fp_verbose) SHD_LOGV(always) << "  make_recv_buff: " << _info->len << std::endl;
...
public:
    ...
    void release(void){
        if (fp_verbose) SHD_LOGV(always) << "send buff: commit " << size() << std::endl;
        _info->len = _len;//size();
        _info->flags = RB_USER; //release the frame
        if (::write(_fd, NULL, 0) < 0){ //notifies the kernel
            SHD_LOGV(rarely) << SHD_THROW_SITE_INFO("write error") << std::endl;
        }
    }

    SHD_INLINE bool ready(void){return _info->flags & RB_KERNEL;}

    SHD_INLINE sptr get_new(void){
        if (fp_verbose) SHD_LOGV(always) << "  make_send_buff: " << _len << std::endl;
        _info->flags = RB_USER_PROCESS; //claim the frame
...
public:
    ...
    {
        //get system sizes
        iface->ioctl(SMINI_E_GET_RB_INFO, &_rb_size);
        ...
        SHD_ASSERT_THROW(_mapped_mem != MAP_FAILED);
    ...
    managed_recv_buffer::sptr get_recv_buff(double timeout){
        if (fp_verbose) SHD_LOGV(always) << "get_recv_buff: " << _recv_index << std::endl;
        ...
        if (not mrb.ready()){
            for (size_t i = 0; i < poll_breakout; i++){
                ...
                if (fp_verbose) SHD_LOGV(always) << "  POLLIN: " << poll_ret << std::endl;
    ...
    managed_send_buffer::sptr get_send_buff(double timeout){
        if (fp_verbose) SHD_LOGV(always) << "get_send_buff: " << _send_index << std::endl;
        ...
        if (not msb.ready()){
            ...
            if (fp_verbose) SHD_LOGV(always) << "  POLLOUT: " << poll_ret << std::endl;
...
private:
    ...
    //mapped memory sizes
    smini_e_ring_buffer_size_t _rb_size;
