...
#include "smini2_regs.hpp" //wishbone address constants
...
class smini2_dboard_iface : public dboard_iface{
public:
    smini2_dboard_iface(
        timed_wb_iface::sptr wb_iface,
        shd::i2c_iface::sptr i2c_iface,
        shd::spi_iface::sptr spi_iface,
        smini2_clock_ctrl::sptr clock_ctrl
    );
    ~smini2_dboard_iface(void);
...
private:
    ...
    shd::i2c_iface::sptr _i2c_iface;
    shd::spi_iface::sptr _spi_iface;
    smini2_clock_ctrl::sptr _clock_ctrl;
 ...
 * Make Function
 **********************************************************************/
dboard_iface::sptr make_smini2_dboard_iface(
    timed_wb_iface::sptr wb_iface,
    shd::i2c_iface::sptr i2c_iface,
    shd::spi_iface::sptr spi_iface,
    smini2_clock_ctrl::sptr clock_ctrl
){
    return dboard_iface::sptr(new smini2_dboard_iface(wb_iface, i2c_iface, spi_iface, clock_ctrl));
}

/***********************************************************************
 * Structors
 **********************************************************************/
smini2_dboard_iface::smini2_dboard_iface(
    timed_wb_iface::sptr wb_iface,
    shd::i2c_iface::sptr i2c_iface,
    shd::spi_iface::sptr spi_iface,
    smini2_clock_ctrl::sptr clock_ctrl
...
smini2_dboard_iface::~smini2_dboard_iface(void){
    /* NOP */
}
 ...
 * Clocks
 **********************************************************************/
void smini2_dboard_iface::set_clock_rate(unit_t unit, double rate){
    if (unit == UNIT_BOTH) throw shd::runtime_error("UNIT_BOTH not supported.");
...
double smini2_dboard_iface::get_clock_rate(unit_t unit){
    if (unit == UNIT_BOTH) throw shd::runtime_error("UNIT_BOTH not supported.");
    return _clock_rates[unit]; //get from shadow
}

std::vector<double> smini2_dboard_iface::get_clock_rates(unit_t unit){
    if (unit == UNIT_BOTH) throw shd::runtime_error("UNIT_BOTH not supported.");
    switch(unit){
...
void smini2_dboard_iface::set_clock_enabled(unit_t unit, bool enb){
    if (unit == UNIT_BOTH) throw shd::runtime_error("UNIT_BOTH not supported.");
    switch(unit){
...
double smini2_dboard_iface::get_codec_rate(unit_t unit){
    if (unit == UNIT_BOTH) throw shd::runtime_error("UNIT_BOTH not supported.");
 ...
 * GPIO
 **********************************************************************/
void smini2_dboard_iface::set_pin_ctrl(unit_t unit, uint32_t value, uint32_t mask){
    _gpio->set_pin_ctrl(unit, static_cast<uint16_t>(value), static_cast<uint16_t>(mask));
}

uint32_t smini2_dboard_iface::get_pin_ctrl(unit_t unit){
    return static_cast<uint32_t>(_gpio->get_pin_ctrl(unit));
}

void smini2_dboard_iface::set_atr_reg(unit_t unit, atr_reg_t reg, uint32_t value, uint32_t mask){
    _gpio->set_atr_reg(unit, reg, static_cast<uint16_t>(value), static_cast<uint16_t>(mask));
}

uint32_t smini2_dboard_iface::get_atr_reg(unit_t unit, atr_reg_t reg){
    return static_cast<uint32_t>(_gpio->get_atr_reg(unit, reg));
}

void smini2_dboard_iface::set_gpio_ddr(unit_t unit, uint32_t value, uint32_t mask){
    _gpio->set_gpio_ddr(unit, static_cast<uint16_t>(value), static_cast<uint16_t>(mask));
}

uint32_t smini2_dboard_iface::get_gpio_ddr(unit_t unit){
    return static_cast<uint32_t>(_gpio->get_gpio_ddr(unit));
}

void smini2_dboard_iface::set_gpio_out(unit_t unit, uint32_t value, uint32_t mask){
    _gpio->set_gpio_out(unit, static_cast<uint16_t>(value), static_cast<uint16_t>(mask));
}

uint32_t smini2_dboard_iface::get_gpio_out(unit_t unit){
    return static_cast<uint32_t>(_gpio->get_gpio_out(unit));
}

uint32_t smini2_dboard_iface::read_gpio(unit_t unit){
 ...
 * SPI
 **********************************************************************/
static const shd::dict<dboard_iface::unit_t, int> unit_to_spi_dev = map_list_of
    (dboard_iface::UNIT_TX, SPI_SS_TX_DB)
    (dboard_iface::UNIT_RX, SPI_SS_RX_DB)
;

void smini2_dboard_iface::write_spi(
    unit_t unit,
    const spi_config_t &config,
    uint32_t data,
    size_t num_bits
...
uint32_t smini2_dboard_iface::read_write_spi(
    unit_t unit,
    const spi_config_t &config,
    uint32_t data,
    size_t num_bits
 ...
 * I2C
 **********************************************************************/
void smini2_dboard_iface::write_i2c(uint16_t addr, const byte_vector_t &bytes){
    return _i2c_iface->write_i2c(addr, bytes);
}

byte_vector_t smini2_dboard_iface::read_i2c(uint16_t addr, size_t num_bytes){
 ...
 * Aux DAX/ADC
 **********************************************************************/
void smini2_dboard_iface::_write_aux_dac(unit_t unit){
    static const shd::dict<unit_t, int> unit_to_spi_dac = map_list_of
...
void smini2_dboard_iface::write_aux_dac(unit_t unit, aux_dac_t which, double value){
    if (unit == UNIT_BOTH) throw shd::runtime_error("UNIT_BOTH not supported.");

    _dac_regs[unit].data = boost::math::iround(4095*value/3.3);
    _dac_regs[unit].cmd = ad5623_regs_t::CMD_WR_UP_DAC_CHAN_N;

    typedef shd::dict<aux_dac_t, ad5623_regs_t::addr_t> aux_dac_to_addr;
    static const shd::dict<unit_t, aux_dac_to_addr> unit_to_which_to_addr = map_list_of
        (UNIT_RX, map_list_of
...
double smini2_dboard_iface::read_aux_adc(unit_t unit, aux_adc_t which){
    static const shd::dict<unit_t, int> unit_to_spi_adc = map_list_of
...
shd::time_spec_t smini2_dboard_iface::get_command_time()
{
    return _wb_iface->get_time();
}

void smini2_dboard_iface::set_command_time(const shd::time_spec_t& t)
{
    _wb_iface->set_time(t);
}

void smini2_dboard_iface::set_fe_connection(unit_t, const std::string&, const fe_connection_t&)
{
