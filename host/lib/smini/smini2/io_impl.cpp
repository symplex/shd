 ...
 * helpers
 **********************************************************************/
static SHD_INLINE pt::time_duration to_time_dur(double timeout){
...
public:
     ...
     */
    SHD_INLINE seq_type get_curr_seq_out(void){
     ...
     */
    SHD_INLINE bool check_fc_condition(double timeout){
     ...
     */
    SHD_INLINE void update_fc_condition(seq_type seq){
        boost::mutex::scoped_lock lock(_fc_mutex);
        _last_seq_ack = seq;
        lock.unlock();
        _fc_cond.notify_one();
...
struct smini2_impl::io_impl{
    ...
    managed_send_buffer::sptr get_send_buff(size_t chan, double timeout){
        ...
        if (buff.get()) buff->cast<uint32_t *>()[0] = shd::htonx(fc_mon.get_curr_seq_out());
 ...
 **********************************************************************/
void smini2_impl::io_impl::recv_pirate_loop(
    ...
    while (not boost::this_thread::interruption_requested()){
        ...
        try{
            ...
            if (if_packet_info.sid == SMINI2_TX_ASYNC_SID and if_packet_info.packet_type != vrt::if_packet_info_t::PACKET_TYPE_DATA){

                //fill in the async metadata
                async_metadata_t metadata;
                load_metadata_from_buff(shd::ntohx<uint32_t>, metadata, if_packet_info, vrt_hdr, tick_rate, index);

                //catch the flow control packets and react
                if (metadata.event_code == 0){
                    uint32_t fc_word32 = (vrt_hdr + if_packet_info.num_header_words32)[1];
                    fc_mon.update_fc_condition(shd::ntohx(fc_word32));
                    continue;
                }
                //else SHD_MSG(often) << "metadata.event_code " << metadata.event_code << std::endl;
            ...
            SHD_MSG(error) << "Error in recv pirate loop: " << e.what() << std::endl;
        }
    }
}
 ...
 * Helper Functions
 **********************************************************************/
void smini2_impl::io_init(void){
    //create new io impl
    _io_impl = SHD_PIMPL_MAKE(io_impl, ());
    ...
    BOOST_FOREACH(const std::string &mb, _mbc.keys()){
        ...
        _io_impl->fc_mons.push_back(flow_control_monitor::sptr(new flow_control_monitor(
            device_addr.cast("send_buff_size", SMINI2_SRAM_BYTES) /
    ...
    BOOST_FOREACH(const std::string &mb, _mbc.keys()){
        ...
        _io_impl->pirate_tasks.push_back(task::make(boost::bind(
            &smini2_impl::io_impl::recv_pirate_loop, _io_impl.get(),
            _mbc[mb].tx_dsp_xport, index++
        )));
    }
}

void smini2_impl::update_tick_rate(const double rate){
...
void smini2_impl::update_rx_samp_rate(const std::string &mb, const size_t dsp, const double rate){
    boost::shared_ptr<sph::recv_packet_streamer> my_streamer =
...
void smini2_impl::update_tx_samp_rate(const std::string &mb, const size_t dsp, const double rate){
    boost::shared_ptr<sph::send_packet_streamer> my_streamer =
...
void smini2_impl::update_rates(void){
    BOOST_FOREACH(const std::string &mb, _mbc.keys()){
...
void smini2_impl::update_rx_subdev_spec(const std::string &which_mb, const subdev_spec_t &spec){
...
void smini2_impl::update_tx_subdev_spec(const std::string &which_mb, const subdev_spec_t &spec){
    fs_path root = "/mboards/" + which_mb + "/dboards";
 ...
 * Async Data
 **********************************************************************/
bool smini2_impl::recv_async_msg(
 ...
 **********************************************************************/
void smini2_impl::program_stream_dest(
    zero_copy_if::sptr &xport, const shd::stream_args_t &args
){
    //perform an initial flush of transport
    while (xport->get_recv_buff(0.0)){}

    //program the stream command
    smini2_stream_ctrl_t stream_ctrl = smini2_stream_ctrl_t();
    stream_ctrl.sequence = shd::htonx(uint32_t(0 /* don't care seq num */));
    stream_ctrl.vrt_hdr = shd::htonx(uint32_t(SMINI2_INVALID_VRT_HEADER));
    ...
    if (args.args.has_key("addr") and args.args.has_key("port")){
        ...
        stream_ctrl.ip_addr = shd::htonx(uint32_t(endpoint.address().to_v4().to_ulong()));
        stream_ctrl.udp_port = shd::htonx(uint32_t(endpoint.port()));

        for (size_t i = 0; i < 3; i++){
            SHD_MSG(status) << "ARP attempt " << i << std::endl;
            ...
            if (recv_buff and recv_buff->size() >= sizeof(uint32_t)){
                const uint32_t result = shd::ntohx(recv_buff->cast<const uint32_t *>()[0]);
                if (result == 0){
                    SHD_MSG(status) << "Success! " << std::endl;
                    return;
                }
            }
        }
        throw shd::runtime_error("Device failed to ARP when programming alternative streaming destination.");
    }
 ...
 * Receive streamer
 **********************************************************************/
rx_streamer::sptr smini2_impl::get_rx_stream(const shd::stream_args_t &args_){
 ...
 * Transmit streamer
 **********************************************************************/
tx_streamer::sptr smini2_impl::get_tx_stream(const shd::stream_args_t &args_){
    stream_args_t args = args_;
    ...
    for (size_t chan_i = 0; chan_i < args.channels.size(); chan_i++){
        ...
        BOOST_FOREACH(const std::string &mb, _mbc.keys()){
            ...
            if (chan < num_chan_so_far){
                ...
                my_streamer->set_xport_chan_get_buff(chan_i, boost::bind(
                    &smini2_impl::io_impl::get_send_buff, _io_impl.get(), abs, _1
                ));
